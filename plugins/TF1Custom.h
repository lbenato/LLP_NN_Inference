...
#ifndef ROOT_TF1Custom
#define ROOT_TF1Custom
...
class TF1CustomParameters {
 public:
  TF1CustomParameters() {} // needed for the I/O
 TF1CustomParameters(Int_t npar) :
  ...
  // copy constructor
 TF1CustomParameters(const TF1CustomParameters &rhs) :
  fParameters(rhs.fParameters),
    fParNames(rhs.fParNames)
      {}
  // assignment
  TF1CustomParameters &operator=(const TF1CustomParameters &rhs)
    {
  ...
  ClassDef(TF1CustomParameters, 1)  // The Parameters of a parameteric function
    private:
...
namespace ROOT {
  namespace Internal {
    /// Internal class used by TF1Custom for defining
    /// template specialization for different TF1Custom constructors
      template<class Func>
	struct TF1CustomBuilder {
	  static void Build(TF1Custom *f, Func func);
	};
 
      template<class Func>
	struct TF1CustomBuilder<Func *> {
	static void Build(TF1Custom *f, Func *func);
      };
 
      // Internal class used by TF1Custom for obtaining the type from a functor
      // out of the set of valid operator() signatures.
      template<typename T>
	struct GetFunctorTypeCustom {
	};
 
      template<typename F, typename T>
	struct GetFunctorTypeCustom<T(F::*)(const T *, const double *)> {
	using type = T;
      };
 
      template<typename F, typename T>
	struct GetFunctorTypeCustom<T(F::*)(const T *, const double *) const> {
	using type = T;
      };
 
      template<typename F, typename T>
	struct GetFunctorTypeCustom<T(F::*)(T *, double *)> {
	using type = T;
      };
 
      template<typename F, typename T>
	struct GetFunctorTypeCustom<T(F::*)(T *, double *) const> {
	using type = T;
      };
 
      // Internal class used by TF1Custom to get the right operator() signature
      // from a Functor with several ones.
      template<typename T, typename F>
	auto GetTheRightOpCustom(T(F::*opPtr)(const T *, const double *)) -> decltype(opPtr)
      {
	return opPtr;
      }
 
      template<typename T, typename F>
	auto GetTheRightOpCustom(T(F::*opPtr)(const T *, const double *) const) -> decltype(opPtr)
      {
	return opPtr;
      }
 
      template<typename T, typename F>
	auto GetTheRightOpCustom(T(F::*opPtr)(T *, double *)) -> decltype(opPtr)
      {
	return opPtr;
      }
 
      template<typename T, typename F>
	auto GetTheRightOpCustom(T(F::*opPtr)(T *, double *) const) -> decltype(opPtr)
      {
	return opPtr;
      }
  }
...
class TF1Custom : public TNamed, public TAttLine, public TAttFill, public TAttMarker {
 
   template<class Func>
     friend struct ROOT::Internal::TF1CustomBuilder;
 ...
 public:
   ...
   struct TF1CustomFunctorPointer {
     virtual  ~TF1CustomFunctorPointer() {}
     virtual  TF1CustomFunctorPointer * Clone() const = 0;
   };
 ...
 protected:
   ...
   std::unique_ptr<TF1CustomFunctorPointer>  fFunctor; //! Functor object to wrap any C++ callable object
   std::unique_ptr<TFormula>   fFormula;        //Pointer to TFormula in case when user define formula
   std::unique_ptr<TF1CustomParameters> fParams;   //Pointer to Function parameters object (exists only for not-formula functions)
   std::unique_ptr<TF1AbsComposition> fComposition; //Pointer to composition (NSUM or CONV)
 
   /// General constructor for TF1Custom. Most of the other constructors delegate on it
 TF1Custom(EFType functionType, const char *name, Double_t xmin, Double_t xmax, Int_t npar, Int_t ndim, EAddToList addToGlobList, TF1CustomParameters *params = nullptr, TF1CustomFunctorPointer * functor = nullptr):
 ...
 protected:
   ...
   template <class T>
     struct TF1CustomFunctorPointerImpl: TF1CustomFunctorPointer {
   TF1CustomFunctorPointerImpl(const ROOT::Math::ParamFunctorTempl<T> &func): fImpl(func) {};
   TF1CustomFunctorPointerImpl(const std::function<T(const T *f, const Double_t *param)> &func) : fImpl(func){};
     virtual ~TF1CustomFunctorPointerImpl() {}
     virtual  TF1CustomFunctorPointer * Clone() const { return new TF1CustomFunctorPointerImpl<T>(fImpl); }
   ...
   static TF1Custom   *fgCurrent;   //pointer to current function being processed
 ...
 public:
   ...
   TF1Custom();
   TF1Custom(const char *name, const char *formula, Double_t xmin = 0, Double_t xmax = 1, EAddToList addToGlobList = EAddToList::kDefault, bool vectorize = false);
   TF1Custom(const char *name, const char *formula, Double_t xmin, Double_t xmax, Option_t * option);  // same as above but using a string for option
   TF1Custom(const char *name, Double_t xmin, Double_t xmax, Int_t npar, Int_t ndim = 1, EAddToList addToGlobList = EAddToList::kDefault);
   TF1Custom(const char *name, Double_t (*fcn)(Double_t *, Double_t *), Double_t xmin = 0, Double_t xmax = 1, Int_t npar = 0, Int_t ndim = 1, EAddToList addToGlobList = EAddToList::kDefault);
   TF1Custom(const char *name, Double_t (*fcn)(const Double_t *, const Double_t *), Double_t xmin = 0, Double_t xmax = 1, Int_t npar = 0, Int_t ndim = 1, EAddToList addToGlobList = EAddToList::kDefault);
 
   template <class T>
     TF1Custom(const char *name, std::function<T(const T *data, const Double_t *param)> &fcn, Double_t xmin = 0, Double_t xmax = 1, Int_t npar = 0, Int_t ndim = 1, EAddToList addToGlobList = EAddToList::kDefault):
   TF1Custom(EFType::kTemplScalar, name, xmin, xmax, npar, ndim, addToGlobList, new TF1CustomParameters(npar), new TF1CustomFunctorPointerImpl<T>(fcn))
     {
       fType = std::is_same<T, double>::value ? TF1Custom::EFType::kTemplScalar : TF1Custom::EFType::kTemplVec;
     }
   ...
   template <class T>
     TF1Custom(const char *name, T(*fcn)(const T *, const Double_t *), Double_t xmin = 0, Double_t xmax = 1, Int_t npar = 0, Int_t ndim = 1, EAddToList addToGlobList = EAddToList::kDefault):
   TF1Custom(EFType::kTemplVec, name, xmin, xmax, npar, ndim, addToGlobList, new TF1CustomParameters(npar), new TF1CustomFunctorPointerImpl<T>(fcn))
     {}
 
   // Constructors using functors (compiled mode only)
   TF1Custom(const char *name, ROOT::Math::ParamFunctor f, Double_t xmin = 0, Double_t xmax = 1, Int_t npar = 0, Int_t ndim = 1, EAddToList addToGlobList = EAddToList::kDefault);
   ...
   template <typename Func>
     TF1Custom(const char *name, Func f, Double_t xmin, Double_t xmax, Int_t npar, Int_t ndim = 1, EAddToList addToGlobList = EAddToList::kDefault) :
   TF1Custom(EFType::kTemplScalar, name, xmin, xmax, npar, ndim, addToGlobList)
     {
       //actual fType set in TF1CustomBuilder
       ROOT::Internal::TF1CustomBuilder<Func>::Build(this, f);
     }
 
   // backward compatible interface
   template <typename Func>
     TF1Custom(const char *name, Func f, Double_t xmin, Double_t xmax, Int_t npar, const char *, EAddToList addToGlobList = EAddToList::kDefault) :
   TF1Custom(EFType::kTemplScalar, name, xmin, xmax, npar, 1, addToGlobList, new TF1CustomParameters(npar))
     {
       ROOT::Internal::TF1CustomBuilder<Func>::Build(this, f);
     }
   ...
   template <class PtrObj, typename MemFn>
     TF1Custom(const char *name, const  PtrObj &p, MemFn memFn, Double_t xmin, Double_t xmax, Int_t npar, Int_t ndim = 1, EAddToList addToGlobList = EAddToList::kDefault) :
   TF1Custom(EFType::kTemplScalar, name, xmin, xmax, npar, ndim, addToGlobList, new TF1CustomParameters(npar), new TF1CustomFunctorPointerImpl<double>(ROOT::Math::ParamFunctor(p, memFn)))
     {}
 
   // backward compatible interface
   template <class PtrObj, typename MemFn>
     TF1Custom(const char *name, const  PtrObj &p, MemFn memFn, Double_t xmin, Double_t xmax, Int_t npar, const char *, const char *, EAddToList addToGlobList = EAddToList::kDefault) :
   TF1Custom(EFType::kTemplScalar, name, xmin, xmax, npar, 1, addToGlobList, new TF1CustomParameters(npar), new TF1CustomFunctorPointerImpl<double>(ROOT::Math::ParamFunctor(p, memFn)))
     {}
 
   TF1Custom(const TF1Custom &f1);
   TF1Custom &operator=(const TF1Custom &rhs);
   virtual   ~TF1Custom();
   ...
   virtual TF1Custom     *DrawCopy(Option_t *option = "") const;
   ...
   virtual void     SetVectorized(Bool_t vectorized)
   {
     if (fType == EFType::kFormula && fFormula)
       ...
       Warning("SetVectorized", "Can only set vectorized flag on formula-based TF1Custom");
   }
   virtual void     Update();
 
   static  TF1Custom     *GetCurrent();
   static  void     AbsValue(Bool_t reject = kTRUE);
   static  void     RejectPoint(Bool_t reject = kTRUE);
   static  Bool_t   RejectedPoint();
   static  void     SetCurrent(TF1Custom *f1);
 ...
 private:
   ...
   ClassDef(TF1Custom, 12) // The Parametric 1-D function
     };
...
namespace ROOT {
  namespace Internal {
      ...
      template<class Func>
	void TF1CustomBuilder<Func>::Build(TF1Custom *f, Func func)
	{
	  using Fnc_t = typename ROOT::Internal::GetFunctorTypeCustom<decltype(ROOT::Internal::GetTheRightOpCustom(&Func::operator()))>::type;
	  f->fType = std::is_same<Fnc_t, double>::value? TF1Custom::EFType::kTemplScalar : TF1Custom::EFType::kTemplVec;
	  f->fFunctor.reset(new TF1Custom::TF1CustomFunctorPointerImpl<Fnc_t>(ROOT::Math::ParamFunctorTempl<Fnc_t>(func)));
	  f->fParams.reset(new TF1CustomParameters(f->fNpar));
	}
	...
	void TF1CustomBuilder<Func *>::Build(TF1Custom *f, Func *func)
	{
      ...
      /// TF1Custom building from a string
      /// used to build a TFormula based on a lambda function
      template<>
	struct TF1CustomBuilder<const char *> {
	static void Build(TF1Custom *f, const char *formula)
	{
	  f->fType = TF1Custom::EFType::kFormula;
...
inline Double_t TF1Custom::operator()(Double_t x, Double_t y, Double_t z, Double_t t) const
{
  return Eval(x, y, z, t);
}
 
template <class T>
inline T TF1Custom::operator()(const T *x, const Double_t *params)
{
...
template <class T>
T TF1Custom::EvalPar(const T *x, const Double_t *params)
{
  if (fType == EFType::kTemplVec || fType == EFType::kTemplScalar) {
    ...
    return TF1Custom::EvalPar((double *)x, params);
}
 
////////////////////////////////////////////////////////////////////////////////
///   Eval for vectorized functions
// template <class T>
// T TF1Custom::Eval(T x, T y, T z, T t) const
// {
...
// Internal to TF1Custom. Evaluates Templated interfaces
template <class T>
inline T TF1Custom::EvalParTempl(const T *data, const Double_t *params)
{
  assert(fType == EFType::kTemplScalar || fType == EFType::kTemplVec);
  if (!params) params = (Double_t *)fParams->GetParameters();
  if (fFunctor)
    return ((TF1CustomFunctorPointerImpl<T> *)fFunctor.get())->fImpl(data, params);
...
#ifdef R__HAS_VECCORE
// Internal to TF1Custom. Evaluates Vectorized TF1Custom on data of type Double_v
inline double TF1Custom::EvalParVec(const Double_t *data, const Double_t *params)
{
  ...
  if (fFunctor) {
    res = ((TF1CustomFunctorPointerImpl<ROOT::Double_v> *) fFunctor.get())->fImpl(d.data(), params);
  } else {
    //    res = GetSave(x);
    return TMath::SignalingNaN();
  }
...
inline void TF1Custom::SetRange(Double_t xmin, Double_t,  Double_t xmax, Double_t)
{
  TF1Custom::SetRange(xmin, xmax);
}
inline void TF1Custom::SetRange(Double_t xmin, Double_t, Double_t,  Double_t xmax, Double_t, Double_t)
{
  TF1Custom::SetRange(xmin, xmax);
}
 
template <typename Func>
void TF1Custom::SetFunction(Func f)
{
  // set function from a generic C++ callable object
  fType = EFType::kPtrScalarFreeFcn;
  fFunctor.reset(new TF1Custom::TF1CustomFunctorPointerImpl<double>(ROOT::Math::ParamFunctor(f)));
}
template <class PtrObj, typename MemFn>
  void TF1Custom::SetFunction(PtrObj &p, MemFn memFn)
{
  // set from a pointer to a member function
  fType = EFType::kPtrScalarFreeFcn;
  fFunctor.reset(new TF1Custom::TF1CustomFunctorPointerImpl<double>(ROOT::Math::ParamFunctor(p, memFn)));
}
 
template <class T>
inline T TF1Custom::GradientPar(Int_t ipar, const T *x, Double_t eps)
{
  if (fType == EFType::kTemplVec || fType == EFType::kTemplScalar) {
...
template <class T>
inline T TF1Custom::GradientParTempl(Int_t ipar, const T *x, Double_t eps)
{
  if (GetNpar() == 0)
    return 0;
  ...
  ((TF1Custom *)this)->GetParLimits(ipar, al, bl);
  if (al * bl != 0 && al >= bl) {
...
template <class T>
inline void TF1Custom::GradientPar(const T *x, T *grad, Double_t eps)
{
  if (fType == EFType::kTemplVec || fType == EFType::kTemplScalar) {
...
template <class T>
inline void TF1Custom::GradientParTempl(const T *x, T *grad, Double_t eps)
{
  if (eps < 1e-10 || eps > 1) {
    Warning("Derivative", "parameter esp=%g out of allowed range[1e-10,1], reset to 0.01", eps);
    eps = 0.01;
