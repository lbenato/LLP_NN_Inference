...
print "What is known at this point?"
print "ctauPoints_XX: ", ctauPoints_XX
print "MX: ", MX
print "ctauPoints_split: ", ctauPoints_split
print "Mglu: ", Mglu

print "What is known at this point?"
print "ctauPoints: ", ctauPoints
print "M: ", M

#Mglu, MX
def fillValues(massPoints,filedir,filename):
    val = {}
    mass = []
    ctau = []

    if options.channel=="JetJet":
        mass_string = "XXTo4J_M"
        ctau_string = "CTau"
    elif options.channel=="splitSUSY":
        mass_string = "splitSUSY_M"
        ctau_string = "CTau-"
    elif options.channel=="gluinoGMSB":
        mass_string = "gluinoGMSB_M"
        ctau_string = "ctau"

    for i, s in enumerate(sorted(massPoints)):
        print "mass: ", i,s
        for j, r in enumerate(sorted(ctauPoints)):
            print "ctau: ", j,r
            try:
                filepath = filedir+"/"+filename+".txt"
                file = open( (filepath % (s , r)), 'r')
                card = filename % (s , r)
                if isMM:
                    card += "_MM"
                elif isEE:
...
'''
def limit_vs_mass(channel, H, ctaupoint, tagvar, save=False):
...
def limit_vs_ctau(channel, masspoint, tagvar, save=False):
    if "JetJet" in options.channel:
        #particle = "X"
        #decay_string = "Z^{*} #rightarrow XX #rightarrow q#bar{q} q#bar{q}"
        mass_string = "XXTo4J_M"
        ctau_string = "CTau"
        #M = MX
    elif "splitSUSY" in options.channel:
        #particle = "#tilde{g}"
        #decay_string = "#tilde{g} #tilde{g} #rightarrow tbs tbs"
        mass_string = "splitSUSY_M-"
        ctau_string = "CTau-"
        #M = Mglu
    elif "gluinoGMSB" in options.channel:
        #particle = "#tilde{g}"
        #decay_string = "#tilde{g} #tilde{g} #rightarrow g #tilde{G} g #tilde{G}"
        mass_string = "gluinoGMSB_M"
        ctau_string = "ctau"
    ...
    #print RESULTS
    filename = mass_string+"%d_"+ctau_string+"%d"
    if isMM:
        filename += "_MM"
    elif isEE:
        filename += "_EE"
        ...
        filename += "_comb"
    if "splitSUSY" in options.channel:
        filename+="mm"
    if "JetJet" in options.channel:
        filename+="mm"

    print RESULTS, filename
    mass, ctau, val = fillValues(M,RESULTS,filename)
    print "mass, ctau, val: ", mass, ctau, val
    ...
    for j, c in enumerate(sorted(ctau)):
        #print "ctau: ", c
        if options.channel=="JetJet" or options.channel=="splitSUSY":
            name = mass_string+str(masspoint)+"_"+ctau_string+str(c)+"mm"
        else:
            name = mass_string+str(masspoint)+"_"+ctau_string+str(c)
        if isMM:
            name += "_MM"
        elif isEE:
            name += "_EE"
    ...
    #Exp2s.GetYaxis().SetTitle("#sigma(H) B("+particle+particle+" #rightarrow b #bar{b} b #bar{b})/#sigma_{SM}")
    Exp2s.GetYaxis().SetTitle("#sigma B("+decay_string+") ("+unit+")")
    ...
    newFile = TFile(RESULTS+"/Exclusion_m" + str(masspoint) + ".root", "RECREATE")
    newFile.cd()
    Exp0s.Write("m"+str(masspoint)+"_exp")
    Exp1s.Write("m"+str(masspoint)+"_1sigma")
    c1.Write()
    newFile.Close()
    print "Written file: ", RESULTS+"/Exclusion_m" + str(masspoint) + ".root"
...
'''
def significance_vs_mass(channel, H, ctaupoint, tagvar, save=False):
    particle = "#chi" if "SUSY" in options.channel else "#pi"
    if "HeavyHiggs" in options.channel: particle = "S"
    drawTheory = False
    mass_string = "MH"+str(H)+"_MS"
...
def plot_all_limits_vs_ctau(vector_expected, vector_1sigma, channel, tagvar):
    particle = "#chi" if "SUSY" in options.channel else "#pi"
    if "HeavyHiggs" in options.channel: particle = "S"
    if "JetJet" in options.channel: particle = "X"
    if "splitSUSY" in options.channel: particle = "#chi"
    if "gluinoGMSB" in options.channel: particle = "#chi"
    colors = [881,801,856,920,825,2,602,880,798,5]#920 is grey
    colors = [881,801,856,825,2,602,880,798,5]#920 is grey
    c2 = TCanvas("c2", "AllLimits", 800, 600)
    c2.cd()
    c2.SetGrid()
    c2.GetPad(0).SetTopMargin(0.06)
    c2.GetPad(0).SetRightMargin(0.05)
    ...
    #for b in (vector_1sigma.keys()):
        print "here order messed up? ", i, b
        ...
        if options.channel=="JetJet":
            vector_1sigma[b].GetYaxis().SetTitle("#sigma B("+particle+particle+" #rightarrow q #bar{q} q #bar{q}) ("+unit+")")
        if options.channel=="splitSUSY":
            vector_1sigma[b].GetYaxis().SetTitle("#sigma B("+particle+particle+" #rightarrow tbs tbs) ("+unit+")")
        if options.channel=="gluinoGMSB":
            vector_1sigma[b].GetYaxis().SetTitle("#sigma B("+particle+particle+" #rightarrow qg qg) ("+unit+")")
        ...
        if i == 0:
            ...
            axis.SetLimits(ctauPoints[0]-ctauPoints[0]*0.1,50500.); 
            vector_1sigma[b].SetMinimum(0.01)
            vector_1sigma[b].SetMaximum(1000.)            
    ...
    for i, b in enumerate(sorted(vector_expected)):
        ...
        if options.channel=="JetJet":
            vector_expected[b].GetYaxis().SetTitle("#sigma B("+particle+particle+" #rightarrow q #bar{q} q #bar{q}) ("+unit+")")
        if options.channel=="splitSUSY":
            vector_expected[b].GetYaxis().SetTitle("#sigma B("+particle+particle+" #rightarrow tbs tbs) ("+unit+")")
        if options.channel=="gluinoGMSB":
            vector_expected[b].GetYaxis().SetTitle("#sigma B("+particle+particle+" #rightarrow qg qg) ("+unit+")")
    ...
    for i, b in enumerate(sorted(vector_expected)):
        print "expected : ", b
        vector_expected[b].Write(tagvar+"_m"+str(b)+"_exp")
    for i, b in enumerate(sorted(vector_1sigma)):
        vector_1sigma[b].Write(tagvar+"_m"+str(b)+"_1sigma")
    c2.Write()
    newFile.Close()
...
'''
def plot_all_limits_vs_mass(H, vector_expected, vector_1sigma, channel, tagvar):
    particle = "#chi" if "SUSY" in options.channel else "#pi"
    if "HeavyHiggs" in options.channel: particle = "S"
    colors = [881,801,856,920,825,2,602,880,798,5]#920 is grey
    colors = [881,801,856,825,2,602,880,798,5]#920 is grey
...
if options.fom=="Limits":
    ...
    for a in sorted(np.array(massPoints)):
        vect_exp[a], vect_1sigma[a] = limit_vs_ctau(options.channel, a, options.key)
    #plot_all_limits_vs_ctau(vect_exp,vect_1sigma,options.channel,options.key)
    plot_all_limits_vs_ctau(vect_exp,vect_exp,options.channel,options.key)
