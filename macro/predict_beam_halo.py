...
ERA                = "2016"
REGION             = "BHpos"#"HBHE"#"ZtoEEBoost"#"WtoMN"#"WtoEN"
CUT                = "isSRNoVeto"#"isSRCosmicVeto"#"isSRCosmic"#"isSRHBHE"#"isZtoEE"#"isWtoMN"#"isWtoEN"#"isJetMET_low_dPhi_MET_200_Lep"#"isJetHT_unprescaled"
KILL_QCD           = True#False
DO_ETA             = True
DO_PHI             = False#False#
if DO_PHI:
...
PRE_PLOTDIR        = "plots/Efficiency_AN_fix_ARC/v5_calo_AOD_"+ERA+"_"
PLOTDIR            = "plots/Efficiency_AN_fix_ARC/v5_calo_AOD_"+ERA+"_"+REGION+"/"#"_2017_signal/"#

YIELDDIR_BASE      = "plots/Yields_AN_fix_ARC/v6_calo_AOD_"+ERA+"_"
YIELDDIR           = "plots/Yields_AN_fix_ARC/v6_calo_AOD_"+ERA+"_"+REGION+"/"#"_2017_signal/"#
...
def draw_data_combination(era,regions,regions_labels=[],datasets=[],add_label="",lab_2="",check_closure=False,eta=False,phi=False,eta_cut=False,phi_cut=False,do_ratio=False,isMC=False):
    ...
    BASEDIR = "plots/Efficiency_AN_fix_ARC/v5_calo_AOD_"+era+"_"
    OUTDIR  = "plots/Efficiency_AN_fix_ARC/v5_calo_AOD_"+era+"_combination/"
    infiles = {}
    graph =  {}
    eff   =  {}
    hist_num = {}
    ...
    for i, r in enumerate(regions):
        ...
        graph[r+reg_label].GetYaxis().SetRangeUser(-0.0001,0.1 if check_closure else maxeff)
...
def background_prediction(tree_weight_dict,sample_list,extr_regions=[],regions_labels=[],datasets=[],add_label="",label_2="",check_closure=False,plot_distr="",eta=False,phi=False,eta_cut=False,phi_cut=False):
    ...
    for i,r in enumerate(extr_regions):
        ...
        EFFDIR[r+reg_label+dset] = "plots/Efficiency_AN_fix_ARC/v5_calo_AOD_"+ERA+"_"+ r + "/"
    ...
    for i, s in enumerate(sample_list):
        ...
        if CUT=="isSRHBHE" or CUT=="isSRCosmic" or CUT=="isSRCosmicVeto" or CUT=="isSRNoVeto":
            list_of_variables += ["isSR"]
            list_of_variables += ["nCosmicMuons*","isDT_fit","dt_ecal_*","min_*"]
        else:
            list_of_variables += [CUT]
        ...
        for l, ss in enumerate(samples[s]['files']):
            ...
            for arrays in gen:
                ...
                array_size_tot+=len(arrays[ key_list[0] ])
                tree_w_array = tree_weight_dict[s][ss]*np.ones( len(arrays[ key_list[0] ])  )
                if CUT == "isSR" or CUT=="isSR" or CUT=="isSRpos" or CUT=="isSRNoVeto":
                    cut_mask = arrays["isSR"]>0
                    if "v6_calo_AOD" in NTUPLEDIR and "v5_ntuples" not in NTUPLEDIR:
                        #cosmic
                        print "Without cosmic veto! not designed for 0.7"
                ...
                #HEM
                if (CUT=="isSR" or CUT=="isSRCosmic" or CUT=="isSRCosmicVeto" or CUT=="isSRHBHE" or CUT=="isSRBH" or CUT=="isSRNoVeto") and str(ERA)=="2018":
                    cut_mask = np.logical_and( cut_mask, np.logical_or( arrays["RunNumber"]<319077, np.logical_and( arrays["RunNumber"]>=319077, arrays["nCHSJets_in_HEM_pt_30_all_eta"]==0)))
...
def predict_BH():
    #prediction performed after running: macro/print_tables.py
    #CSC inefficiency estimated after running: macro/beam_halo_cracks.py
    csc_eff = 0.652582159624
    csc_eff_unc = 0.0325042710593
    ...
    y_2["2016_B-F"] = 1
    pred_2["2016_B-F"] = 0.46
    unc_pred_2["2016_B-F"] = math.sqrt(0.10**2 + 0.09**2)
    y_2_no_veto["2016_B-F"] = 24
    rejected["2016_B-F"] = y_2_no_veto["2016_B-F"]-y_2["2016_B-F"]
    left["2016_B-F"] = y_2["2016_B-F"] - pred_2["2016_B-F"]

    y_2["2016_G-H"] = 0
    pred_2["2016_G-H"] = 0.37
    unc_pred_2["2016_G-H"] = math.sqrt(0.09**2 + 0.08**2)
    y_2_no_veto["2016_G-H"] = 15
    rejected["2016_G-H"] = y_2_no_veto["2016_G-H"]-y_2["2016_G-H"]
    left["2016_G-H"] = y_2["2016_G-H"] - pred_2["2016_G-H"]

    y_2["2017"] = 5
    pred_2["2017"] = 0.51
    unc_pred_2["2017"] = math.sqrt(0.08**2 + 0.19**2)
    y_2_no_veto["2017"] = 45
    rejected["2017"] = y_2_no_veto["2017"]-y_2["2017"]
    left["2017"] = y_2["2017"] - pred_2["2017"]

    y_2["2018"] = 0
    pred_2["2018"] = 0.56
    unc_pred_2["2018"] = math.sqrt(0.08**2 + 0.35**2)
    y_2_no_veto["2018"] = 54
    rejected["2018"] = y_2_no_veto["2018"]-y_2["2018"]
    left["2018"] = y_2["2018"] - pred_2["2018"]
    
    y_2["tot"] = 6
    pred_2["tot"] = 1.89
    unc_pred_2["tot"] = math.sqrt(0.12**2 + 0.70**2)
    y_2_no_veto["tot"] = y_2_no_veto["2016_B-F"] + y_2_no_veto["2016_G-H"] + y_2_no_veto["2017"] + y_2_no_veto["2018"]
    rejected["tot"] = y_2_no_veto["tot"] - y_2["tot"]
    left["tot"] = y_2["tot"] - pred_2["tot"]

    ineff = left["tot"]/y_2_no_veto["tot"]
    #can do: propagate left and y_2_no_veto
    #relative uncertainties go in quadrature!
    #unc in quadrature: numerator propagation, poisson on y_2, poisson on y_2_no_veto    
    unc_den = math.sqrt(y_2_no_veto["tot"])/y_2_no_veto["tot"]
    print "den: ", y_2_no_veto["tot"]
    print "unc_den ", math.sqrt(y_2_no_veto["tot"])
    print "rel unc_den ", unc_den*100
    unc_num = math.sqrt(  1./y_2["tot"] + (unc_pred_2["tot"]/pred_2["tot"])**2  )
    print "num ", left["tot"]
    print "unc_num ", left["tot"]*unc_num
    print "rel unc_num ", unc_num*100
    print "old method: ", math.sqrt(  ( math.sqrt(y_2["tot"]) )**2 + (unc_pred_2["tot"])**2  )
    print "rel old method: ", math.sqrt(  ( math.sqrt(y_2["tot"]) )**2 + (unc_pred_2["tot"])**2  )/left["tot"]
    unc_ineff = math.sqrt( unc_den**2  + unc_num**2 )
    print "rel unc ineff ",unc_ineff*100
    pred_bh = ineff*1./csc_eff
    unc_bh = math.sqrt( 1. + unc_ineff**2 + csc_eff_unc**2 )
    print unc_bh, " poisson wins"

    for k in ["2016_B-F","2016_G-H","2017","2018","tot"]:
        print "\n"
        print "\n"
        print k
        print "\n"
        print "y_2",y_2[k]
        print "pred_2" ,pred_2[k]
        print "unc_pred_2" ,unc_pred_2[k]
        print "rejected" ,rejected[k]
        print "y_2_no_veto" ,y_2_no_veto[k]
        print "left" ,left[k]
        print "\n"

    print rejected["tot"], y_2_no_veto["tot"], left["tot"]
    print "Beam halo inefficiency: %.3f $\pm$ %.3f"%(ineff,unc_ineff*ineff)
    print "Beam halo prediction: %.3f $\pm$ %.3f"%(pred_bh,unc_bh*pred_bh)

    #prepare yaml
    BASEDIR = "plots/Yields_AN_fix_ARC/v6_calo_AOD_%s_SR/"
    filename = "BkgPredResults_%s_SR_beam_halo%s"
    #2016BF 
    results = {}
    OUTDIR = BASEDIR%("2016")
    name = filename%("2016","_B-F")
    results["bkg_bh"] = pred_bh
    results["unc_bh"] = unc_bh*100
    print results
