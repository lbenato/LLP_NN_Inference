...
if ERA=="2016":
    ...
    if SEL=="ZtoLLPho":
        LUMI = lumi["SingleMuon"+ERA_TAG]["tot"]#+lumi["SingleElectron"]["tot"]
if ERA=="2017":
...
if SEL=="ZtoLLPho":
    if ERA=="2018":
        ...
        dataEE = ["SingleElectron"+ERA_TAG]
    dataMM = ["SingleMuon"+ERA_TAG]
    data = ["SingleLepton"+ERA_TAG]#dataMM+dataEE
    back = ["DYJetsToLL"]
if SEL=="TtoEM":
    data = ["MuonEG"]
    back = ["TTbarGenMET"]
...
sign = ['SUSY_mh127_ctau500','SUSY_mh200_ctau500','SUSY_mh300_ctau500','SUSY_mh600_ctau500','SUSY_mh800_ctau500','SUSY_mh1000_ctau500','SUSY_mh1800_ctau500']

sel_sign = ['SUSY_mh400_ctau500']

MAIN = "/nfs/dust/cms/group/cms-llp/v6_calo_AOD/v6_calo_AOD_"+ERA+"_"+SEL+"_v5_ntuples/"
MAIN_MM = "/nfs/dust/cms/group/cms-llp/v6_calo_AOD/v6_calo_AOD_"+ERA+"_ZtoMMPho_v5_ntuples_updated/"
MAIN_EE = "/nfs/dust/cms/group/cms-llp/v6_calo_AOD/v6_calo_AOD_"+ERA+"_ZtoEEPho_v5_ntuples_updated/"

MAIN_MET = "/nfs/dust/cms/group/cms-llp/v6_calo_AOD/v6_calo_AOD_"+ERA+"_SR_no_photonEFrac_eleEFrac_cut/"
#"/nfs/dust/cms/group/cms-llp/v6_calo_AOD/v6_calo_AOD_"+ERA+"_SR_no_photonEFrac_cut/"

OUT = "/afs/desy.de/user/l/lbenato/LLP_inference/CMSSW_11_1_3/src/NNInferenceCMSSW/LLP_NN_Inference/plots/v6_calo_AOD_"+ERA+"_"+SEL+"_v5_ntuples_updated/smearing/"
...
def calc_sf(var,cut,label="",scale=True, do_smear=False):
    ...
    for s in back:
        ...
        hist[s] = TH1F(s,";"+variable[var]['title'],25,-2,2)
        hist[s].Sumw2()
        chain[s] = TChain("tree")
        for j, ss in enumerate(samples[s]['files']):
        ...
        #first gaussian fit
        fit_back_g = TF1("fit_back_g","gaus",5,5)
        fit_back_g.SetParameter(0,0.01)
        fit_back_g.SetParameter(1,hist[s].GetMean())
        fit_back_g.SetParameter(2,hist[s].GetRMS())
        hist[s].Fit(fit_back_g,"E")
        back_time_fit = hist[s].GetFunction("fit_back_g")
        hist[s].GetListOfFunctions().Remove(hist[s].GetFunction("fit_back_g"))
        #then CB
        fit_back = TF1("fit_back","crystalball",5,5)
        fit_back.SetParameter(0,fit_back_g.GetParameter(0))
        fit_back.SetParameter(1,fit_back_g.GetParameter(1))#hist[s].GetMean())
        fit_back.SetParameter(2,fit_back_g.GetParameter(2))#hist[s].GetRMS())
        fit_back.SetParameter(3,1)
        fit_back.SetParameter(4,1)
        hist[s].Fit(fit_back,"E")
        if ERA=="2018":
            back_time_fit = hist[s].GetFunction("fit_back")
    ...
    for s in data:
        ...
        if ERA=="2016":
            ...
            fit_time_d.SetParameter(0,my_data.GetParameter(0))#0.01)
            fit_time_d.SetParameter(1,my_data.GetParameter(1))#0)
            fit_time_d.SetParameter(2,my_data.GetParameter(2))#0.5)
            fit_time_d.FixParameter(3,my_data.GetParameter(3))#0)
            fit_time_d.FixParameter(4,my_data.GetParameter(4))#0.5)
            ##data_time_fit = my_data.Clone("fit_time_d")
        else:
            fit_time_d = TF1("fit_time_d","crystalball",-5,5)
            fit_time_d.SetParameter(0,my_data.GetParameter(0))#0.01)
            fit_time_d.SetParameter(1,my_data.GetParameter(1))#0)
            fit_time_d.SetParameter(2,my_data.GetParameter(2))#0.5)
            fit_time_d.SetParameter(3,my_data.GetParameter(3))#1)
            fit_time_d.SetParameter(4,my_data.GetParameter(4))#1)
        hist[s].Fit(fit_time_d,"E")
        data_time_fit = hist[s].GetFunction("fit_time_d")
        hist[s].GetListOfFunctions().Remove(hist[s].GetFunction("fit_time_d"))
    ...
    for v in var_to_model:
        ...
        fit_back.SetParameter(1,fit_data.GetParameter(1))#hist_var_b[v].GetMean())
        fit_back.SetParameter(2,fit_data.GetParameter(2))#hist_var_b[v].GetRMS())
    ...
    b_h_smeared = TH1F("b_h_smeared",";"+variable[var]['title'],25,-2,2)
    b_h_smeared.Sumw2()
    ...
    for s in sign:
        ...
        num_s_gm_h[s] = TH1F("num_s_gm_h_"+s,"",len(bins_pt)-1,bins_pt)
        den_s_gm_h[s] = TH1F("den_s_gm_h_"+s,"",len(bins_pt)-1,bins_pt)
        s_graph[s] = TGraphAsymmErrors()
        s_graph_gm[s] = TGraphAsymmErrors()
    ...
    #Drop this approach, as the resolution for photons is different wrt ttbar
    '''
    smear_cb = my_data.Clone("smear_cb")
    smear_cb.SetParameter(0,my_data.GetParameter(0))
    #Do modify the mean!
    smear_cb.SetParameter(1,my_data.GetParameter(1)-my_back.GetParameter(1))
    ...
    s_EventWeight   = {}
    s_probs         = {}
    s_isGenMatchedCaloCorrLLPAccept = {}
    s_eta           = {}
    s_pt            = {}
    ...
    for s in sign:
        ...
        s_probs[s] = np.array([])
        s_isGenMatchedCaloCorrLLPAccept[s] = np.array([])
        s_eta[s] = np.array([])
        s_pt[s] = np.array([])
    ...
    for b in back:
        for j, ss in enumerate(samples[b]['files']):
            for M in [MAIN_EE,MAIN_MM]:
                ...
                b_tmp_eta = []
                b_tmp_pt = []
                b_tmp_isGenMatchedCaloCorrLLPAccept = []
                ...
                for e in range(0,tree.GetEntries()):
                    ...
                    for n in range(min(1,neg_jets.size())):
                        ...
                        photonEFrac_rnd = min(neg_jets[n].photonEFrac + gRandom.Gaus(shift_b["photonEFrac"].GetParameter(1),shift_b["photonEFrac"].GetParameter(2)),1)
    ...
    for s in sign:
        for j, ss in enumerate(samples[s]['files']):
            ...
            s_tmp_eta = []
            s_tmp_pt = []
            s_tmp_isGenMatchedCaloCorrLLPAccept = []
            ...
            for e in range(0,tree.GetEntries()):
                ...
                for n in range(min(1,neg_jets.size())):
                    ...
                    s_tmp_isGenMatchedCaloCorrLLPAccept.append(neg_jets[n].isGenMatchedCaloCorrLLPAccept)
            ...
            s_isGenMatchedCaloCorrLLPAccept[s] = np.concatenate(( s_isGenMatchedCaloCorrLLPAccept[s],s_tmp_isGenMatchedCaloCorrLLPAccept ))
    ...
    for s in sign:
        ...
        m_s_gm_den[s] = np.logical_and(m_s_time[s], (s_isGenMatchedCaloCorrLLPAccept[s]>0)  ) 
        m_s_gm_num[s] = np.logical_and(m_s_probs[s], np.logical_and(m_s_time[s], (s_isGenMatchedCaloCorrLLPAccept[s]>0)  )  )
        ...
        root_numpy.fill_hist(den_s_gm_h[s],     s_pt[s][m_s_gm_den[s]],           s_EventWeight[s][m_s_gm_den[s]])
        root_numpy.fill_hist(num_s_gm_h[s],     s_pt[s][m_s_gm_num[s]],           s_EventWeight[s][m_s_gm_num[s]])
        ...
        s_graph_gm[s].BayesDivide(num_s_gm_h[s],den_s_gm_h[s])
        ...
        s_graph_gm[s].SetMarkerStyle(24)
        s_graph_gm[s].SetMarkerColor(samples[s]['linecolor'])
        s_graph_gm[s].SetLineColor(samples[s]['linecolor'])
        s_graph_gm[s].SetLineStyle(7)
    ...
    for v in var_to_model:
        ...
        leg = TLegend(0.75, 0.79, 1., 0.99)
        leg.SetFillStyle(0)
        hist_var_sb[v].SetLineColor(862)#(samples[back[0]]['linecolor']+2)
        hist_var_sb[v].SetFillColorAlpha(862,0.1)
        hist_var_sb[v].SetLineWidth(2)
        hist_var_sb[v].SetMarkerColor(862)#(samples[back[0]]['linecolor']+2)
    ...
    leg2.AddEntry(d_h_sigprob_shift1,"data smeared +1 ns shift","PL")
    ...
    b_h_sigprob_shift1.SetLineColor(862)
    b_h_sigprob_shift1.SetFillColorAlpha(862,0.1)
    ...
    #b_graph_pre_0p9.SetMarkerColor(862)
    #b_graph_pre_0p9.SetLineColor(862)
    ...
    for s in ['SUSY_mh127_ctau500','SUSY_mh200_ctau500','SUSY_mh300_ctau500','SUSY_mh1000_ctau500']:
    ...
    can5_pt_gm = TCanvas("can5_pt_gm","can5_pt_gm",900,800)
    can5_pt_gm.SetRightMargin(0.05)
    can5_pt_gm.cd()
    leg2 = TLegend(0.1+0.55, 0.7-0.02, 0.99, 0.95-0.02)
    leg2 = TLegend(0.1+0.45, 0.7-0.02-0.55, 0.99, 0.95-0.02-0.55)
    leg2.AddEntry(d_graph1_pt,"+1 ns shift, manipulated data","PL")
    ...
    d_graph2_pt.Draw("P,sames")
    #for s in ['SUSY_mh600_ctau500','SUSY_mh800_ctau500','SUSY_mh1800_ctau500']:
    for s in ['SUSY_mh300_ctau500','SUSY_mh1000_ctau500']:
        leg2.AddEntry(s_graph_gm[s],samples[s]['label'],"PL")
        s_graph_gm[s].Draw("P,sames")
        s_graph_gm[s].SetLineWidth(2)
    b_graph1_pt.SetMaximum(1.01)
    b_graph1_pt.SetMinimum(0.)
    b_graph1_pt.GetXaxis().SetRangeUser(bins_pt[0],bins_pt[-1])
    b_graph1_pt.GetXaxis().SetTitle("jet p_{T}")
    ...
    leg2.Draw()
    OUTSTRING = OUT
    OUTSTRING += "data_vs_back_vs_sign_gen_matched_tag_eff_shifted_jet_time_vs_pt"
    drawCMS_simple(LUMI, "Preliminary", ERA=ERA, onTop=True,left_marg_CMS=0.1,left_marg_LUMI=0.75)
    drawRegion(SEL)
    can5_pt_gm.SetLogx()
    can5_pt_gm.Update()
    can5_pt_gm.Print(OUTSTRING+label+'.png')
    can5_pt_gm.Print(OUTSTRING+label+'.pdf')
    can5_pt_gm.Close()
    ...
    ratio1_1bin.SetMarkerColor(1)
    ratio2_1bin.SetMarkerColor(2)
    ratio1_1bin.SetLineColor(1)
    ratio2_1bin.SetLineColor(2)
    ...
    ratio1_pt.Draw("PE,sames")
    ratio2_pt.Draw("PE,sames")
...
if SEL=="ZtoLLPho":
    if ERA=="2016":
        ...
        if ERA_TAG=="GH":
            lab = "_G-H"
    calc_sf(var="JetsNegative[0].timeRecHitsEB",cut = "fabs(Z_mass-91.2)<10. && Z_pho_tight && MinJetMetDPhi>0.5 && JetsNegative[0].eleEFrac<0.6",label=lab,scale=True,do_smear=False)
