...
MAIN = "/afs/desy.de/user/l/lbenato/LLP_inference/CMSSW_11_1_3/src/NNInferenceCMSSW/LLP_NN_Inference/"
MAIN = "/nfs/dust/cms/group/cms-llp/v6_calo_AOD/v6_calo_cosmics_2018/"
OUT = "/afs/desy.de/user/l/lbenato/LLP_inference/CMSSW_11_1_3/src/NNInferenceCMSSW/LLP_NN_Inference/plots/cosmics_fit_on_cosmics_dataset/"

for d in data:

    list_of_variables = ["nDTSegments","nCosmicMuons","nCosmicMuonsOneLeg","DTSegments.x","DTSegments.y","DTSegments.z","CSCSegments.x","CSCSegments.y","CSCSegments.z","DTSegments.time","EcalRecHitsAK4.x","EcalRecHitsAK4.y","EcalRecHitsAK4.z","CaloJets.pt","CaloJets.eta","CaloJets.timeRecHitsEB","CaloJets.nRecHitsEB","pt","DTSegments.eta","DTSegments.phi","CSCSegments.eta","CSCSegments.phi","EcalRecHitsAK4.eta","EcalRecHitsAK4.phi","RunNumber","LumiNumber","EventNumber","CosmicMuonsOneLeg.*","n_clusters_3_stations","Points.*","nCaloJets","isCosmic"]
    print "opening: ", MAIN + d + ".root"
    gen = uproot.iterate([MAIN + d + ".root"],"tree",list_of_variables)
    for arrays in gen:
        print "SELECT affected event"
        cut_mask = np.logical_and(arrays["nCosmicMuonsOneLeg"]==1,arrays["nCosmicMuons"]>1)
        #cut_mask = np.logical_and(cut_mask,arrays["pt"]>20)
        #cut_mask = np.logical_and(cut_mask,arrays["EventNumber"]==113145169)
        cut_mask = np.logical_and(cut_mask,arrays["isCosmic"]<1)
        ...
        nCaloJets=arrays["nCaloJets"][cut_mask]
        n_clusters_3_stations=arrays["n_clusters_3_stations"][cut_mask]
        ...
        DT_x = arrays["Points.x"][cut_mask]
        DT_y = arrays["Points.y"][cut_mask]
        DT_z = arrays["Points.z"][cut_mask]
        ...
        ECAL_x = np.divide(arrays["EcalRecHitsAK4.x"][cut_mask],100.)
        ECAL_y = np.divide(arrays["EcalRecHitsAK4.y"][cut_mask],100.)
        ECAL_z = np.divide(arrays["EcalRecHitsAK4.z"][cut_mask],100.)
        ECAL_eta = arrays["EcalRecHitsAK4.eta"][cut_mask]
        ECAL_phi = arrays["EcalRecHitsAK4.phi"][cut_mask]
        jet_pt   = arrays["CaloJets.pt"][cut_mask]
        jet_time = arrays["CaloJets.timeRecHitsEB"][cut_mask]
        jet_n = arrays["CaloJets.nRecHitsEB"][cut_mask]
        CosmicsOneLegdxy = np.divide(arrays["CosmicMuonsOneLeg.dxy"][cut_mask],100.)
        CosmicsOneLegpt = arrays["CosmicMuonsOneLeg.pt"][cut_mask]

    for ev in range(nDTSegments.shape[0]):
        #if DT_x[ev].size == 0 or DT_x[ev].size>100:
        #    continue
        if ECAL_x[ev].size < 1:
            continue
        if n_clusters_3_stations[ev]<2:
            continue
        if CosmicsOneLegpt[ev][0]<=50:
            continue
        #if EventNumber[ev]!=19923702:
        #    continue
        ...
        #try again
        #dbscan = DBSCAN(eps=2,min_samples=2).fit(DT)
        ...
        if n_clusters_>=2:
            ...
            mean_ECAL_z = np.sum(ECAL_z[ev].flatten())/np.size(ECAL_z[ev])
            print "mean_ECAL_z: ", mean_ECAL_z
            ...
            for k1 in range(len(m.keys())):
                ...
                for k2 in range(len(m.keys())):
                    ...
                    if k2!=k1 and ( (mean_x[k1]*mean_x[k2])<0 or (mean_y[k1]*mean_y[k2])<0 or (mean_z[k1]*mean_z[k2])<0 ):
                        ##opposite emispheres
                        print "pair: ", k1, k2
                        ...
                        tmp_ECAL = abs((mean_z[k1]+mean_z[k2])/2. - mean_ECAL_z)
        ...
        root_numpy.fill_hist(h_xy_ecal_tag,ECAL[:,[0,1]])
        root_numpy.fill_hist(h_xz_ecal_tag,ECAL[:,[2,0]])
        root_numpy.fill_hist(h_yz_ecal_tag,ECAL[:,[2,1]])
        root_numpy.fill_hist(h_xyz_ecal_tag,ECAL[:,[2,0,1]])

        #root_numpy.fill_hist(h_etaphi_ecal_tag,ECAL_cyl[:,[0,1]])
        #root_numpy.fill_hist(h_etaz_ecal_tag,ECAL_cyl[:,[0,2]])
        #root_numpy.fill_hist(h_phiz_ecal_tag,ECAL_cyl[:,[1,2]])
        #root_numpy.fill_hist(h_etaphiz_ecal_tag,ECAL_cyl[:,[0,1,2]])
        ...
        leg.SetHeader("nj: "+str(jet_pt[ev].shape[0])+", t "+str(jet_time[ev][0])+", n "+str(jet_n[ev][0]))
        leg.AddEntry(h_xy_dt,"DT segments","P")
        leg.AddEntry(h_xy_dt,"dxy "+str(CosmicsOneLegdxy[ev][0]),"P")
        if CosmicsOneLegdxy[ev].shape[0]>1:
            leg.AddEntry(h_xy_dt,"dxy "+str(CosmicsOneLegdxy[ev][1]),"P")
        ...
        OUTSTRING = OUT
        OUTSTRING += "data_run"+str(RunNumber[ev])+"_lumi"+str(LumiNumber[ev])+"_ev"+str(EventNumber[ev])
...
def altro():
    ...
    if n_clusters_>2:
        ...
        mean_ECAL_z = np.sum(ECAL_z.flatten())/np.size(ECAL_z)
        print "mean_ECAL_z: ", mean_ECAL_z
