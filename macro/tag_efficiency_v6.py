...
NTUPLEDIR_v5 = "/nfs/dust/cms/group/cms-llp/v6_calo_AOD/v6_calo_AOD_"+ERA+"_"+REGION+"_v5_ntuples/"#_bin_1_2/"#"_2017_signal/"#
...
PRE_PLOTDIR        = "plots/Efficiency/v6_calo_AOD_"+ERA+"_"
PLOTDIR            = "plots/Efficiency/v6_calo_AOD_"+ERA+"_"+REGION+"/"#"_2017_signal/"#
...
sign = [
    ...
    #"SUSY_mh400_pl1000",
    #"SUSY_mh300_pl1000","SUSY_mh250_pl1000","SUSY_mh200_pl1000","SUSY_mh175_pl1000","SUSY_mh150_pl1000","SUSY_mh127_pl1000",
    'SUSY_mh127_ctau500',#'SUSY_mh300_ctau500','SUSY_mh600_ctau500','SUSY_mh1000_ctau500','SUSY_mh1800_ctau500',
    #'SUSY_mh127_ctau3000','SUSY_mh300_ctau3000','SUSY_mh600_ctau3000','SUSY_mh1000_ctau3000','SUSY_mh1800_ctau3000',
...
def plot_roc_curve(tree_weight_dict_s,tree_weight_dict_b,sign,NTUPLE_SIGN,back,NTUPLE_BACK,add_label="",check_closure=False,eta=False,j_idx=-1,eta_cut=0.,eta_invert=False):
    ...
    for i, s in enumerate(back):
        ...
        for l, ss in enumerate(samples[s]["files"]):
            ...
            chain[s].Add(NTUPLE_BACK + ss + ".root")
            ...
            print "n_iter: ", n_iter
            gen = uproot.iterate([NTUPLE_BACK + ss + ".root"],"tree",list_of_variables,entrysteps=chunk_size)
    ...
    for i, s in enumerate(sign):
        ...
        for l, ss in enumerate(samples[s]["files"]):
            ...
            chain[s].Add(NTUPLE_SIGN + ss + ".root")
            chain_entries_cumulative[l] = chain[s].GetEntries()
            if l==0:
                chain_entries[l]=chain[s].GetEntries()
                print "Entries per sample ", ss, " : ", chain_entries[l]
            ...
            print "chunk_size: ", chunk_size
            print "n_iter: ", n_iter
            gen = uproot.iterate([NTUPLE_SIGN + ss + ".root"],"tree",list_of_variables,entrysteps=chunk_size)
        ...
        plt.grid(True)
        plt.savefig(PLOTDIR+"ROC"+add_label+"_"+s+".pdf")
        plt.savefig(PLOTDIR+"ROC"+add_label+"_"+s+".png")
        print "Info: ROC curve file "+PLOTDIR+"ROC"+add_label+"_"+s+".pdf has been created"

        with open(PLOTDIR+"fpr"+add_label+"_"+s+".npy", "wb") as f:
            np.save(f, fpr)
        with open(PLOTDIR+"tpr"+add_label+"_"+s+".npy", "wb") as f:
            np.save(f, tpr)
        with open(PLOTDIR+"thresholds"+add_label+"_"+s+".npy", "wb") as f:
            np.save(f, thresholds)
        with open(PLOTDIR+"idx"+add_label+"_"+s+".npy", "wb") as f:
            np.save(f, idx)
...
def compare_roc_curves(list_comparison,add_label=""):    
    ...
    colors = ['crimson','green','skyblue','orange','gray','crimson','green','skyblue','orange','gray',]
    linestyles = ['-','-','-','-','-', '--','--','--','--','--',]
    ...
    for i,l in enumerate(list_comparison):
        tpr[l] = np.load(PLOTDIR+"tpr_"+l+".npy")
        fpr[l] = np.load(PLOTDIR+"fpr_"+l+".npy")
        idx[l] = np.load(PLOTDIR+"idx_"+l+".npy")
        thresholds[l] = np.load(PLOTDIR+"thresholds_"+l+".npy")
        plt.plot(fpr[l], tpr[l], color=colors[i], lw=2, linestyle=linestyles[i], label=l)#"AUC = {0:.4f}".format(AUC))
    ...
    #plt.plot([0, 1], [0, 1], color="navy", lw=2, linestyle="--")
    #plt.plot(fpr_L,tpr_L,"ro",color="blue",label="cut based m=200 GeV; ct=1 m")#, all eta(2018)")
...
def get_tree_weights(sample_list,LUMI,NTUPL=""):
    ...
    for i, s in enumerate(sample_list):
        for l, ss in enumerate(samples[s]['files']):
            ...
            if ('Run201') in ss:
                ...
                filename = TFile(DIR+ss+'.root', "READ")
    ...
    for i, s in enumerate(sample_list):
        ...
        for l, ss in enumerate(samples[s]['files']):
            ...
            for arrays in gen:
                ...
                runnumber = runnumber[bin1_m]
                mindphi = mindphi[bin1_m]
                luminumber = luminumber[bin1_m]
                eventnumber = eventnumber[bin1_m]
                pt_v = pt_v[bin1_m]
                eta_v = eta_v[bin1_m]
                phi_v = phi_v[bin1_m]
                score_v = score_v[bin1_m]
...
#ROC:
#plot_roc_curve(get_tree_weights(sign,LUMI,NTUPLEDIR),get_tree_weights(back,LUMI,NTUPLEDIR_v5),sign,NTUPLEDIR,back,NTUPLEDIR_v5,add_label="",check_closure=False,eta=False,j_idx=-1,eta_cut=0,eta_invert=False)
...
'''
s_list=[
    #'SUSY_mh127_ctau500','SUSY_mh300_ctau500','SUSY_mh600_ctau500','SUSY_mh1000_ctau500',
    'SUSY_mh1800_ctau500',
    #'SUSY_mh127_ctau3000','SUSY_mh300_ctau3000','SUSY_mh600_ctau3000','SUSY_mh1000_ctau3000','SUSY_mh1800_ctau3000',
]

for s in s_list:
    plot_roc_curve(get_tree_weights([s],LUMI,NTUPLEDIR),get_tree_weights(back,LUMI,NTUPLEDIR_v5),[s],NTUPLEDIR,back,NTUPLEDIR_v5,add_label="",check_closure=False,eta=False,j_idx=-1,eta_cut=0,eta_invert=False)

'''

compare_roc_curves(list_comparison=['SUSY_mh127_ctau500','SUSY_mh300_ctau500','SUSY_mh600_ctau500','SUSY_mh1000_ctau500', 'SUSY_mh1800_ctau500','SUSY_mh127_ctau3000','SUSY_mh300_ctau3000','SUSY_mh600_ctau3000','SUSY_mh1000_ctau3000', 'SUSY_mh1800_ctau3000',],add_label="SUSY")
exit()
