    ...
    for b in sign:
        ...
        for i, ss in enumerate(samples[b]['files']):
            ...
            for arrays in gen:
                ...
                #TODO perhaps flatten??
                sorted_genLLP0_pt_resolved_q = np.sort( np.concatenate((genb0_pt[acc_LLP0_resolved_per_part_or],genb1_pt[acc_LLP0_resolved_per_part_or])).flatten()   )
                #TODO perhaps flatten??
                sorted_genLLP1_pt_resolved_q = np.sort( np.concatenate((genb2_pt[acc_LLP1_resolved_per_part_or],genb3_pt[acc_LLP1_resolved_per_part_or])).flatten()   )
                ...
                #TODO perhaps flatten??
                sorted_genLLP0_eta_resolved_q = np.sort( np.concatenate((genb0_eta[acc_LLP0_resolved_per_part_or],genb1_eta[acc_LLP0_resolved_per_part_or])).flatten() )
                #TODO perhaps flatten??
                sorted_genLLP1_eta_resolved_q = np.sort( np.concatenate((genb2_eta[acc_LLP1_resolved_per_part_or],genb3_eta[acc_LLP1_resolved_per_part_or])).flatten() )
                ...
                #new: when q1q2, use higgs
                sorted_genLLP0_pt_resolved_h = np.sort(genh0_pt[acc_LLP0_resolved_per_part_or])
                sorted_genLLP1_pt_resolved_h = np.sort(genh1_pt[acc_LLP1_resolved_per_part_or])
                sorted_genLLP0_eta_resolved_h = np.sort(genh0_eta[acc_LLP0_resolved_per_part_or])
                sorted_genLLP1_eta_resolved_h = np.sort(genh1_eta[acc_LLP1_resolved_per_part_or])
                ...
                bins_pt_resolved_q = np.array(  [0., np.array_split(sorted_genLLP0_pt_resolved_q,n_splits)[1][0], np.array_split(sorted_genLLP0_pt_resolved_q,n_splits)[2][0], np.array_split(sorted_genLLP0_pt_resolved_q,n_splits)[3][0], 5000.])
                bins_eta_resolved_q = np.array([0., np.array_split(sorted_genLLP0_eta_resolved_q,n_splits)[1][0], np.array_split(sorted_genLLP0_eta_resolved_q,n_splits)[2][0], np.array_split(sorted_genLLP0_eta_resolved_q,n_splits)[3][0], 10.])


                #new: when q1q2, use higgs
                bins_pt_resolved_h = np.array(  [0., np.array_split(sorted_genLLP0_pt_resolved_h,n_splits)[1][0], np.array_split(sorted_genLLP0_pt_resolved_h,n_splits)[2][0], np.array_split(sorted_genLLP0_pt_resolved_h,n_splits)[3][0], 5000.])
                bins_eta_resolved_h = np.array([0., np.array_split(sorted_genLLP0_eta_resolved_h,n_splits)[1][0], np.array_split(sorted_genLLP0_eta_resolved_h,n_splits)[2][0], np.array_split(sorted_genLLP0_eta_resolved_h,n_splits)[3][0], 10.])

                #compare with 2 splits
                '''
                n_splits = 2
                bins_r_resolved = np.array(  [0., np.array_split(sorted_genLLP0_r_resolved,n_splits)[1][0], 500.])
                bins_z_resolved = np.array(  [0., np.array_split(sorted_genLLP0_z_resolved,n_splits)[1][0], 1000.])
                bins_pt_resolved_q = np.array(  [0., np.array_split(sorted_genLLP0_pt_resolved,n_splits)[1][0], 5000.])
                bins_eta_resolved_q = np.array([0., np.array_split(sorted_genLLP0_eta_resolved,n_splits)[1][0], 10.])
                #new: when q1q2, use higgs
                bins_pt_resolved_h = np.array(  [0., np.array_split(sorted_genLLP0_pt_resolved_h,n_splits)[1][0], 5000.])
                bins_eta_resolved_h = np.array([0., np.array_split(sorted_genLLP0_eta_resolved_h,n_splits)[1][0], 10.])
                '''


                #compare with 3 splits
                '''
                n_splits = 3
                bins_r_resolved = np.array(  [0., np.array_split(sorted_genLLP0_r_resolved,n_splits)[1][0], np.array_split(sorted_genLLP0_r_resolved,n_splits)[2][0], 500.])
                bins_z_resolved = np.array(  [0., np.array_split(sorted_genLLP0_z_resolved,n_splits)[1][0], np.array_split(sorted_genLLP0_z_resolved,n_splits)[2][0], 1000.])
                bins_pt_resolved_q = np.array(  [0., np.array_split(sorted_genLLP0_pt_resolved_q,n_splits)[1][0], np.array_split(sorted_genLLP0_pt_resolved_q,n_splits)[2][0], 5000.])
                bins_eta_resolved_q = np.array([0., np.array_split(sorted_genLLP0_eta_resolved_q,n_splits)[1][0], np.array_split(sorted_genLLP0_eta_resolved_q,n_splits)[2][0], 10.])
                #new: when q1q2, use higgs
                bins_pt_resolved_h = np.array(  [0., np.array_split(sorted_genLLP0_pt_resolved_h,n_splits)[1][0], np.array_split(sorted_genLLP0_pt_resolved_h,n_splits)[2][0], 5000.])
                bins_eta_resolved_h = np.array([0., np.array_split(sorted_genLLP0_eta_resolved_h,n_splits)[1][0], np.array_split(sorted_genLLP0_eta_resolved_h,n_splits)[2][0], 10.])
                '''

                #bins_eta_resolved_q = np.array([0., 0.4, 10.])
                #bins_eta_resolved_q = np.array([0., 0.4, 5.])
                #bins_pt_resolved_q = np.array( [0.,  300.,  500., 800.,  5000.])
                ...
                #Given low stat for merged case, 2 splits are better
                n_splits = 2
                bins_r_merged = np.array(  [0., np.array_split(sorted_genLLP0_r_merged,n_splits)[1][0], 500.])
                bins_z_merged = np.array(  [0., np.array_split(sorted_genLLP0_z_merged,n_splits)[1][0], 1000.])
                bins_pt_merged = np.array(  [0., np.array_split(sorted_genLLP0_pt_merged,n_splits)[1][0], 5000.])
                bins_eta_merged = np.array(  [0., np.array_split(sorted_genLLP0_eta_merged,n_splits)[1][0], 10.])
                ...
                m_den_eta = np.concatenate((m_denLLP0_eta,m_denLLP1_eta)).flatten()
                ...
                m_num_eta_T = np.concatenate((m_numLLP0_eta_T,m_numLLP1_eta_T)).flatten()
                ...
                m_num_eta_T_presel = np.concatenate((m_numLLP0_eta_T_presel,m_numLLP1_eta_T_presel)).flatten()
                ...
                m_eff_LLP0_T = np.divide(m_num_LLP0_T,m_den_LLP0,out=np.zeros_like(m_den_LLP0),where=(m_den_LLP0 != 0))
                m_eff_LLP0_T_presel = np.divide(m_num_LLP0_T_presel,m_den_LLP0,out=np.zeros_like(m_den_LLP0),where=(m_den_LLP0 != 0))
                ...
                m_eff_LLP1_T = np.divide(m_num_LLP1_T,m_den_LLP1,out=np.zeros_like(m_den_LLP1),where=(m_den_LLP1 != 0))
                m_eff_LLP1_T_presel = np.divide(m_num_LLP1_T_presel,m_den_LLP1,out=np.zeros_like(m_den_LLP1),where=(m_den_LLP1 != 0))
                ...
                #print "all merged num :"
                #print m_num_T
                #print "all merged den :"
                #print m_den
                ...
                m_eff_LLP0_1T = np.divide(m_num_LLP0_1T,m_den_LLP0,out=np.zeros_like(m_den_LLP0),where=(m_den_LLP0 != 0))
                m_eff_LLP0_1T_presel = np.divide(m_num_LLP0_1T_presel,m_den_LLP0,out=np.zeros_like(m_den_LLP0),where=(m_den_LLP0 != 0))
                ...
                m_eff_LLP0_2T = np.divide(m_num_LLP0_2T,m_den_LLP0,out=np.zeros_like(m_den_LLP0),where=(m_den_LLP0 != 0))
                m_eff_LLP0_2T_presel = np.divide(m_num_LLP0_2T_presel,m_den_LLP0,out=np.zeros_like(m_den_LLP0),where=(m_den_LLP0 != 0))
                ...
                #attempt: use higgs also for q1 and q2
                #it performs a bit worse
                #print "attempt!"
                #r_denLLP0_pt_q1  = genh0_pt[acc_LLP0_resolved_per_part_q1]
                #r_denLLP1_pt_q1  = genh1_pt[acc_LLP1_resolved_per_part_q1]
                #r_denLLP0_eta_q1 = genh0_eta[acc_LLP0_resolved_per_part_q1]
                #r_denLLP1_eta_q1 = genh1_eta[acc_LLP1_resolved_per_part_q1]
                ...
                r_den_eta_q = np.concatenate((r_denLLP0_eta_q1,r_denLLP0_eta_q2))
                r_den_eta_q = np.concatenate((r_den_eta_q,r_denLLP1_eta_q1))
                r_den_eta_q = np.concatenate((r_den_eta_q,r_denLLP1_eta_q2)).flatten()


                #attempt: use higgs also for q1 and q2
                #print "attempt!"
                #it performs a bit worse
                #r_denLLP0_pt_q2  = genh0_pt[acc_LLP0_resolved_per_part_q2]
                #r_denLLP1_pt_q2  = genh1_pt[acc_LLP1_resolved_per_part_q2]
                #r_denLLP0_eta_q2 = genh0_eta[acc_LLP0_resolved_per_part_q2]
                #r_denLLP1_eta_q2 = genh1_eta[acc_LLP1_resolved_per_part_q2]
                ...
                #new: when q1q2, use higgs
                r_denLLP0_pt_q1q2  = genh0_pt[acc_LLP0_resolved_per_part_q1q2]
                r_denLLP1_pt_q1q2  = genh1_pt[acc_LLP1_resolved_per_part_q1q2]
                r_denLLP0_eta_q1q2 = genh0_eta[acc_LLP0_resolved_per_part_q1q2]
                r_denLLP1_eta_q1q2 = genh1_eta[acc_LLP1_resolved_per_part_q1q2]


                #concatenate everything
                r_den_r_qq = np.concatenate((r_denLLP0_r_q1q2,r_denLLP1_r_q1q2)).flatten()
                r_den_z_qq = np.concatenate((r_denLLP0_z_q1q2,r_denLLP1_z_q1q2)).flatten()
                r_den_pt_qq = np.concatenate((r_denLLP0_pt_q1q2,r_denLLP1_pt_q1q2)).flatten()
                r_den_eta_qq = np.concatenate((r_denLLP0_eta_q1q2,r_denLLP1_eta_q1q2)).flatten()

                #merge LLP0 and LLP1
                #later if needed
                #r_den_r = np.concatenate((r_denLLP0_r,r_denLLP1_r)).flatten()
                #r_den_z = np.concatenate((r_denLLP0_z,r_denLLP1_z)).flatten()
                #r_den_pt1 = np.concatenate((r_denLLP0_pt1,r_denLLP1_pt1)).flatten()
                #r_den_pt2 = np.concatenate((r_denLLP0_pt2,r_denLLP1_pt2)).flatten()
                #r_den_eta1 = np.concatenate((r_denLLP0_eta1,r_denLLP1_eta1)).flatten()
                #r_den_eta2 = np.concatenate((r_denLLP0_eta2,r_denLLP1_eta2)).flatten()
                ...
                #merge LLP0 and LLP1
                #later if needed
                #r_num_r_T = np.concatenate((r_numLLP0_r_T,r_numLLP1_r_T)).flatten()
                #r_num_z_T = np.concatenate((r_numLLP0_z_T,r_numLLP1_z_T)).flatten()
                #r_num_pt1_T = np.concatenate((r_numLLP0_pt1_T,r_numLLP1_pt1_T)).flatten()
                #r_num_pt2_T = np.concatenate((r_numLLP0_pt2_T,r_numLLP1_pt2_T)).flatten()
                #r_num_eta1_T = np.concatenate((r_numLLP0_eta1_T,r_numLLP1_eta1_T)).flatten()
                #r_num_eta2_T = np.concatenate((r_numLLP0_eta2_T,r_numLLP1_eta2_T)).flatten()
                ...
                #merge LLP0 and LLP1
                #later if needed
                #r_num_r_T_presel = np.concatenate((r_numLLP0_r_T_presel,r_numLLP1_r_T_presel)).flatten()
                #r_num_z_T_presel = np.concatenate((r_numLLP0_z_T_presel,r_numLLP1_z_T_presel)).flatten()
                #r_num_pt1_T_presel = np.concatenate((r_numLLP0_pt1_T_presel,r_numLLP1_pt1_T_presel)).flatten()
                #r_num_pt2_T_presel = np.concatenate((r_numLLP0_pt2_T_presel,r_numLLP1_pt2_T_presel)).flatten()
                #r_num_eta1_T_presel = np.concatenate((r_numLLP0_eta1_T_presel,r_numLLP1_eta1_T_presel)).flatten()
                #r_num_eta2_T_presel = np.concatenate((r_numLLP0_eta2_T_presel,r_numLLP1_eta2_T_presel)).flatten()
                

                #now one map per b-quark
                r_den1_LLP0, r_den1_LLP0_edges = np.histogramdd(np.transpose([r_denLLP0_r,r_denLLP0_z,r_denLLP0_pt1,r_denLLP0_eta1]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_den1_LLP1, r_den1_LLP1_edges = np.histogramdd(np.transpose([r_denLLP1_r,r_denLLP1_z,r_denLLP1_pt1,r_denLLP1_eta1]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_den1, r_den1_edges = np.histogramdd(np.transpose([r_den_r,r_den_z,r_den_pt1,r_den_eta1]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))

                r_den2_LLP0, r_den2_LLP0_edges = np.histogramdd(np.transpose([r_denLLP0_r,r_denLLP0_z,r_denLLP0_pt2,r_denLLP0_eta2]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_den2_LLP1, r_den2_LLP1_edges = np.histogramdd(np.transpose([r_denLLP1_r,r_denLLP1_z,r_denLLP1_pt2,r_denLLP1_eta2]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_den2, r_den2_edges = np.histogramdd(np.transpose([r_den_r,r_den_z,r_den_pt2,r_den_eta2]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))

                #At least one tag
                #LLP0
                r_num1_LLP0_T, r_num1_LLP0_edges_T = np.histogramdd(np.transpose([r_numLLP0_r_T,r_numLLP0_z_T,r_numLLP0_pt1_T,r_numLLP0_eta1_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_num1_LLP0_T_presel, r_num1_LLP0_edges_T_presel = np.histogramdd(np.transpose([r_numLLP0_r_T_presel,r_numLLP0_z_T_presel,r_numLLP0_pt1_T_presel,r_numLLP0_eta1_T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_eff1_LLP0_T = np.divide(r_num1_LLP0_T,r_den1_LLP0,out=np.zeros_like(r_den1_LLP0),where=(r_den1_LLP0 != 0))
                r_eff1_LLP0_T_presel = np.divide(r_num1_LLP0_T_presel,r_den1_LLP0,out=np.zeros_like(r_den1_LLP0),where=(r_den1_LLP0 != 0))

                r_num2_LLP0_T, r_num2_LLP0_edges_T = np.histogramdd(np.transpose([r_numLLP0_r_T,r_numLLP0_z_T,r_numLLP0_pt2_T,r_numLLP0_eta2_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_num2_LLP0_T_presel, r_num2_LLP0_edges_T_presel = np.histogramdd(np.transpose([r_numLLP0_r_T_presel,r_numLLP0_z_T_presel,r_numLLP0_pt2_T_presel,r_numLLP0_eta2_T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_eff2_LLP0_T = np.divide(r_num2_LLP0_T,r_den2_LLP0,out=np.zeros_like(r_den2_LLP0),where=(r_den2_LLP0 != 0))
                r_eff2_LLP0_T_presel = np.divide(r_num2_LLP0_T_presel,r_den2_LLP0,out=np.zeros_like(r_den2_LLP0),where=(r_den2_LLP0 != 0))

                #LLP1
                r_num1_LLP1_T, r_num1_LLP1_edges_T = np.histogramdd(np.transpose([r_numLLP1_r_T,r_numLLP1_z_T,r_numLLP1_pt1_T,r_numLLP1_eta1_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_num1_LLP1_T_presel, r_num1_LLP1_edges_T_presel = np.histogramdd(np.transpose([r_numLLP1_r_T_presel,r_numLLP1_z_T_presel,r_numLLP1_pt1_T_presel,r_numLLP1_eta1_T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_eff1_LLP1_T = np.divide(r_num1_LLP1_T,r_den1_LLP1,out=np.zeros_like(r_den1_LLP1),where=(r_den1_LLP1 != 0))
                r_eff1_LLP1_T_presel = np.divide(r_num1_LLP1_T_presel,r_den1_LLP1,out=np.zeros_like(r_den1_LLP1),where=(r_den1_LLP1 != 0))

                r_num2_LLP1_T, r_num2_LLP1_edges_T = np.histogramdd(np.transpose([r_numLLP1_r_T,r_numLLP1_z_T,r_numLLP1_pt2_T,r_numLLP1_eta2_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_num2_LLP1_T_presel, r_num2_LLP1_edges_T_presel = np.histogramdd(np.transpose([r_numLLP1_r_T_presel,r_numLLP1_z_T_presel,r_numLLP1_pt2_T_presel,r_numLLP1_eta2_T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_eff2_LLP1_T = np.divide(r_num2_LLP1_T,r_den2_LLP1,out=np.zeros_like(r_den2_LLP1),where=(r_den2_LLP1 != 0))
                r_eff2_LLP1_T_presel = np.divide(r_num2_LLP1_T_presel,r_den2_LLP1,out=np.zeros_like(r_den2_LLP1),where=(r_den2_LLP1 != 0))

                #LLP0+LLP1
                #later if needed
                #r_num1_T, r_num1_edges_T = np.histogramdd(np.transpose([r_num_r_T,r_num_z_T,r_num_pt1_T,r_num_eta1_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                #r_num1_T_presel, r_num1_edges_T_presel = np.histogramdd(np.transpose([r_num_r_T_presel,r_num_z_T_presel,r_num_pt1_T_presel,r_num_eta1_T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                #r_eff1_T = np.divide(r_num1_T,r_den1,out=np.zeros_like(r_den1),where=(r_den1 != 0))
                #r_eff1_T_presel = np.divide(r_num1_T_presel,r_den1,out=np.zeros_like(r_den1),where=(r_den1 != 0))

                #r_num2_T, r_num2_edges_T = np.histogramdd(np.transpose([r_num_r_T,r_num_z_T,r_num_pt2_T,r_num_eta2_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                #r_num2_T_presel, r_num2_edges_T_presel = np.histogramdd(np.transpose([r_num_r_T_presel,r_num_z_T_presel,r_num_pt2_T_presel,r_num_eta2_T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                #r_eff2_T = np.divide(r_num2_T,r_den2,out=np.zeros_like(r_den2),where=(r_den2 != 0))
                #r_eff2_T_presel = np.divide(r_num2_T_presel,r_den2,out=np.zeros_like(r_den2),where=(r_den2 != 0))
                ...
                #it performs a bit worse
                #print "attempt!"
                #r_numLLP0_pt_q1_T  = genh0_pt[np.logical_and(nTagJets_to_LLP0>0,acc_LLP0_resolved_q1)]
                #r_numLLP1_pt_q1_T  = genh1_pt[np.logical_and(nTagJets_to_LLP1>0,acc_LLP1_resolved_q1)]
                #r_numLLP0_eta_q1_T = genh0_eta[np.logical_and(nTagJets_to_LLP0>0,acc_LLP0_resolved_q1)]
                #r_numLLP1_eta_q1_T = genh1_eta[np.logical_and(nTagJets_to_LLP1>0,acc_LLP1_resolved_q1)]
                ...
                #print "attempt!"
                #it performs a bit worse
                #r_numLLP0_pt_q2_T  = genh0_pt[np.logical_and(nTagJets_to_LLP0>0,acc_LLP0_resolved_q2)]
                #r_numLLP1_pt_q2_T  = genh1_pt[np.logical_and(nTagJets_to_LLP1>0,acc_LLP1_resolved_q2)]
                #r_numLLP0_eta_q2_T = genh0_eta[np.logical_and(nTagJets_to_LLP0>0,acc_LLP0_resolved_q2)]
                #r_numLLP1_eta_q2_T = genh1_eta[np.logical_and(nTagJets_to_LLP1>0,acc_LLP1_resolved_q2)]
                ...
                r_num_eta_q_T = np.concatenate((r_numLLP0_eta_q1_T,r_numLLP0_eta_q2_T))
                r_num_eta_q_T = np.concatenate((r_num_eta_q_T,r_numLLP1_eta_q1_T))
                r_num_eta_q_T = np.concatenate((r_num_eta_q_T,r_numLLP1_eta_q2_T)).flatten()
                ...
                #new: when q1q2, use higgs
                r_numLLP0_pt_q1q2_1T  = genh0_pt[np.logical_and(nTagJets_to_LLP0==1,acc_LLP0_resolved_q1q2)]
                r_numLLP1_pt_q1q2_1T  = genh1_pt[np.logical_and(nTagJets_to_LLP1==1,acc_LLP1_resolved_q1q2)]
                r_numLLP0_eta_q1q2_1T = genh0_eta[np.logical_and(nTagJets_to_LLP0==1,acc_LLP0_resolved_q1q2)]
                r_numLLP1_eta_q1q2_1T = genh1_eta[np.logical_and(nTagJets_to_LLP1==1,acc_LLP1_resolved_q1q2)]


                #concatenate everything
                r_num_r_qq_1T = np.concatenate((r_numLLP0_r_q1q2_1T,r_numLLP1_r_q1q2_1T)).flatten()
                r_num_z_qq_1T = np.concatenate((r_numLLP0_z_q1q2_1T,r_numLLP1_z_q1q2_1T)).flatten()
                r_num_pt_qq_1T = np.concatenate((r_numLLP0_pt_q1q2_1T,r_numLLP1_pt_q1q2_1T)).flatten()
                r_num_eta_qq_1T = np.concatenate((r_numLLP0_eta_q1q2_1T,r_numLLP1_eta_q1q2_1T)).flatten()
                ...
                #new: when q1q2, use higgs
                r_numLLP0_pt_q1q2_2T  = genh0_pt[np.logical_and(nTagJets_to_LLP0>1,acc_LLP0_resolved_q1q2)]
                r_numLLP1_pt_q1q2_2T  = genh1_pt[np.logical_and(nTagJets_to_LLP1>1,acc_LLP1_resolved_q1q2)]
                r_numLLP0_eta_q1q2_2T = genh0_eta[np.logical_and(nTagJets_to_LLP0>1,acc_LLP0_resolved_q1q2)]
                r_numLLP1_eta_q1q2_2T = genh1_eta[np.logical_and(nTagJets_to_LLP1>1,acc_LLP1_resolved_q1q2)]


                #concatenate everything
                r_num_r_qq_2T = np.concatenate((r_numLLP0_r_q1q2_2T,r_numLLP1_r_q1q2_2T)).flatten()
                r_num_z_qq_2T = np.concatenate((r_numLLP0_z_q1q2_2T,r_numLLP1_z_q1q2_2T)).flatten()
                r_num_pt_qq_2T = np.concatenate((r_numLLP0_pt_q1q2_2T,r_numLLP1_pt_q1q2_2T)).flatten()
                r_num_eta_qq_2T = np.concatenate((r_numLLP0_eta_q1q2_2T,r_numLLP1_eta_q1q2_2T)).flatten()

                #with preselections
                #At least one tag
                r_numLLP0_r_q1_T_presel   = genLLP0_r[np.logical_and(nTagJets_to_LLP0>0,np.logical_and(preselected,acc_LLP0_resolved_q1)) ]
                r_numLLP1_r_q1_T_presel   = genLLP1_r[np.logical_and(nTagJets_to_LLP1>0,np.logical_and(preselected,acc_LLP1_resolved_q1)) ]
                r_numLLP0_z_q1_T_presel   = genLLP0_z[np.logical_and(nTagJets_to_LLP0>0,np.logical_and(preselected,acc_LLP0_resolved_q1)) ]
                r_numLLP1_z_q1_T_presel   = genLLP1_z[np.logical_and(nTagJets_to_LLP1>0,np.logical_and(preselected,acc_LLP1_resolved_q1)) ]
                r_numLLP0_pt_q1_T_presel  = genb0_pt[np.logical_and(nTagJets_to_LLP0>0,np.logical_and(preselected,acc_LLP0_resolved_q1)) ]
                r_numLLP1_pt_q1_T_presel  = genb2_pt[np.logical_and(nTagJets_to_LLP1>0,np.logical_and(preselected,acc_LLP1_resolved_q1)) ]
                r_numLLP0_eta_q1_T_presel = genb0_eta[np.logical_and(nTagJets_to_LLP0>0,np.logical_and(preselected,acc_LLP0_resolved_q1)) ]
                r_numLLP1_eta_q1_T_presel = genb2_eta[np.logical_and(nTagJets_to_LLP1>0,np.logical_and(preselected,acc_LLP1_resolved_q1)) ]
                #attempt: use higgs also for q1 and q2
                #print "attempt!"
                #it performs a bit worse
                #r_numLLP0_pt_q1_T_presel  = genh0_pt[np.logical_and(nTagJets_to_LLP0>0,np.logical_and(preselected,acc_LLP0_resolved_q1)) ]
                #r_numLLP1_pt_q1_T_presel  = genh1_pt[np.logical_and(nTagJets_to_LLP1>0,np.logical_and(preselected,acc_LLP1_resolved_q1)) ]
                #r_numLLP0_eta_q1_T_presel = genh0_eta[np.logical_and(nTagJets_to_LLP0>0,np.logical_and(preselected,acc_LLP0_resolved_q1)) ]
                #r_numLLP1_eta_q1_T_presel = genh1_eta[np.logical_and(nTagJets_to_LLP1>0,np.logical_and(preselected,acc_LLP1_resolved_q1)) ]

                r_numLLP0_r_q2_T_presel   = genLLP0_r[np.logical_and(nTagJets_to_LLP0>0,np.logical_and(preselected,acc_LLP0_resolved_q2)) ]
                r_numLLP1_r_q2_T_presel   = genLLP1_r[np.logical_and(nTagJets_to_LLP1>0,np.logical_and(preselected,acc_LLP1_resolved_q2)) ]
                r_numLLP0_z_q2_T_presel   = genLLP0_z[np.logical_and(nTagJets_to_LLP0>0,np.logical_and(preselected,acc_LLP0_resolved_q2)) ]
                r_numLLP1_z_q2_T_presel   = genLLP1_z[np.logical_and(nTagJets_to_LLP1>0,np.logical_and(preselected,acc_LLP1_resolved_q2)) ]
                r_numLLP0_pt_q2_T_presel  = genb1_pt[np.logical_and(nTagJets_to_LLP0>0,np.logical_and(preselected,acc_LLP0_resolved_q2)) ]
                r_numLLP1_pt_q2_T_presel  = genb3_pt[np.logical_and(nTagJets_to_LLP1>0,np.logical_and(preselected,acc_LLP1_resolved_q2)) ]
                r_numLLP0_eta_q2_T_presel = genb1_eta[np.logical_and(nTagJets_to_LLP0>0,np.logical_and(preselected,acc_LLP0_resolved_q2)) ]
                r_numLLP1_eta_q2_T_presel = genb3_eta[np.logical_and(nTagJets_to_LLP1>0,np.logical_and(preselected,acc_LLP1_resolved_q2)) ] 
                #attempt: use higgs also for q1 and q2
                #print "attempt!"
                #it performs a bit worse
                #r_numLLP0_pt_q2_T_presel  = genh0_pt[np.logical_and(nTagJets_to_LLP0>0,np.logical_and(preselected,acc_LLP0_resolved_q2)) ]
                #r_numLLP1_pt_q2_T_presel  = genh1_pt[np.logical_and(nTagJets_to_LLP1>0,np.logical_and(preselected,acc_LLP1_resolved_q2)) ]
                #r_numLLP0_eta_q2_T_presel = genh0_eta[np.logical_and(nTagJets_to_LLP0>0,np.logical_and(preselected,acc_LLP0_resolved_q2)) ]
                #r_numLLP1_eta_q2_T_presel = genh1_eta[np.logical_and(nTagJets_to_LLP1>0,np.logical_and(preselected,acc_LLP1_resolved_q2)) ] 

                r_numLLP0_r_q1q2_1T_presel   = genLLP0_r[np.logical_and(nTagJets_to_LLP0==1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_r_q1q2_1T_presel   = genLLP1_r[np.logical_and(nTagJets_to_LLP1==1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]
                r_numLLP0_z_q1q2_1T_presel   = genLLP0_z[np.logical_and(nTagJets_to_LLP0==1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_z_q1q2_1T_presel   = genLLP1_z[np.logical_and(nTagJets_to_LLP1==1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]
                r_numLLP0_pt1_q1q2_1T_presel  = genb0_pt[np.logical_and(nTagJets_to_LLP0==1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_pt1_q1q2_1T_presel  = genb2_pt[np.logical_and(nTagJets_to_LLP1==1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]
                r_numLLP0_pt2_q1q2_1T_presel  = genb1_pt[np.logical_and(nTagJets_to_LLP0==1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_pt2_q1q2_1T_presel  = genb3_pt[np.logical_and(nTagJets_to_LLP1==1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]
                r_numLLP0_eta1_q1q2_1T_presel = genb0_eta[np.logical_and(nTagJets_to_LLP0==1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_eta1_q1q2_1T_presel = genb2_eta[np.logical_and(nTagJets_to_LLP1==1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]
                r_numLLP0_eta2_q1q2_1T_presel = genb1_eta[np.logical_and(nTagJets_to_LLP0==1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_eta2_q1q2_1T_presel = genb3_eta[np.logical_and(nTagJets_to_LLP1==1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]
                #new: when q1q2, use higgs
                r_numLLP0_pt_q1q2_1T_presel  = genh0_pt[np.logical_and(nTagJets_to_LLP0==1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_pt_q1q2_1T_presel  = genh1_pt[np.logical_and(nTagJets_to_LLP1==1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]
                r_numLLP0_eta_q1q2_1T_presel = genh0_eta[np.logical_and(nTagJets_to_LLP0==1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_eta_q1q2_1T_presel = genh1_eta[np.logical_and(nTagJets_to_LLP1==1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]

                r_numLLP0_r_q1q2_2T_presel   = genLLP0_r[np.logical_and(nTagJets_to_LLP0>1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_r_q1q2_2T_presel   = genLLP1_r[np.logical_and(nTagJets_to_LLP1>1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]
                r_numLLP0_z_q1q2_2T_presel   = genLLP0_z[np.logical_and(nTagJets_to_LLP0>1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_z_q1q2_2T_presel   = genLLP1_z[np.logical_and(nTagJets_to_LLP1>1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]
                r_numLLP0_pt1_q1q2_2T_presel  = genb0_pt[np.logical_and(nTagJets_to_LLP0>1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_pt1_q1q2_2T_presel  = genb2_pt[np.logical_and(nTagJets_to_LLP1>1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]
                r_numLLP0_pt2_q1q2_2T_presel  = genb1_pt[np.logical_and(nTagJets_to_LLP0>1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_pt2_q1q2_2T_presel  = genb3_pt[np.logical_and(nTagJets_to_LLP1>1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]
                r_numLLP0_eta1_q1q2_2T_presel = genb0_eta[np.logical_and(nTagJets_to_LLP0>1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_eta1_q1q2_2T_presel = genb2_eta[np.logical_and(nTagJets_to_LLP1>1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]
                r_numLLP0_eta2_q1q2_2T_presel = genb1_eta[np.logical_and(nTagJets_to_LLP0>1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_eta2_q1q2_2T_presel = genb3_eta[np.logical_and(nTagJets_to_LLP1>1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]
                #new: when q1q2, use higgs
                r_numLLP0_pt_q1q2_2T_presel  = genh0_pt[np.logical_and(nTagJets_to_LLP0>1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_pt_q1q2_2T_presel  = genh1_pt[np.logical_and(nTagJets_to_LLP1>1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]
                r_numLLP0_eta_q1q2_2T_presel = genh0_eta[np.logical_and(nTagJets_to_LLP0>1,np.logical_and(preselected,acc_LLP0_resolved_q1q2)) ]
                r_numLLP1_eta_q1q2_2T_presel = genh1_eta[np.logical_and(nTagJets_to_LLP1>1,np.logical_and(preselected,acc_LLP1_resolved_q1q2)) ]


                #One map per b-quark when q1, q2
                #q1
                r_den_LLP0_q1, r_den_LLP0_edges_q1 = np.histogramdd(np.transpose([r_denLLP0_r_q1,r_denLLP0_z_q1,r_denLLP0_pt_q1,r_denLLP0_eta_q1]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_den_LLP1_q1, r_den_LLP1_edges_q1 = np.histogramdd(np.transpose([r_denLLP1_r_q1,r_denLLP1_z_q1,r_denLLP1_pt_q1,r_denLLP1_eta_q1]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                #attempt: use higgs also for q1 and q2
                #print "attempt!"
                #it performs a bit worse
                #r_den_LLP0_q1, r_den_LLP0_edges_q1 = np.histogramdd(np.transpose([r_denLLP0_r_q1,r_denLLP0_z_q1,r_denLLP0_pt_q1,r_denLLP0_eta_q1]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                #r_den_LLP1_q1, r_den_LLP1_edges_q1 = np.histogramdd(np.transpose([r_denLLP1_r_q1,r_denLLP1_z_q1,r_denLLP1_pt_q1,r_denLLP1_eta_q1]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))

                #q2
                r_den_LLP0_q2, r_den_LLP0_edges_q2 = np.histogramdd(np.transpose([r_denLLP0_r_q2,r_denLLP0_z_q2,r_denLLP0_pt_q2,r_denLLP0_eta_q2]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_den_LLP1_q2, r_den_LLP1_edges_q2 = np.histogramdd(np.transpose([r_denLLP1_r_q2,r_denLLP1_z_q2,r_denLLP1_pt_q2,r_denLLP1_eta_q2]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                #attempt: use higgs also for q1 and q2
                #print "attempt!"
                #it performs a bit worse
                #r_den_LLP0_q2, r_den_LLP0_edges_q2 = np.histogramdd(np.transpose([r_denLLP0_r_q2,r_denLLP0_z_q2,r_denLLP0_pt_q2,r_denLLP0_eta_q2]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                #r_den_LLP1_q2, r_den_LLP1_edges_q2 = np.histogramdd(np.transpose([r_denLLP1_r_q2,r_denLLP1_z_q2,r_denLLP1_pt_q2,r_denLLP1_eta_q2]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))

                #6D maps for q1q2
                r_den_LLP0_q1q2, r_den_LLP0_edges_q1q2 = np.histogramdd(np.transpose([r_denLLP0_r_q1q2,r_denLLP0_z_q1q2,r_denLLP0_pt1_q1q2,r_denLLP0_pt2_q1q2,r_denLLP0_eta1_q1q2,r_denLLP0_eta2_q1q2]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_pt_resolved_q,bins_eta_resolved_q,bins_eta_resolved_q))
                r_den_LLP1_q1q2, r_den_LLP1_edges_q1q2 = np.histogramdd(np.transpose([r_denLLP1_r_q1q2,r_denLLP1_z_q1q2,r_denLLP1_pt1_q1q2,r_denLLP1_pt2_q1q2,r_denLLP1_eta1_q1q2,r_denLLP1_eta2_q1q2]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_pt_resolved_q,bins_eta_resolved_q,bins_eta_resolved_q))
                
                #new: when q1q2, use higgs
                #4D maps for q1q2
                r_den_LLP0_q1q2_new, r_den_LLP0_edges_q1q2_new = np.histogramdd(np.transpose([r_denLLP0_r_q1q2,r_denLLP0_z_q1q2,r_denLLP0_pt_q1q2,r_denLLP0_eta_q1q2]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                r_den_LLP1_q1q2_new, r_den_LLP1_edges_q1q2_new = np.histogramdd(np.transpose([r_denLLP1_r_q1q2,r_denLLP1_z_q1q2,r_denLLP1_pt_q1q2,r_denLLP1_eta_q1q2]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                
                #concatenate everything
                print "bins can be improved!"
                r_den_q, r_den_edges_q = np.histogramdd(np.transpose([r_den_r_q,r_den_z_q,r_den_pt_q,r_den_eta_q]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_den_qq, r_den_edges_qq = np.histogramdd(np.transpose([r_den_r_qq,r_den_z_qq,r_den_pt_qq,r_den_eta_qq]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))

                #w/o preselections
                #At least one tag
                #q1
                r_num_LLP0_q1_T, r_num_LLP0_edges_q1_T = np.histogramdd(np.transpose([r_numLLP0_r_q1_T,r_numLLP0_z_q1_T,r_numLLP0_pt_q1_T,r_numLLP0_eta_q1_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_num_LLP1_q1_T, r_num_LLP1_edges_q1_T = np.histogramdd(np.transpose([r_numLLP1_r_q1_T,r_numLLP1_z_q1_T,r_numLLP1_pt_q1_T,r_numLLP1_eta_q1_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                #attempt: use higgs also for q1 and q2
                #print "attempt!"
                #it performs a bit worse
                #r_num_LLP0_q1_T, r_num_LLP0_edges_q1_T = np.histogramdd(np.transpose([r_numLLP0_r_q1_T,r_numLLP0_z_q1_T,r_numLLP0_pt_q1_T,r_numLLP0_eta_q1_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                #r_num_LLP1_q1_T, r_num_LLP1_edges_q1_T = np.histogramdd(np.transpose([r_numLLP1_r_q1_T,r_numLLP1_z_q1_T,r_numLLP1_pt_q1_T,r_numLLP1_eta_q1_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                r_eff_LLP0_q1_T = np.divide(r_num_LLP0_q1_T,r_den_LLP0_q1,out=np.zeros_like(r_den_LLP0_q1),where=(r_den_LLP0_q1 != 0))
                r_eff_LLP1_q1_T = np.divide(r_num_LLP1_q1_T,r_den_LLP1_q1,out=np.zeros_like(r_den_LLP1_q1),where=(r_den_LLP1_q1 != 0))
                #q2
                r_num_LLP0_q2_T, r_num_LLP0_edges_q2_T = np.histogramdd(np.transpose([r_numLLP0_r_q2_T,r_numLLP0_z_q2_T,r_numLLP0_pt_q2_T,r_numLLP0_eta_q2_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_num_LLP1_q2_T, r_num_LLP1_edges_q2_T = np.histogramdd(np.transpose([r_numLLP1_r_q2_T,r_numLLP1_z_q2_T,r_numLLP1_pt_q2_T,r_numLLP1_eta_q2_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                #attempt: use higgs also for q1 and q2
                #print "attempt!"
                #it performs a bit worse
                #r_num_LLP0_q2_T, r_num_LLP0_edges_q2_T = np.histogramdd(np.transpose([r_numLLP0_r_q2_T,r_numLLP0_z_q2_T,r_numLLP0_pt_q2_T,r_numLLP0_eta_q2_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                #r_num_LLP1_q2_T, r_num_LLP1_edges_q2_T = np.histogramdd(np.transpose([r_numLLP1_r_q2_T,r_numLLP1_z_q2_T,r_numLLP1_pt_q2_T,r_numLLP1_eta_q2_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))

                r_eff_LLP0_q2_T = np.divide(r_num_LLP0_q2_T,r_den_LLP0_q2,out=np.zeros_like(r_den_LLP0_q2),where=(r_den_LLP0_q2 != 0))
                r_eff_LLP1_q2_T = np.divide(r_num_LLP1_q2_T,r_den_LLP1_q2,out=np.zeros_like(r_den_LLP1_q2),where=(r_den_LLP1_q2 != 0))


                #concatenate everything
                print "bins can be improved!"
                r_num_q_T, r_num_edges_q_T = np.histogramdd(np.transpose([r_num_r_q_T,r_num_z_q_T,r_num_pt_q_T,r_num_eta_q_T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_eff_q_T = np.divide(r_num_q_T,r_den_q,out=np.zeros_like(r_den_q),where=(r_den_q != 0))


                #6D maps for q1q2
                #One tag
                r_num_LLP0_q1q2_1T, r_num_LLP0_edges_q1q2_1T = np.histogramdd(np.transpose([r_numLLP0_r_q1q2_1T,r_numLLP0_z_q1q2_1T,r_numLLP0_pt1_q1q2_1T,r_numLLP0_pt2_q1q2_1T,r_numLLP0_eta1_q1q2_1T,r_numLLP0_eta2_q1q2_1T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_pt_resolved_q,bins_eta_resolved_q,bins_eta_resolved_q))
                ...
                r_eff_LLP0_q1q2_1T = np.divide(r_num_LLP0_q1q2_1T,r_den_LLP0_q1q2,out=np.zeros_like(r_den_LLP0_q1q2),where=(r_den_LLP0_q1q2 != 0))
                r_eff_LLP1_q1q2_1T = np.divide(r_num_LLP1_q1q2_1T,r_den_LLP1_q1q2,out=np.zeros_like(r_den_LLP1_q1q2),where=(r_den_LLP1_q1q2 != 0))
                #Two tags
                ...
                r_eff_LLP0_q1q2_2T = np.divide(r_num_LLP0_q1q2_2T,r_den_LLP0_q1q2,out=np.zeros_like(r_den_LLP0_q1q2),where=(r_den_LLP0_q1q2 != 0))
                r_eff_LLP1_q1q2_2T = np.divide(r_num_LLP1_q1q2_2T,r_den_LLP1_q1q2,out=np.zeros_like(r_den_LLP1_q1q2),where=(r_den_LLP1_q1q2 != 0))

                #new: when q1q2, use higgs
                #4D maps for q1q2
                #One tag
                r_num_LLP0_q1q2_1T_new, r_num_LLP0_edges_q1q2_1T_new = np.histogramdd(np.transpose([r_numLLP0_r_q1q2_1T,r_numLLP0_z_q1q2_1T,r_numLLP0_pt_q1q2_1T,r_numLLP0_eta_q1q2_1T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                r_num_LLP1_q1q2_1T_new, r_num_LLP1_edges_q1q2_1T_new = np.histogramdd(np.transpose([r_numLLP1_r_q1q2_1T,r_numLLP1_z_q1q2_1T,r_numLLP1_pt_q1q2_1T,r_numLLP1_eta_q1q2_1T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                r_eff_LLP0_q1q2_1T_new = np.divide(r_num_LLP0_q1q2_1T_new,r_den_LLP0_q1q2_new,out=np.zeros_like(r_den_LLP0_q1q2_new),where=(r_den_LLP0_q1q2_new != 0))
                r_eff_LLP1_q1q2_1T_new = np.divide(r_num_LLP1_q1q2_1T_new,r_den_LLP1_q1q2_new,out=np.zeros_like(r_den_LLP1_q1q2_new),where=(r_den_LLP1_q1q2_new != 0))
                #Two tags
                r_num_LLP0_q1q2_2T_new, r_num_LLP0_edges_q1q2_2T_new = np.histogramdd(np.transpose([r_numLLP0_r_q1q2_2T,r_numLLP0_z_q1q2_2T,r_numLLP0_pt_q1q2_2T,r_numLLP0_eta_q1q2_2T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                r_num_LLP1_q1q2_2T_new, r_num_LLP1_edges_q1q2_2T_new = np.histogramdd(np.transpose([r_numLLP1_r_q1q2_2T,r_numLLP1_z_q1q2_2T,r_numLLP1_pt_q1q2_2T,r_numLLP1_eta_q1q2_2T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                r_eff_LLP0_q1q2_2T_new = np.divide(r_num_LLP0_q1q2_2T_new,r_den_LLP0_q1q2_new,out=np.zeros_like(r_den_LLP0_q1q2_new),where=(r_den_LLP0_q1q2_new != 0))
                r_eff_LLP1_q1q2_2T_new = np.divide(r_num_LLP1_q1q2_2T_new,r_den_LLP1_q1q2_new,out=np.zeros_like(r_den_LLP1_q1q2_new),where=(r_den_LLP1_q1q2_new != 0))


                #concatenate everything
                print "bins can be improved!"
                r_num_qq_1T, r_num_edges_qq_1T = np.histogramdd(np.transpose([r_num_r_qq_1T,r_num_z_qq_1T,r_num_pt_qq_1T,r_num_eta_qq_1T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                r_eff_qq_1T = np.divide(r_num_qq_1T,r_den_qq,out=np.zeros_like(r_den_qq),where=(r_den_qq != 0))


                r_num_qq_2T, r_num_edges_qq_2T = np.histogramdd(np.transpose([r_num_r_qq_2T,r_num_z_qq_2T,r_num_pt_qq_2T,r_num_eta_qq_2T]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                r_eff_qq_2T = np.divide(r_num_qq_2T,r_den_qq,out=np.zeros_like(r_den_qq),where=(r_den_qq != 0))


                #with preselections
                #At least one tag
                #q1
                r_num_LLP0_q1_T_presel, r_num_LLP0_edges_q1_T_presel = np.histogramdd(np.transpose([r_numLLP0_r_q1_T_presel,r_numLLP0_z_q1_T_presel,r_numLLP0_pt_q1_T_presel,r_numLLP0_eta_q1_T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_num_LLP1_q1_T_presel, r_num_LLP1_edges_q1_T_presel = np.histogramdd(np.transpose([r_numLLP1_r_q1_T_presel,r_numLLP1_z_q1_T_presel,r_numLLP1_pt_q1_T_presel,r_numLLP1_eta_q1_T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                #attempt: use higgs also for q1 and q2
                #print "attempt!"
                #it performs a bit worse
                #r_num_LLP0_q1_T_presel, r_num_LLP0_edges_q1_T_presel = np.histogramdd(np.transpose([r_numLLP0_r_q1_T_presel,r_numLLP0_z_q1_T_presel,r_numLLP0_pt_q1_T_presel,r_numLLP0_eta_q1_T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                #r_num_LLP1_q1_T_presel, r_num_LLP1_edges_q1_T_presel = np.histogramdd(np.transpose([r_numLLP1_r_q1_T_presel,r_numLLP1_z_q1_T_presel,r_numLLP1_pt_q1_T_presel,r_numLLP1_eta_q1_T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))

                r_eff_LLP0_q1_T_presel = np.divide(r_num_LLP0_q1_T_presel,r_den_LLP0_q1,out=np.zeros_like(r_den_LLP0_q1),where=(r_den_LLP0_q1 != 0))
                r_eff_LLP1_q1_T_presel = np.divide(r_num_LLP1_q1_T_presel,r_den_LLP1_q1,out=np.zeros_like(r_den_LLP1_q1),where=(r_den_LLP1_q1 != 0))
                #q2
                r_num_LLP0_q2_T_presel, r_num_LLP0_edges_q2_T_presel = np.histogramdd(np.transpose([r_numLLP0_r_q2_T_presel,r_numLLP0_z_q2_T_presel,r_numLLP0_pt_q2_T_presel,r_numLLP0_eta_q2_T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                r_num_LLP1_q2_T_presel, r_num_LLP1_edges_q2_T_presel = np.histogramdd(np.transpose([r_numLLP1_r_q2_T_presel,r_numLLP1_z_q2_T_presel,r_numLLP1_pt_q2_T_presel,r_numLLP1_eta_q2_T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_eta_resolved_q))
                #attempt: use higgs also for q1 and q2
                #print "attempt!"
                #it performs a bit worse
                #r_num_LLP0_q2_T_presel, r_num_LLP0_edges_q2_T_presel = np.histogramdd(np.transpose([r_numLLP0_r_q2_T_presel,r_numLLP0_z_q2_T_presel,r_numLLP0_pt_q2_T_presel,r_numLLP0_eta_q2_T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                #r_num_LLP1_q2_T_presel, r_num_LLP1_edges_q2_T_presel = np.histogramdd(np.transpose([r_numLLP1_r_q2_T_presel,r_numLLP1_z_q2_T_presel,r_numLLP1_pt_q2_T_presel,r_numLLP1_eta_q2_T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))

                r_eff_LLP0_q2_T_presel = np.divide(r_num_LLP0_q2_T_presel,r_den_LLP0_q2,out=np.zeros_like(r_den_LLP0_q2),where=(r_den_LLP0_q2 != 0))
                r_eff_LLP1_q2_T_presel = np.divide(r_num_LLP1_q2_T_presel,r_den_LLP1_q2,out=np.zeros_like(r_den_LLP1_q2),where=(r_den_LLP1_q2 != 0))
                #6D maps for q1q2
                #One tag
                r_num_LLP0_q1q2_1T_presel, r_num_LLP0_edges_q1q2_1T_presel = np.histogramdd(np.transpose([r_numLLP0_r_q1q2_1T_presel,r_numLLP0_z_q1q2_1T_presel,r_numLLP0_pt1_q1q2_1T_presel,r_numLLP0_pt2_q1q2_1T_presel,r_numLLP0_eta1_q1q2_1T_presel,r_numLLP0_eta2_q1q2_1T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_pt_resolved_q,bins_eta_resolved_q,bins_eta_resolved_q))
                r_num_LLP1_q1q2_1T_presel, r_num_LLP1_edges_q1q2_1T_presel = np.histogramdd(np.transpose([r_numLLP1_r_q1q2_1T_presel,r_numLLP1_z_q1q2_1T_presel,r_numLLP1_pt1_q1q2_1T_presel,r_numLLP1_pt2_q1q2_1T_presel,r_numLLP1_eta1_q1q2_1T_presel,r_numLLP1_eta2_q1q2_1T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_pt_resolved_q,bins_eta_resolved_q,bins_eta_resolved_q))
                r_eff_LLP0_q1q2_1T_presel = np.divide(r_num_LLP0_q1q2_1T_presel,r_den_LLP0_q1q2,out=np.zeros_like(r_den_LLP0_q1q2),where=(r_den_LLP0_q1q2 != 0))
                r_eff_LLP1_q1q2_1T_presel = np.divide(r_num_LLP1_q1q2_1T_presel,r_den_LLP1_q1q2,out=np.zeros_like(r_den_LLP1_q1q2),where=(r_den_LLP1_q1q2 != 0))
                #Two tags
                r_num_LLP0_q1q2_2T_presel, r_num_LLP0_edges_q1q2_2T_presel = np.histogramdd(np.transpose([r_numLLP0_r_q1q2_2T_presel,r_numLLP0_z_q1q2_2T_presel,r_numLLP0_pt1_q1q2_2T_presel,r_numLLP0_pt2_q1q2_2T_presel,r_numLLP0_eta1_q1q2_2T_presel,r_numLLP0_eta2_q1q2_2T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_pt_resolved_q,bins_eta_resolved_q,bins_eta_resolved_q))
                r_num_LLP1_q1q2_2T_presel, r_num_LLP1_edges_q1q2_2T_presel = np.histogramdd(np.transpose([r_numLLP1_r_q1q2_2T_presel,r_numLLP1_z_q1q2_2T_presel,r_numLLP1_pt1_q1q2_2T_presel,r_numLLP1_pt2_q1q2_2T_presel,r_numLLP1_eta1_q1q2_2T_presel,r_numLLP1_eta2_q1q2_2T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_q,bins_pt_resolved_q,bins_eta_resolved_q,bins_eta_resolved_q))
                r_eff_LLP0_q1q2_2T_presel = np.divide(r_num_LLP0_q1q2_2T_presel,r_den_LLP0_q1q2,out=np.zeros_like(r_den_LLP0_q1q2),where=(r_den_LLP0_q1q2 != 0))
                r_eff_LLP1_q1q2_2T_presel = np.divide(r_num_LLP1_q1q2_2T_presel,r_den_LLP1_q1q2,out=np.zeros_like(r_den_LLP1_q1q2),where=(r_den_LLP1_q1q2 != 0))

                #new: when q1q2, use higgs
                #4D maps for q1q2
                #One tag
                r_num_LLP0_q1q2_1T_presel_new, r_num_LLP0_edges_q1q2_1T_presel_new = np.histogramdd(np.transpose([r_numLLP0_r_q1q2_1T_presel,r_numLLP0_z_q1q2_1T_presel,r_numLLP0_pt_q1q2_1T_presel,r_numLLP0_eta_q1q2_1T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                r_num_LLP1_q1q2_1T_presel_new, r_num_LLP1_edges_q1q2_1T_presel_new = np.histogramdd(np.transpose([r_numLLP1_r_q1q2_1T_presel,r_numLLP1_z_q1q2_1T_presel,r_numLLP1_pt_q1q2_1T_presel,r_numLLP1_eta_q1q2_1T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                r_eff_LLP0_q1q2_1T_presel_new = np.divide(r_num_LLP0_q1q2_1T_presel_new,r_den_LLP0_q1q2_new,out=np.zeros_like(r_den_LLP0_q1q2_new),where=(r_den_LLP0_q1q2_new != 0))
                r_eff_LLP1_q1q2_1T_presel_new = np.divide(r_num_LLP1_q1q2_1T_presel_new,r_den_LLP1_q1q2_new,out=np.zeros_like(r_den_LLP1_q1q2_new),where=(r_den_LLP1_q1q2_new != 0))
                #Two tags
                r_num_LLP0_q1q2_2T_presel_new, r_num_LLP0_edges_q1q2_2T_presel_new = np.histogramdd(np.transpose([r_numLLP0_r_q1q2_2T_presel,r_numLLP0_z_q1q2_2T_presel,r_numLLP0_pt_q1q2_2T_presel,r_numLLP0_eta_q1q2_2T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                r_num_LLP1_q1q2_2T_presel_new, r_num_LLP1_edges_q1q2_2T_presel_new = np.histogramdd(np.transpose([r_numLLP1_r_q1q2_2T_presel,r_numLLP1_z_q1q2_2T_presel,r_numLLP1_pt_q1q2_2T_presel,r_numLLP1_eta_q1q2_2T_presel]), bins = (bins_r_resolved,bins_z_resolved,bins_pt_resolved_h,bins_eta_resolved_h))
                r_eff_LLP0_q1q2_2T_presel_new = np.divide(r_num_LLP0_q1q2_2T_presel_new,r_den_LLP0_q1q2_new,out=np.zeros_like(r_den_LLP0_q1q2_new),where=(r_den_LLP0_q1q2_new != 0))
                r_eff_LLP1_q1q2_2T_presel_new = np.divide(r_num_LLP1_q1q2_2T_presel_new,r_den_LLP1_q1q2_new,out=np.zeros_like(r_den_LLP1_q1q2_new),where=(r_den_LLP1_q1q2_new != 0))
        ...
        maps[b]['resolved']['bins_eta'] = bins_eta_resolved_q
        '''
        ...
        maps[b]['resolved']['eff_qq_1T'] = r_eff_qq_1T
        maps[b]['resolved']['eff_LLP0_q1q2_1T_new'] = r_eff_LLP0_q1q2_1T_new
        maps[b]['resolved']['eff_LLP1_q1q2_1T_new'] = r_eff_LLP1_q1q2_1T_new
        maps[b]['resolved']['eff_LLP0_q1q2_2T_new'] = r_eff_LLP0_q1q2_2T_new
        maps[b]['resolved']['eff_LLP1_q1q2_2T_new'] = r_eff_LLP1_q1q2_2T_new
        maps[b]['resolved']['eff_qq_2T'] = r_eff_qq_2T

        #print "Compare maps"
        #print 'eff_qq_1T'
        #print maps[b]['resolved']['eff_qq_1T']
        #print 'eff_LLP1_q1q2_1T_new'
        #print maps[b]['resolved']['eff_LLP1_q1q2_1T_new']
        #print 'eff_LLP0_q1q2_1T_new'
        #print maps[b]['resolved']['eff_LLP0_q1q2_1T_new']
        #exit()


        maps[b]['resolved']['eff_LLP0_q1_presel'] = r_eff_LLP0_q1_T_presel
        maps[b]['resolved']['eff_LLP1_q1_presel'] = r_eff_LLP1_q1_T_presel
        maps[b]['resolved']['eff_LLP0_q2_presel'] = r_eff_LLP0_q2_T_presel
        maps[b]['resolved']['eff_LLP1_q2_presel'] = r_eff_LLP1_q2_T_presel
        maps[b]['resolved']['eff_LLP0_q1q2_1T_presel'] = r_eff_LLP0_q1q2_1T_presel
        maps[b]['resolved']['eff_LLP1_q1q2_1T_presel'] = r_eff_LLP1_q1q2_1T_presel
        maps[b]['resolved']['eff_LLP0_q1q2_2T_presel'] = r_eff_LLP0_q1q2_2T_presel
        maps[b]['resolved']['eff_LLP1_q1q2_2T_presel'] = r_eff_LLP1_q1q2_2T_presel
        maps[b]['resolved']['eff_LLP0_q1q2_1T_presel_new'] = r_eff_LLP0_q1q2_1T_presel_new
        maps[b]['resolved']['eff_LLP1_q1q2_1T_presel_new'] = r_eff_LLP1_q1q2_1T_presel_new
        maps[b]['resolved']['eff_LLP0_q1q2_2T_presel_new'] = r_eff_LLP0_q1q2_2T_presel_new
        maps[b]['resolved']['eff_LLP1_q1q2_2T_presel_new'] = r_eff_LLP1_q1q2_2T_presel_new
        ...
        maps[b]['resolved']['bins_eta'] = bins_eta_resolved_q
        #new: when q1q2, use higgs
        maps[b]['resolved']['bins_pt_h'] = bins_pt_resolved_h
        maps[b]['resolved']['bins_eta_h'] = bins_eta_resolved_h
...
'''
def use_maps(sign, skip_odd=False, cross_maps=""):
    ...
    table_r1_LLP0_presel = PrettyTable([
        'signal',
        'true q1 LLP0',
        'pred q1 LLP0',
        'diff q1 LLP0',
    ...
    table_r2_LLP0_presel = PrettyTable([
        'signal',
        'true q1q2 1T LLP0',
        'pred q1q2 1T LLP0',
        'diff q1q2 1T LLP0',
    ...
    table_r1_LLP1_presel = PrettyTable([
        'signal',
        'true q1 LLP1',
        'pred q1 LLP1',
        'diff q1 LLP1',
    ...
    table_r2_LLP1_presel = PrettyTable([
        'signal',
        'true q1q2 1T LLP1',
        'pred q1q2 1T LLP1',
        'diff q1q2 1T LLP1',
    ...
    for b in sign:
        ...
        if cross_maps=="":
            map_file = MAIN_OUT+"Tag_eff_map_"+b+".yaml" 
        else:
            map_name = b.replace("HH",cross_maps).replace("HZ",cross_maps).replace("ZZ",cross_maps)
            map_file = MAIN_OUT+"Tag_eff_map_"+map_name+".yaml" 
        ...
        map_file_H = MAIN_OUT+"Tag_eff_map_"+map_name_H+".yaml" 

        map_name_Z = b.replace("HZ","ZZ").replace("HH","ZZ")
        map_file_Z = MAIN_OUT+"Tag_eff_map_"+map_name_Z+".yaml" 
        ...
        for i, ss in enumerate(samples[b]['files']):
            ...
            for arrays in gen:
                ...
                preselected = arrays["isPreselected"][start:n_events:step]
                ...
                genh0_id = np.transpose(np.stack(( arrays["GenHiggs.pdgId"][start:n_events:step] )))[0:1]
                genh1_id = np.transpose(np.stack(( arrays["GenHiggs.pdgId"][start:n_events:step] )))[1:2]
        ...
        nTagLLP0_r_q1_presel    = nTagJets_to_LLP0[np.logical_and(preselected,LLP0_resolved_q1)]
        nTagLLP0_r_q2_presel    = nTagJets_to_LLP0[np.logical_and(preselected,LLP0_resolved_q2)]
        nTagLLP0_r_q1q2_presel  = nTagJets_to_LLP0[np.logical_and(preselected,LLP0_resolved_q1q2)]

        nTagLLP1_r_q1_presel    = nTagJets_to_LLP1[np.logical_and(preselected,LLP1_resolved_q1)]
        nTagLLP1_r_q2_presel    = nTagJets_to_LLP1[np.logical_and(preselected,LLP1_resolved_q2)]
        nTagLLP1_r_q1q2_presel  = nTagJets_to_LLP1[np.logical_and(preselected,LLP1_resolved_q1q2)]
        ...
        #TODO
        print "Bin definition must be unified"
        ...
        m_LLP0_eta_idx = np.digitize(m_LLP0_eta,bins_eta_merged)-1
        ...
        m_LLP1_eta_idx = np.digitize(m_LLP1_eta,bins_eta_merged)-1
        ...
        nTagLLP0_pred_m_unified = 0
        nTagLLP1_pred_m_unified = 0
        ...
        for it in range(len(m_LLP0_r_idx)):
            ...
            nTagLLP0_pred_m_unified+= maps['merged']['eff'][ m_LLP0_r_idx[it] ][  m_LLP0_z_idx[it] ][  m_LLP0_pt_idx[it] ][  m_LLP0_eta_idx[it] ]

            pred_presel = maps['merged']['eff_LLP0_presel'][ m_LLP0_r_idx[it] ][  m_LLP0_z_idx[it] ][  m_LLP0_pt_idx[it] ][  m_LLP0_eta_idx[it] ]
        ...
        for it in range(len(m_LLP1_r_idx)):
            ...
            nTagLLP1_pred_m_unified+= maps['merged']['eff'][ m_LLP1_r_idx[it] ][  m_LLP1_z_idx[it] ][  m_LLP1_pt_idx[it] ][  m_LLP1_eta_idx[it] ]
            #print "check map value ", maps['merged']['eff'][ m_LLP1_r_idx[it] ][  m_LLP1_z_idx[it] ][  m_LLP1_pt_idx[it] ][  m_LLP1_eta_idx[it] ]
            #if maps['merged']['eff'][ m_LLP1_r_idx[it] ][  m_LLP1_z_idx[it] ][  m_LLP1_pt_idx[it] ][  m_LLP1_eta_idx[it] ] > 1.1e+20:
            #    print maps['merged']['eff']
            #    print m_LLP1_r_idx[it], m_LLP1_z_idx[it], m_LLP1_pt_idx[it], m_LLP1_eta_idx[it]
            #print "temp unified LLP1 merged: ", nTagLLP1_pred_m_unified


            pred_presel = maps['merged']['eff_LLP1_presel'][ m_LLP1_r_idx[it] ][  m_LLP1_z_idx[it] ][  m_LLP1_pt_idx[it] ][  m_LLP1_eta_idx[it] ]
        ...
        row = [
            ...
            str( round( 100*( (nTagLLP0_pred_m + nTagLLP1_pred_m - nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum() - nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum())/ ( nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum() + nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum() ) ), 2) ) + "%",

            #str(nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum() + nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum()) + " +- " + str( round( (100*np.sqrt(nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum() + nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum()) / (nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum()+ nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum()) ),2) ) + "%",
            #str( round(nTagLLP0_pred_m + nTagLLP1_pred_m,2) ) + " +- " + str( round( (100*np.sqrt(nTagLLP0_pred_m + nTagLLP1_pred_m) / (nTagLLP0_pred_m + nTagLLP1_pred_m)),2) ) + "%",
            #str( round( 100*( (nTagLLP0_pred_m+nTagLLP1_pred_m-nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum()-nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum())/ ( nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum() + nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum() ) ), 2) ) + "%",
        ]
        ...
        row_unified = [
            b,
            str(nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum()) + " +- " + str( round( (100*np.sqrt(nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum()) / nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum() ),2) ) + "%",
            str( round(nTagLLP0_pred_m_unified,2) ),
            str( round( 100*( (nTagLLP0_pred_m_unified-nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum())/ ( nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum() ) ), 2) ) + "%",
        
            str(nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum()) + " +- " + str( round( (100*np.sqrt(nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum()) / nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum() ),2) ) + "%",
            str( round(nTagLLP1_pred_m_unified,2) ),
            str( round( 100*( (nTagLLP1_pred_m_unified-nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum())/ ( nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum() ) ), 2) ) + "%",


            str( round(nTagLLP0_pred_m_unified+nTagLLP1_pred_m_unified,2) ),
            str( round( 100*( (nTagLLP0_pred_m_unified + nTagLLP1_pred_m_unified - nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum() - nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum())/ ( nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum() + nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum() ) ), 2) ) + "%",
        ]
        table_m.add_row(row_unified)
        table_m_unified.add_row(row_unified)

        row_presel = [
            b,
            str(nTagLLP0_m_presel[nTagLLP0_m_presel>0].astype(bool).sum()) + " +- " + str( round( (100*np.sqrt(nTagLLP0_m_presel[nTagLLP0_m_presel>0].astype(bool).sum()) / nTagLLP0_m_presel[nTagLLP0_m_presel>0].astype(bool).sum() ),2) ) + "%",
            str( round(nTagLLP0_pred_m_presel,2) ),# + " +- " + str( round( (100*np.sqrt(nTagLLP0_pred_m_presel) / nTagLLP0_pred_m_presel),2) ) + "%",
            str( round( 100*( (nTagLLP0_pred_m_presel-nTagLLP0_m_presel[nTagLLP0_m_presel>0].astype(bool).sum())/ ( nTagLLP0_m_presel[nTagLLP0_m_presel>0].astype(bool).sum() ) ), 2) ) + "%",
        
            str(nTagLLP1_m_presel[nTagLLP1_m_presel>0].astype(bool).sum()) + " +- " + str( round( (100*np.sqrt(nTagLLP1_m_presel[nTagLLP1_m_presel>0].astype(bool).sum()) / nTagLLP1_m_presel[nTagLLP1_m_presel>0].astype(bool).sum() ),2) ) + "%",
            str( round(nTagLLP1_pred_m_presel,2) ),# + " +- " + str( round( (100*np.sqrt(nTagLLP1_pred_m_presel) / nTagLLP1_pred_m_presel),2) ) + "%",
            str( round( 100*( (nTagLLP1_pred_m_presel-nTagLLP1_m_presel[nTagLLP1_m_presel>0].astype(bool).sum())/ ( nTagLLP1_m_presel[nTagLLP1_m_presel>0].astype(bool).sum() ) ), 2) ) + "%",
        ]
        ...
        #print "attempt!"
        #it performs a bit worse
        #r_LLP0_pt_q1 = LLP0_pt[LLP0_resolved_q1]
        #r_LLP0_eta_q1 = LLP0_eta[LLP0_resolved_q1]
        ...
        #print "attempt!"
        #it performs a bit worse
        #r_LLP0_pt_q2 = LLP0_pt[LLP0_resolved_q2]
        #r_LLP0_eta_q2 = LLP0_eta[LLP0_resolved_q2]
        ...
        #new: when q1q2, use higgs
        r_LLP0_pt_q1q2 = LLP0_pt[LLP0_resolved_q1q2]
        r_LLP0_eta_q1q2 = LLP0_eta[LLP0_resolved_q1q2]
        ...
        #print "attempt!"
        #it performs a bit worse
        #r_LLP1_pt_q1 = LLP1_pt[LLP1_resolved_q1]
        #r_LLP1_eta_q1 = LLP1_eta[LLP1_resolved_q1]
        ...
        #print "attempt!"
        #it performs a bit worse
        #r_LLP1_pt_q2 = LLP1_pt[LLP1_resolved_q2]
        #r_LLP1_eta_q2 = LLP1_eta[LLP1_resolved_q2]
        ...
        #new: when q1q2, use higgs
        r_LLP1_pt_q1q2 = LLP1_pt[LLP1_resolved_q1q2]
        r_LLP1_eta_q1q2 = LLP1_eta[LLP1_resolved_q1q2]

        bins_r_resolved =  maps['resolved']['bins_r']
        bins_z_resolved = maps['resolved']['bins_z']
        bins_pt_resolved_q = maps['resolved']['bins_pt']
        bins_eta_resolved_q = maps['resolved']['bins_eta']

        #new: when q1q2, use higgs
        bins_pt_resolved_h = maps['resolved']['bins_pt_h']
        bins_eta_resolved_h = maps['resolved']['bins_eta_h']
        ...
        r_LLP0_eta_idx_q1  = np.digitize(r_LLP0_eta_q1,bins_eta_resolved_q)-1
        #attempt: use higgs also for q1 and q2
        #print "attempt!"
        #it performs a bit worse
        #r_LLP0_pt_idx_q1   = np.digitize(r_LLP0_pt_q1,bins_pt_resolved_h)-1
        #r_LLP0_eta_idx_q1  = np.digitize(r_LLP0_eta_q1,bins_eta_resolved_h)-1
        ...
        r_LLP0_eta_idx_q2  = np.digitize(r_LLP0_eta_q2,bins_eta_resolved_q)-1
        #attempt: use higgs also for q1 and q2
        #print "attempt!"
        #it performs a bit worse
        #r_LLP0_pt_idx_q2   = np.digitize(r_LLP0_pt_q2,bins_pt_resolved_h)-1
        #r_LLP0_eta_idx_q2  = np.digitize(r_LLP0_eta_q2,bins_eta_resolved_h)-1
        ...
        r_LLP0_eta1_idx_q1q2  = np.digitize(r_LLP0_eta1_q1q2,bins_eta_resolved_q)-1
        r_LLP0_eta2_idx_q1q2  = np.digitize(r_LLP0_eta2_q1q2,bins_eta_resolved_q)-1
        #new: when q1q2, use higgs
        r_LLP0_pt_idx_q1q2   = np.digitize(r_LLP0_pt_q1q2,bins_pt_resolved_h)-1
        r_LLP0_eta_idx_q1q2  = np.digitize(r_LLP0_eta_q1q2,bins_eta_resolved_h)-1
        ...
        r_LLP1_eta_idx_q1  = np.digitize(r_LLP1_eta_q1,bins_eta_resolved_q)-1
        #attempt: use higgs also for q1 and q2
        #print "attempt!"
        #it performs a bit worse
        #r_LLP1_pt_idx_q1   = np.digitize(r_LLP1_pt_q1,bins_pt_resolved_h)-1
        #r_LLP1_eta_idx_q1  = np.digitize(r_LLP1_eta_q1,bins_eta_resolved_h)-1
        ...
        r_LLP1_eta_idx_q2  = np.digitize(r_LLP1_eta_q2,bins_eta_resolved_q)-1
        #attempt: use higgs also for q1 and q2
        #print "attempt!"
        #it performs a bit worse
        #r_LLP1_pt_idx_q2   = np.digitize(r_LLP1_pt_q2,bins_pt_resolved_h)-1
        #r_LLP1_eta_idx_q2  = np.digitize(r_LLP1_eta_q2,bins_eta_resolved_h)-1
        ...
        r_LLP1_eta1_idx_q1q2  = np.digitize(r_LLP1_eta1_q1q2,bins_eta_resolved_q)-1
        r_LLP1_eta2_idx_q1q2  = np.digitize(r_LLP1_eta2_q1q2,bins_eta_resolved_q)-1
        #new: when q1q2, use higgs
        r_LLP1_pt_idx_q1q2   = np.digitize(r_LLP1_pt_q1q2,bins_pt_resolved_h)-1
        r_LLP1_eta_idx_q1q2  = np.digitize(r_LLP1_eta_q1q2,bins_eta_resolved_h)-1


        #LLP0 predictions
        nTagLLP0_pred_r_q1 = 0
        nTagLLP0_pred_r_q2 = 0
        nTagLLP0_pred_r_q1_unified = 0
        nTagLLP0_pred_r_q2_unified = 0

        nTagLLP0_pred_r_q1q2_1T = 0
        nTagLLP0_pred_r_q1q2_2T = 0
        #new: when q1q2, use higgs
        nTagLLP0_pred_r_q1q2_1T_new = 0
        nTagLLP0_pred_r_q1q2_2T_new = 0
        nTagLLP0_pred_r_q1q2_1T_new_unified = 0
        nTagLLP0_pred_r_q1q2_2T_new_unified = 0

        nTagLLP0_pred_r_q1_presel = 0
        nTagLLP0_pred_r_q2_presel = 0
        nTagLLP0_pred_r_q1q2_1T_presel = 0
        nTagLLP0_pred_r_q1q2_2T_presel = 0
        #new: when q1q2, use higgs
        nTagLLP0_pred_r_q1q2_1T_presel_new = 0
        nTagLLP0_pred_r_q1q2_2T_presel_new = 0
        ...
        for it in range(len(r_LLP0_r_idx_q1)):
            ...
            #unified
            nTagLLP0_pred_r_q1_unified += maps['resolved']['eff_q'][ r_LLP0_r_idx_q1[it] ][  r_LLP0_z_idx_q1[it] ][  r_LLP0_pt_idx_q1[it] ][  r_LLP0_eta_idx_q1[it] ]

            pred_presel = maps['resolved']['eff_LLP0_q1_presel'][ r_LLP0_r_idx_q1[it] ][  r_LLP0_z_idx_q1[it] ][  r_LLP0_pt_idx_q1[it] ][  r_LLP0_eta_idx_q1[it] ]
        ...
        for it in range(len(r_LLP0_r_idx_q2)):
            ...
            #unified
            nTagLLP0_pred_r_q2_unified += maps['resolved']['eff_q'][ r_LLP0_r_idx_q2[it] ][  r_LLP0_z_idx_q2[it] ][  r_LLP0_pt_idx_q2[it] ][  r_LLP0_eta_idx_q2[it] ]

            pred_presel = maps['resolved']['eff_LLP0_q2_presel'][ r_LLP0_r_idx_q2[it] ][  r_LLP0_z_idx_q2[it] ][  r_LLP0_pt_idx_q2[it] ][  r_LLP0_eta_idx_q2[it] ]
        ...
        for it in range(len(r_LLP0_r_idx_q1q2)):
            ...
            pred_1T_presel = maps['resolved']['eff_LLP0_q1q2_1T_presel'][ r_LLP0_r_idx_q1q2[it] ][  r_LLP0_z_idx_q1q2[it] ][  r_LLP0_pt1_idx_q1q2[it] ][  r_LLP0_pt2_idx_q1q2[it] ][  r_LLP0_eta1_idx_q1q2[it] ][  r_LLP0_eta2_idx_q1q2[it] ]
            nTagLLP0_pred_r_q1q2_1T_presel+= pred_1T_presel
            ...
            pred_2T_presel = maps['resolved']['eff_LLP0_q1q2_2T_presel'][ r_LLP0_r_idx_q1q2[it] ][  r_LLP0_z_idx_q1q2[it] ][  r_LLP0_pt1_idx_q1q2[it] ][  r_LLP0_pt2_idx_q1q2[it] ][  r_LLP0_eta1_idx_q1q2[it] ][  r_LLP0_eta2_idx_q1q2[it] ]
            nTagLLP0_pred_r_q1q2_2T_presel+= pred_2T_presel
        ...
        for it in range(len(r_LLP0_r_idx_q1q2)):
            #new: when q1q2, use higgs
            pred_1T_new = maps['resolved']['eff_LLP0_q1q2_1T_new'][ r_LLP0_r_idx_q1q2[it] ][  r_LLP0_z_idx_q1q2[it] ][  r_LLP0_pt_idx_q1q2[it] ][  r_LLP0_eta_idx_q1q2[it] ]
            nTagLLP0_pred_r_q1q2_1T_new+= pred_1T_new
            #print "debug pred_1T_new ", pred_1T_new

            #unified
            pred_1T_new_unified = maps['resolved']['eff_qq_1T'][ r_LLP0_r_idx_q1q2[it] ][  r_LLP0_z_idx_q1q2[it] ][  r_LLP0_pt_idx_q1q2[it] ][  r_LLP0_eta_idx_q1q2[it] ]
            nTagLLP0_pred_r_q1q2_1T_new_unified += pred_1T_new_unified
            #print "debug pred_1T_new_unified ", pred_1T_new_unified


            pred_1T_presel_new = maps['resolved']['eff_LLP0_q1q2_1T_presel_new'][ r_LLP0_r_idx_q1q2[it] ][  r_LLP0_z_idx_q1q2[it] ][  r_LLP0_pt_idx_q1q2[it] ][  r_LLP0_eta_idx_q1q2[it] ]
            nTagLLP0_pred_r_q1q2_1T_presel_new+= pred_1T_presel_new

            pred_2T_new = maps['resolved']['eff_LLP0_q1q2_2T_new'][ r_LLP0_r_idx_q1q2[it] ][  r_LLP0_z_idx_q1q2[it] ][  r_LLP0_pt_idx_q1q2[it] ][  r_LLP0_eta_idx_q1q2[it] ]
            nTagLLP0_pred_r_q1q2_2T_new+= pred_2T_new
            #print "debug nTagLLP0_pred_r_q1q2_2T_new tmp ", nTagLLP0_pred_r_q1q2_2T_new

            #unified
            nTagLLP0_pred_r_q1q2_2T_new_unified += maps['resolved']['eff_qq_2T'][ r_LLP0_r_idx_q1q2[it] ][  r_LLP0_z_idx_q1q2[it] ][  r_LLP0_pt_idx_q1q2[it] ][  r_LLP0_eta_idx_q1q2[it] ]
            #print "debug nTagLLP0_pred_r_q1q2_2T_new_unified tmp ", nTagLLP0_pred_r_q1q2_2T_new_unified

            pred_2T_presel_new = maps['resolved']['eff_LLP0_q1q2_2T_presel_new'][ r_LLP0_r_idx_q1q2[it] ][  r_LLP0_z_idx_q1q2[it] ][  r_LLP0_pt_idx_q1q2[it] ][  r_LLP0_eta_idx_q1q2[it] ]
            nTagLLP0_pred_r_q1q2_2T_presel_new+= pred_2T_presel_new
        ...
        #new: when q1q2, use higgs
        nTagLLP1_pred_r_q1q2_1T_new = 0
        nTagLLP1_pred_r_q1q2_2T_new = 0

        nTagLLP1_pred_r_q1_presel = 0
        nTagLLP1_pred_r_q2_presel = 0
        nTagLLP1_pred_r_q1q2_1T_presel = 0
        nTagLLP1_pred_r_q1q2_2T_presel = 0
        #new: when q1q2, use higgs
        nTagLLP1_pred_r_q1q2_1T_presel_new = 0
        nTagLLP1_pred_r_q1q2_2T_presel_new = 0
        ...
        for it in range(len(r_LLP1_r_idx_q1)):
            ...
            pred_presel = maps['resolved']['eff_LLP1_q1_presel'][ r_LLP1_r_idx_q1[it] ][  r_LLP1_z_idx_q1[it] ][  r_LLP1_pt_idx_q1[it] ][  r_LLP1_eta_idx_q1[it] ]
        ...
        for it in range(len(r_LLP1_r_idx_q2)):
            ...
            pred_presel = maps['resolved']['eff_LLP1_q2_presel'][ r_LLP1_r_idx_q2[it] ][  r_LLP1_z_idx_q2[it] ][  r_LLP1_pt_idx_q2[it] ][  r_LLP1_eta_idx_q2[it] ]
        ...
        for it in range(len(r_LLP1_r_idx_q1q2)):
            ...
            pred_1T_presel = maps['resolved']['eff_LLP1_q1q2_1T_presel'][ r_LLP1_r_idx_q1q2[it] ][  r_LLP1_z_idx_q1q2[it] ][  r_LLP1_pt1_idx_q1q2[it] ][  r_LLP1_pt2_idx_q1q2[it] ][  r_LLP1_eta1_idx_q1q2[it] ][  r_LLP1_eta2_idx_q1q2[it] ]
            nTagLLP1_pred_r_q1q2_1T_presel+= pred_1T_presel
            ...
            pred_2T_presel = maps['resolved']['eff_LLP1_q1q2_2T_presel'][ r_LLP1_r_idx_q1q2[it] ][  r_LLP1_z_idx_q1q2[it] ][  r_LLP1_pt1_idx_q1q2[it] ][  r_LLP1_pt2_idx_q1q2[it] ][  r_LLP1_eta1_idx_q1q2[it] ][  r_LLP1_eta2_idx_q1q2[it] ]
            nTagLLP1_pred_r_q1q2_2T_presel+= pred_2T_presel


        for it in range(len(r_LLP1_r_idx_q1q2)):
            #new: when q1q2, use higgs
            pred_1T_new = maps['resolved']['eff_LLP1_q1q2_1T_new'][ r_LLP1_r_idx_q1q2[it] ][  r_LLP1_z_idx_q1q2[it] ][  r_LLP1_pt_idx_q1q2[it] ][  r_LLP1_eta_idx_q1q2[it] ]
            nTagLLP1_pred_r_q1q2_1T_new+= pred_1T_new

            pred_1T_presel_new = maps['resolved']['eff_LLP1_q1q2_1T_presel_new'][ r_LLP1_r_idx_q1q2[it] ][  r_LLP1_z_idx_q1q2[it] ][  r_LLP1_pt_idx_q1q2[it] ][  r_LLP1_eta_idx_q1q2[it] ]
            nTagLLP1_pred_r_q1q2_1T_presel_new+= pred_1T_presel_new


            pred_2T_new = maps['resolved']['eff_LLP1_q1q2_2T_new'][ r_LLP1_r_idx_q1q2[it] ][  r_LLP1_z_idx_q1q2[it] ][  r_LLP1_pt_idx_q1q2[it] ][  r_LLP1_eta_idx_q1q2[it] ]
            nTagLLP1_pred_r_q1q2_2T_new+= pred_2T_new

            pred_2T_presel_new = maps['resolved']['eff_LLP1_q1q2_2T_presel_new'][ r_LLP1_r_idx_q1q2[it] ][  r_LLP1_z_idx_q1q2[it] ][  r_LLP1_pt_idx_q1q2[it] ][  r_LLP1_eta_idx_q1q2[it] ]
            nTagLLP1_pred_r_q1q2_2T_presel_new+= pred_2T_presel_new
        ...
        t_nTagLLP0_r_q1_unc = 100./np.sqrt(nTagLLP0_r_q1[nTagLLP0_r_q1>0].astype(bool).sum()) if np.sqrt(nTagLLP0_r_q1[nTagLLP0_r_q1>0].astype(bool).sum())>0 else 0
        ...
        t_diff_nTagLLP0_r_q1_unified = 100*( (nTagLLP0_pred_r_q1_unified-nTagLLP0_r_q1[nTagLLP0_r_q1>0].astype(bool).sum())/ ( nTagLLP0_r_q1[nTagLLP0_r_q1>0].astype(bool).sum() ) )



        t_nTagLLP0_r_q2 = nTagLLP0_r_q2[nTagLLP0_r_q2>0].astype(bool).sum()
        t_nTagLLP0_r_q2_unc = 100./np.sqrt(nTagLLP0_r_q2[nTagLLP0_r_q2>0].astype(bool).sum()) if np.sqrt(nTagLLP0_r_q2[nTagLLP0_r_q2>0].astype(bool).sum())>0 else 0
        ...
        t_diff_nTagLLP0_r_q2_unified = 100*( (nTagLLP0_pred_r_q2_unified-nTagLLP0_r_q2[nTagLLP0_r_q2>0].astype(bool).sum())/ ( nTagLLP0_r_q2[nTagLLP0_r_q2>0].astype(bool).sum() ) )

        t_nTagLLP0_r_q1q2_1T = nTagLLP0_r_q1q2[ nTagLLP0_r_q1q2==1 ].astype(bool).sum()
        t_nTagLLP0_r_q1q2_1T_unc = 100./np.sqrt(nTagLLP0_r_q1q2[ nTagLLP0_r_q1q2==1 ].astype(bool).sum()) if np.sqrt(nTagLLP0_r_q1q2[ nTagLLP0_r_q1q2==1 ].astype(bool).sum())>0 else 0
        ...
        #new: when q1q2, use higgs
        #only pred and diff change!
        t_diff_nTagLLP0_r_q1q2_1T_new = 100*( ( (nTagLLP0_pred_r_q1q2_1T_new)- nTagLLP0_r_q1q2[ nTagLLP0_r_q1q2==1 ].astype(bool).sum() )/ ( nTagLLP0_r_q1q2[ nTagLLP0_r_q1q2==1 ].astype(bool).sum() ) )
        t_diff_nTagLLP0_r_q1q2_1T_new_unified = 100*( ( (nTagLLP0_pred_r_q1q2_1T_new_unified)- nTagLLP0_r_q1q2[ nTagLLP0_r_q1q2==1 ].astype(bool).sum() )/ ( nTagLLP0_r_q1q2[ nTagLLP0_r_q1q2==1 ].astype(bool).sum() ) )


        t_nTagLLP0_r_q1q2_2T = nTagLLP0_r_q1q2[ nTagLLP0_r_q1q2>1 ].astype(bool).sum()
        t_nTagLLP0_r_q1q2_2T_unc = 100./np.sqrt(nTagLLP0_r_q1q2[ nTagLLP0_r_q1q2>1 ].astype(bool).sum()) if np.sqrt(nTagLLP0_r_q1q2[ nTagLLP0_r_q1q2>1 ].astype(bool).sum())>0 else 0
        ...
        #new: when q1q2, use higgs
        #only pred and diff change!
        t_diff_nTagLLP0_r_q1q2_2T_new = 100*( ( (nTagLLP0_pred_r_q1q2_2T_new)- nTagLLP0_r_q1q2[ nTagLLP0_r_q1q2>1 ].astype(bool).sum() )/ ( nTagLLP0_r_q1q2[ nTagLLP0_r_q1q2>1 ].astype(bool).sum() ) )
        t_diff_nTagLLP0_r_q1q2_2T_new_unified = 100*( ( (nTagLLP0_pred_r_q1q2_2T_new_unified)- nTagLLP0_r_q1q2[ nTagLLP0_r_q1q2>1 ].astype(bool).sum() )/ ( nTagLLP0_r_q1q2[ nTagLLP0_r_q1q2>1 ].astype(bool).sum() ) )
        ...
        row = [
            ...
            str(round(nTagLLP0_pred_r_q1,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q1),2) if np.sqrt(nTagLLP0_pred_r_q1)>0 else 0) + "%",
            str( round(t_diff_nTagLLP0_r_q1, 2) ) + "%",

            str(t_nTagLLP0_r_q2) + " +- " + str( round( t_nTagLLP0_r_q2_unc,2) ) + "%",
            str(round(nTagLLP0_pred_r_q2,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q2),2) if np.sqrt(nTagLLP0_pred_r_q2)>0 else 0 ) + "%",
        ...
        row_unified = [
            b,
            str(t_nTagLLP0_r_q1) + " +- " + str( round( t_nTagLLP0_r_q1_unc,2) ) + "%",
            str(round(nTagLLP0_pred_r_q1_unified,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q1),2) if np.sqrt(nTagLLP0_pred_r_q1)>0 else 0) + "%",
            str( round(t_diff_nTagLLP0_r_q1_unified, 2) ) + "%",

            str(t_nTagLLP0_r_q2) + " +- " + str( round( t_nTagLLP0_r_q2_unc,2) ) + "%",
            str(round(nTagLLP0_pred_r_q2_unified,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q2),2) if np.sqrt(nTagLLP0_pred_r_q2)>0 else 0 ) + "%",
            str( round(t_diff_nTagLLP0_r_q2_unified, 2) ) + "%",

        ]

        table_r1_LLP0.add_row(row)
        table_r1_LLP0.add_row(row_unified)
        ...
        row = [
            ...
            str(round(nTagLLP0_pred_r_q1q2_1T,2) ) + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q1q2_1T),2) if np.sqrt(nTagLLP0_pred_r_q1q2_1T)>0 else 0 ) + "%",
            str( round(t_diff_nTagLLP0_r_q1q2_1T, 2) ) + "%",

            str(t_nTagLLP0_r_q1q2_2T) + " +- " + str( round( t_nTagLLP0_r_q1q2_2T_unc,2) ) + "%",
            str(round(nTagLLP0_pred_r_q1q2_2T,2) ) + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q1q2_2T),2) if np.sqrt(nTagLLP0_pred_r_q1q2_2T)>0 else 0 ) + "%",
        ...
        row_new = [
            b,
            str(t_nTagLLP0_r_q1q2_1T) + " +- " + str( round( t_nTagLLP0_r_q1q2_1T_unc,2) ) + "%",
            str(round(nTagLLP0_pred_r_q1q2_1T_new,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q1q2_1T_new),2) if np.sqrt(nTagLLP0_pred_r_q1q2_1T_new)>0 else 0 ) + "%",
            str( round(t_diff_nTagLLP0_r_q1q2_1T_new, 2) ) + "%",

            str(t_nTagLLP0_r_q1q2_2T) + " +- " + str( round( t_nTagLLP0_r_q1q2_2T_unc,2) ) + "%",
            str(round(nTagLLP0_pred_r_q1q2_2T_new,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q1q2_2T_new),2) if np.sqrt(nTagLLP0_pred_r_q1q2_2T_new)>0 else 0) + "%",
            str( round(t_diff_nTagLLP0_r_q1q2_2T_new, 2) ) + "%",
        ]

        row_new_unified = [
            b,
            str(t_nTagLLP0_r_q1q2_1T) + " +- " + str( round( t_nTagLLP0_r_q1q2_1T_unc,2) ) + "%",
            str(round(nTagLLP0_pred_r_q1q2_1T_new_unified,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q1q2_1T_new),2) if np.sqrt(nTagLLP0_pred_r_q1q2_1T_new)>0 else 0 ) + "%",
            str( round(t_diff_nTagLLP0_r_q1q2_1T_new_unified, 2) ) + "%",

            str(t_nTagLLP0_r_q1q2_2T) + " +- " + str( round( t_nTagLLP0_r_q1q2_2T_unc,2) ) + "%",
            str(round(nTagLLP0_pred_r_q1q2_2T_new_unified,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q1q2_2T_new),2) if np.sqrt(nTagLLP0_pred_r_q1q2_2T_new)>0 else 0) + "%",
            str( round(t_diff_nTagLLP0_r_q1q2_2T_new_unified, 2) ) + "%",
        ]


        ##table_r2_LLP0.add_row(row)
        table_r2_LLP0.add_row(row_new)
        table_r2_LLP0.add_row(row_new_unified)
        ...
        t_nTagLLP1_r_q1_unc = 100./np.sqrt(nTagLLP1_r_q1[nTagLLP1_r_q1>0].astype(bool).sum()) if np.sqrt(nTagLLP1_r_q1[nTagLLP1_r_q1>0].astype(bool).sum())>0 else 0
        ...
        t_nTagLLP1_r_q2_unc = 100./np.sqrt(nTagLLP1_r_q2[nTagLLP1_r_q2>0].astype(bool).sum()) if np.sqrt(nTagLLP1_r_q2[nTagLLP1_r_q2>0].astype(bool).sum()) >0 else 0
        ...
        t_nTagLLP1_r_q1q2_1T_unc = 100./np.sqrt(nTagLLP1_r_q1q2[ nTagLLP1_r_q1q2==1 ].astype(bool).sum()) if np.sqrt(nTagLLP1_r_q1q2[ nTagLLP1_r_q1q2==1 ].astype(bool).sum())>0 else 0
        ...
        #new: when q1q2, use higgs
        #only pred and diff change!
        t_diff_nTagLLP1_r_q1q2_1T_new = 100*( ( (nTagLLP1_pred_r_q1q2_1T_new)- nTagLLP1_r_q1q2[ nTagLLP1_r_q1q2==1 ].astype(bool).sum() )/ ( nTagLLP1_r_q1q2[ nTagLLP1_r_q1q2==1 ].astype(bool).sum() ) )

        t_nTagLLP1_r_q1q2_2T = nTagLLP1_r_q1q2[ nTagLLP1_r_q1q2>1 ].astype(bool).sum()
        t_nTagLLP1_r_q1q2_2T_unc = 100./np.sqrt(nTagLLP1_r_q1q2[ nTagLLP1_r_q1q2>1 ].astype(bool).sum()) if np.sqrt(nTagLLP1_r_q1q2[ nTagLLP1_r_q1q2>1 ].astype(bool).sum())>0 else 0
        ...
        #new: when q1q2, use higgs
        #only pred and diff change!
        t_diff_nTagLLP1_r_q1q2_2T_new = 100*( ( (nTagLLP1_pred_r_q1q2_2T_new)- nTagLLP1_r_q1q2[ nTagLLP1_r_q1q2>1 ].astype(bool).sum() )/ ( nTagLLP1_r_q1q2[ nTagLLP1_r_q1q2>1 ].astype(bool).sum() ) )
        ...
        row = [
            ...
            str(round(nTagLLP1_pred_r_q1,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP1_pred_r_q1),2) if np.sqrt(nTagLLP1_pred_r_q1)>0 else 0) + "%",
            str( round(t_diff_nTagLLP1_r_q1, 2) ) + "%",

            str(t_nTagLLP1_r_q2) + " +- " + str( round( t_nTagLLP1_r_q2_unc,2) ) + "%",
            str(round(nTagLLP1_pred_r_q2,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP1_pred_r_q2),2) if np.sqrt(nTagLLP1_pred_r_q2)>0 else 0 ) + "%",
        ...
        row = [
            ...
            str(round(nTagLLP1_pred_r_q1q2_1T,2) ) + " +- " + str( round( 100./np.sqrt(nTagLLP1_pred_r_q1q2_1T),2) if nTagLLP1_pred_r_q1q2_1T>0 else 0 ) + "%",
            str( round(t_diff_nTagLLP1_r_q1q2_1T, 2) ) + "%",

            str(t_nTagLLP1_r_q1q2_2T) + " +- " + str( round( t_nTagLLP1_r_q1q2_2T_unc,2) ) + "%",
            str(round(nTagLLP1_pred_r_q1q2_2T,2) ) + " +- " + str( round( 100./np.sqrt(nTagLLP1_pred_r_q1q2_2T),2) if nTagLLP1_pred_r_q1q2_2T>0 else 0 ) + "%",
        ...
        row_new = [
            b,
            str(t_nTagLLP1_r_q1q2_1T) + " +- " + str( round( t_nTagLLP1_r_q1q2_1T_unc,2) ) + "%",
            str(round(nTagLLP1_pred_r_q1q2_1T_new,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP1_pred_r_q1q2_1T_new),2) ) + "%",
            str( round(t_diff_nTagLLP1_r_q1q2_1T_new, 2) ) + "%",

            str(t_nTagLLP1_r_q1q2_2T) + " +- " + str( round( t_nTagLLP1_r_q1q2_2T_unc,2) ) + "%",
            str(round(nTagLLP1_pred_r_q1q2_2T_new,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP1_pred_r_q1q2_2T_new),2) ) + "%",
            str( round(t_diff_nTagLLP1_r_q1q2_2T_new, 2) ) + "%",

        ]


        ##table_r2_LLP1.add_row(row)
        table_r2_LLP1.add_row(row_new)
        ...
        ##digitize is the right function to use
        #r_LLP0_r_idx = np.digitize(r_LLP0_r,bins_r_resolved)-1
        #r_LLP0_z_idx = np.digitize(r_LLP0_z,bins_z_resolved)-1
        #r_LLP0_pt1_idx = np.digitize(r_LLP0_pt1,bins_pt_resolved_q)-1
        #r_LLP0_eta1_idx = np.digitize(r_LLP0_eta1,bins_eta_resolved_q)-1
        #r_LLP0_pt2_idx = np.digitize(r_LLP0_pt2,bins_pt_resolved_q)-1
        #r_LLP0_eta2_idx = np.digitize(r_LLP0_eta2,bins_eta_resolved_q)-1

        #r_LLP1_r_idx = np.digitize(r_LLP1_r,bins_r_resolved)-1
        #r_LLP1_z_idx = np.digitize(r_LLP1_z,bins_z_resolved)-1
        #r_LLP1_pt1_idx = np.digitize(r_LLP1_pt1,bins_pt_resolved_q)-1
        #r_LLP1_eta1_idx = np.digitize(r_LLP1_eta1,bins_eta_resolved_q)-1
        #r_LLP1_pt2_idx = np.digitize(r_LLP1_pt2,bins_pt_resolved_q)-1
        #r_LLP1_eta2_idx = np.digitize(r_LLP1_eta2,bins_eta_resolved_q)-1
        ...
        t_nTagLLP0_r_q1_presel = nTagLLP0_r_q1_presel[nTagLLP0_r_q1_presel>0].astype(bool).sum()
        t_nTagLLP0_r_q1_unc_presel = 100./np.sqrt(nTagLLP0_r_q1_presel[nTagLLP0_r_q1_presel>0].astype(bool).sum()) if np.sqrt(nTagLLP0_r_q1_presel[nTagLLP0_r_q1_presel>0].astype(bool).sum()) >0 else 0
        t_diff_nTagLLP0_r_q1_presel = 100*( (nTagLLP0_pred_r_q1_presel-nTagLLP0_r_q1_presel[nTagLLP0_r_q1_presel>0].astype(bool).sum())/ ( nTagLLP0_r_q1_presel[nTagLLP0_r_q1_presel>0].astype(bool).sum() ) )

        t_nTagLLP0_r_q2_presel = nTagLLP0_r_q2_presel[nTagLLP0_r_q2_presel>0].astype(bool).sum()
        t_nTagLLP0_r_q2_unc_presel = 100./np.sqrt(nTagLLP0_r_q2_presel[nTagLLP0_r_q2_presel>0].astype(bool).sum()) if np.sqrt(nTagLLP0_r_q2_presel[nTagLLP0_r_q2_presel>0].astype(bool).sum())>0 else 0
        t_diff_nTagLLP0_r_q2_presel = 100*( (nTagLLP0_pred_r_q2_presel-nTagLLP0_r_q2_presel[nTagLLP0_r_q2_presel>0].astype(bool).sum())/ ( nTagLLP0_r_q2_presel[nTagLLP0_r_q2_presel>0].astype(bool).sum() ) )

        t_nTagLLP0_r_q1q2_1T_presel = nTagLLP0_r_q1q2_presel[ nTagLLP0_r_q1q2_presel==1 ].astype(bool).sum()
        t_nTagLLP0_r_q1q2_1T_unc_presel = 100./np.sqrt(nTagLLP0_r_q1q2_presel[ nTagLLP0_r_q1q2_presel==1 ].astype(bool).sum()) if np.sqrt(nTagLLP0_r_q1q2_presel[ nTagLLP0_r_q1q2_presel==1 ].astype(bool).sum())>0 else 0
        t_diff_nTagLLP0_r_q1q2_1T_presel = 100*( ( (nTagLLP0_pred_r_q1q2_1T_presel)- nTagLLP0_r_q1q2_presel[ nTagLLP0_r_q1q2_presel==1 ].astype(bool).sum() )/ ( nTagLLP0_r_q1q2_presel[ nTagLLP0_r_q1q2_presel==1 ].astype(bool).sum() ) )
        #new: when q1q2, use higgs
        #only pred and diff change!
        t_diff_nTagLLP0_r_q1q2_1T_presel_new = 100*( ( (nTagLLP0_pred_r_q1q2_1T_presel_new)- nTagLLP0_r_q1q2_presel[ nTagLLP0_r_q1q2_presel==1 ].astype(bool).sum() )/ ( nTagLLP0_r_q1q2_presel[ nTagLLP0_r_q1q2_presel==1 ].astype(bool).sum() ) )


        t_nTagLLP0_r_q1q2_2T_presel = nTagLLP0_r_q1q2_presel[ nTagLLP0_r_q1q2_presel>1 ].astype(bool).sum()
        t_nTagLLP0_r_q1q2_2T_unc_presel = 100./np.sqrt(nTagLLP0_r_q1q2_presel[ nTagLLP0_r_q1q2_presel>1 ].astype(bool).sum()) if np.sqrt(nTagLLP0_r_q1q2_presel[ nTagLLP0_r_q1q2_presel>1 ].astype(bool).sum())>0 else 0
        t_diff_nTagLLP0_r_q1q2_2T_presel = 100*( ( (nTagLLP0_pred_r_q1q2_2T_presel)- nTagLLP0_r_q1q2_presel[ nTagLLP0_r_q1q2_presel>1 ].astype(bool).sum() )/ ( nTagLLP0_r_q1q2_presel[ nTagLLP0_r_q1q2_presel>1 ].astype(bool).sum() ) )
        #new: when q1q2, use higgs
        #only pred and diff change!
        t_diff_nTagLLP0_r_q1q2_2T_presel_new = 100*( ( (nTagLLP0_pred_r_q1q2_2T_presel_new)- nTagLLP0_r_q1q2_presel[ nTagLLP0_r_q1q2_presel>1 ].astype(bool).sum() )/ ( nTagLLP0_r_q1q2_presel[ nTagLLP0_r_q1q2_presel>1 ].astype(bool).sum() ) )
        ...
        row_presel = [
            b,
            str(t_nTagLLP0_r_q1_presel) + " +- " + str( round( t_nTagLLP0_r_q1_unc_presel,2) ) + "%",
            str(round(nTagLLP0_pred_r_q1_presel,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q1_presel),2) ) + "%",
            str( round(t_diff_nTagLLP0_r_q1_presel, 2) ) + "%",

            str(t_nTagLLP0_r_q2_presel) + " +- " + str( round( t_nTagLLP0_r_q2_unc_presel,2) ) + "%",
            str(round(nTagLLP0_pred_r_q2_presel,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q2_presel),2) ) + "%",
            str( round(t_diff_nTagLLP0_r_q2_presel, 2) ) + "%",

        ]

        table_r1_LLP0_presel.add_row(row_presel)



        row_presel = [
            b,
            str(t_nTagLLP0_r_q1q2_1T_presel) + " +- " + str( round( t_nTagLLP0_r_q1q2_1T_unc_presel,2) ) + "%",
            str(round(nTagLLP0_pred_r_q1q2_1T_presel,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q1q2_1T_presel),2) ) + "%",
            str( round(t_diff_nTagLLP0_r_q1q2_1T_presel, 2) ) + "%",

            str(t_nTagLLP0_r_q1q2_2T_presel) + " +- " + str( round( t_nTagLLP0_r_q1q2_2T_unc_presel,2) ) + "%",
            str(round(nTagLLP0_pred_r_q1q2_2T_presel,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q1q2_2T_presel),2) ) + "%",
            str( round(t_diff_nTagLLP0_r_q1q2_2T_presel, 2) ) + "%",
        ...
        row_presel_new = [
            b,
            str(t_nTagLLP0_r_q1q2_1T_presel) + " +- " + str( round( t_nTagLLP0_r_q1q2_1T_unc_presel,2) ) + "%",
            str(round(nTagLLP0_pred_r_q1q2_1T_presel_new,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q1q2_1T_presel_new),2) ) + "%",
            str( round(t_diff_nTagLLP0_r_q1q2_1T_presel_new, 2) ) + "%",

            str(t_nTagLLP0_r_q1q2_2T_presel) + " +- " + str( round( t_nTagLLP0_r_q1q2_2T_unc_presel,2) ) + "%",
            str(round(nTagLLP0_pred_r_q1q2_2T_presel_new,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP0_pred_r_q1q2_2T_presel_new),2) ) + "%",
            str( round(t_diff_nTagLLP0_r_q1q2_2T_presel_new, 2) ) + "%",


        ]

        ##table_r2_LLP0_presel.add_row(row_presel)
        table_r2_LLP0_presel.add_row(row_presel_new)


        t_nTagLLP1_r_q1_presel = nTagLLP1_r_q1_presel[nTagLLP1_r_q1_presel>0].astype(bool).sum()
        t_nTagLLP1_r_q1_unc_presel = 100./np.sqrt(nTagLLP1_r_q1_presel[nTagLLP1_r_q1_presel>0].astype(bool).sum()) if np.sqrt(nTagLLP1_r_q1_presel[nTagLLP1_r_q1_presel>0].astype(bool).sum())>0 else 0
        t_diff_nTagLLP1_r_q1_presel = 100*( (nTagLLP1_pred_r_q1_presel-nTagLLP1_r_q1_presel[nTagLLP1_r_q1_presel>0].astype(bool).sum())/ ( nTagLLP1_r_q1_presel[nTagLLP1_r_q1_presel>0].astype(bool).sum() ) )

        t_nTagLLP1_r_q2_presel = nTagLLP1_r_q2_presel[nTagLLP1_r_q2_presel>0].astype(bool).sum()
        t_nTagLLP1_r_q2_unc_presel = 100./np.sqrt(nTagLLP1_r_q2_presel[nTagLLP1_r_q2_presel>0].astype(bool).sum()) if np.sqrt(nTagLLP1_r_q2_presel[nTagLLP1_r_q2_presel>0].astype(bool).sum())>0 else 0
        t_diff_nTagLLP1_r_q2_presel = 100*( (nTagLLP1_pred_r_q2_presel-nTagLLP1_r_q2_presel[nTagLLP1_r_q2_presel>0].astype(bool).sum())/ ( nTagLLP1_r_q2_presel[nTagLLP1_r_q2_presel>0].astype(bool).sum() ) )

        t_nTagLLP1_r_q1q2_1T_presel = nTagLLP1_r_q1q2_presel[ nTagLLP1_r_q1q2_presel==1 ].astype(bool).sum()
        t_nTagLLP1_r_q1q2_1T_unc_presel = 100./np.sqrt(nTagLLP1_r_q1q2_presel[ nTagLLP1_r_q1q2_presel==1 ].astype(bool).sum()) if np.sqrt(nTagLLP1_r_q1q2_presel[ nTagLLP1_r_q1q2_presel==1 ].astype(bool).sum())>0 else 0
        t_diff_nTagLLP1_r_q1q2_1T_presel = 100*( ( (nTagLLP1_pred_r_q1q2_1T_presel)- nTagLLP1_r_q1q2_presel[ nTagLLP1_r_q1q2_presel==1 ].astype(bool).sum() )/ ( nTagLLP1_r_q1q2_presel[ nTagLLP1_r_q1q2_presel==1 ].astype(bool).sum() ) )
        #new: when q1q2, use higgs
        #only pred and diff change!
        t_diff_nTagLLP1_r_q1q2_1T_presel_new = 100*( ( (nTagLLP1_pred_r_q1q2_1T_presel_new)- nTagLLP1_r_q1q2_presel[ nTagLLP1_r_q1q2_presel==1 ].astype(bool).sum() )/ ( nTagLLP1_r_q1q2_presel[ nTagLLP1_r_q1q2_presel==1 ].astype(bool).sum() ) )


        t_nTagLLP1_r_q1q2_2T_presel = nTagLLP1_r_q1q2_presel[ nTagLLP1_r_q1q2_presel>1 ].astype(bool).sum()
        t_nTagLLP1_r_q1q2_2T_unc_presel = 100./np.sqrt(nTagLLP1_r_q1q2_presel[ nTagLLP1_r_q1q2_presel>1 ].astype(bool).sum()) if np.sqrt(nTagLLP1_r_q1q2_presel[ nTagLLP1_r_q1q2_presel>1 ].astype(bool).sum())>0 else 0
        t_diff_nTagLLP1_r_q1q2_2T_presel = 100*( ( (nTagLLP1_pred_r_q1q2_2T_presel)- nTagLLP1_r_q1q2_presel[ nTagLLP1_r_q1q2_presel>1 ].astype(bool).sum() )/ ( nTagLLP1_r_q1q2_presel[ nTagLLP1_r_q1q2_presel>1 ].astype(bool).sum() ) )
        #new: when q1q2, use higgs
        #only pred and diff change!
        t_diff_nTagLLP1_r_q1q2_2T_presel_new = 100*( ( (nTagLLP1_pred_r_q1q2_2T_presel_new)- nTagLLP1_r_q1q2_presel[ nTagLLP1_r_q1q2_presel>1 ].astype(bool).sum() )/ ( nTagLLP1_r_q1q2_presel[ nTagLLP1_r_q1q2_presel>1 ].astype(bool).sum() ) )
        ...
        row_presel = [
            b,
            str(t_nTagLLP1_r_q1_presel) + " +- " + str( round( t_nTagLLP1_r_q1_unc_presel,2) ) + "%",
            str(round(nTagLLP1_pred_r_q1_presel,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP1_pred_r_q1_presel),2) ) + "%",
            str( round(t_diff_nTagLLP1_r_q1_presel, 2) ) + "%",

            str(t_nTagLLP1_r_q2_presel) + " +- " + str( round( t_nTagLLP1_r_q2_unc_presel,2) ) + "%",
            str(round(nTagLLP1_pred_r_q2_presel,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP1_pred_r_q2_presel),2) ) + "%",
            str( round(t_diff_nTagLLP1_r_q2_presel, 2) ) + "%",

        ]

        table_r1_LLP1_presel.add_row(row_presel)


        row_presel = [
            b,
            str(t_nTagLLP1_r_q1q2_1T_presel) + " +- " + str( round( t_nTagLLP1_r_q1q2_1T_unc_presel,2) ) + "%",
            str(round(nTagLLP1_pred_r_q1q2_1T_presel,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP1_pred_r_q1q2_1T_presel),2) ) + "%",
            str( round(t_diff_nTagLLP1_r_q1q2_1T_presel, 2) ) + "%",

            str(t_nTagLLP1_r_q1q2_2T_presel) + " +- " + str( round( t_nTagLLP1_r_q1q2_2T_unc_presel,2) ) + "%",
            str(round(nTagLLP1_pred_r_q1q2_2T_presel,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP1_pred_r_q1q2_2T_presel),2) ) + "%",
            str( round(t_diff_nTagLLP1_r_q1q2_2T_presel, 2) ) + "%",
        ...
        row_presel_new = [
            b,
            str(t_nTagLLP1_r_q1q2_1T_presel) + " +- " + str( round( t_nTagLLP1_r_q1q2_1T_unc_presel,2) ) + "%",
            str(round(nTagLLP1_pred_r_q1q2_1T_presel_new,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP1_pred_r_q1q2_1T_presel_new),2) ) + "%",
            str( round(t_diff_nTagLLP1_r_q1q2_1T_presel_new, 2) ) + "%",

            str(t_nTagLLP1_r_q1q2_2T_presel) + " +- " + str( round( t_nTagLLP1_r_q1q2_2T_unc_presel,2) ) + "%",
            str(round(nTagLLP1_pred_r_q1q2_2T_presel_new,2) ),# + " +- " + str( round( 100./np.sqrt(nTagLLP1_pred_r_q1q2_2T_presel_new),2) ) + "%",
            str( round(t_diff_nTagLLP1_r_q1q2_2T_presel_new, 2) ) + "%",


        ]

        ##table_r2_LLP1_presel.add_row(row_presel)
        table_r2_LLP1_presel.add_row(row_presel_new)
    ...
    print(table_r2_LLP0)
    exit()
    print "  LLP1 no unified option "
    print(table_r1_LLP1)
    print(table_r2_LLP1)
    #print "   with preselection "
    #print(table_r1_LLP0_presel)
    #print(table_r2_LLP0_presel)
    #print(table_r1_LLP1_presel)
    #print(table_r2_LLP1_presel)

'''


##updated
def use_maps_updated(sign, skip_odd=False, cross_maps=""):
    ...
    for b in sign:
        ...
        m_LLP0_eta_idx_H = np.digitize(m_LLP0_eta,bins_eta_merged_H)-1
        ...
        m_LLP1_eta_idx_Z = np.digitize(m_LLP1_eta,bins_eta_merged_Z)-1
        ...
        for it in range(len(m_LLP0_r_idx)):
            pred = maps['merged']['eff'][ m_LLP0_r_idx[it] ][  m_LLP0_z_idx[it] ][  m_LLP0_pt_idx[it] ][  m_LLP0_eta_idx[it] ]
            nTagLLP0_pred_m_unified+= pred

            pred_presel = maps['merged']['eff_LLP0_presel'][ m_LLP0_r_idx[it] ][  m_LLP0_z_idx[it] ][  m_LLP0_pt_idx[it] ][  m_LLP0_eta_idx[it] ]
            nTagLLP0_pred_m_presel+= pred_presel

        for it in range(len(m_LLP0_r_idx_H)):
            pred_H = maps_H['merged']['eff'][ m_LLP0_r_idx_H[it] ][  m_LLP0_z_idx_H[it] ][  m_LLP0_pt_idx_H[it] ][  m_LLP0_eta_idx_H[it] ]
            nTagLLP0_pred_m_H+= pred_H

        for it in range(len(m_LLP1_r_idx)):
            pred = maps['merged']['eff'][ m_LLP1_r_idx[it] ][  m_LLP1_z_idx[it] ][  m_LLP1_pt_idx[it] ][  m_LLP1_eta_idx[it] ]
            nTagLLP1_pred_m_unified+= pred

            pred_presel = maps['merged']['eff_LLP1_presel'][ m_LLP1_r_idx[it] ][  m_LLP1_z_idx[it] ][  m_LLP1_pt_idx[it] ][  m_LLP1_eta_idx[it] ]
            nTagLLP1_pred_m_presel+= pred_presel

        for it in range(len(m_LLP1_r_idx_Z)):
            pred_Z = maps_Z['merged']['eff'][ m_LLP1_r_idx_Z[it] ][  m_LLP1_z_idx_Z[it] ][  m_LLP1_pt_idx_Z[it] ][  m_LLP1_eta_idx_Z[it] ]
            nTagLLP1_pred_m_Z+= pred_Z
        ...
        row_HZ = [
            b,
            str(nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum()) + " +- " + str( round( (100*np.sqrt(nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum()) / nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum() ),2) ) + "%",
            str( round(nTagLLP0_pred_m_H,2) ),
            str( round( 100*( (nTagLLP0_pred_m_H-nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum())/ ( nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum() ) ), 2) ) + "%",
        
            str(nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum()) + " +- " + str( round( (100*np.sqrt(nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum()) / nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum() ),2) ) + "%",
            str( round(nTagLLP1_pred_m_Z,2) ),
            str( round( 100*( (nTagLLP1_pred_m_Z-nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum())/ ( nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum() ) ), 2) ) + "%",


            str( round(nTagLLP0_pred_m_H+nTagLLP1_pred_m_Z,2) ),
            str( round( 100*( (nTagLLP0_pred_m_H + nTagLLP1_pred_m_Z - nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum() - nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum())/ ( nTagLLP0_m[nTagLLP0_m>0].astype(bool).sum() + nTagLLP1_m[nTagLLP1_m>0].astype(bool).sum() ) ), 2) ) + "%",
        ]
        table_m.add_row(row_HZ)


        #TODO these must use unified too!
        row_presel = [
            b,
            str(nTagLLP0_m_presel[nTagLLP0_m_presel>0].astype(bool).sum()) + " +- " + str( round( (100*np.sqrt(nTagLLP0_m_presel[nTagLLP0_m_presel>0].astype(bool).sum()) / nTagLLP0_m_presel[nTagLLP0_m_presel>0].astype(bool).sum() ),2) ) + "%",
            str( round(nTagLLP0_pred_m_presel,2) ),# + " +- " + str( round( (100*np.sqrt(nTagLLP0_pred_m_presel) / nTagLLP0_pred_m_presel),2) ) + "%",
...
use_maps_updated(sign,skip_odd=True)#,cross_maps="HH")
exit()
