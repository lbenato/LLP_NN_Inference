...
#include "NNInferenceCMSSW/LLP_NN_Inference/plugins/Objects_v5_genmet.h"
...
bool dnn_sorter(const JetType& x, const JetType& y) { return x.sigprob > y.sigprob; }
...
int main(int argc, char **argv) {
    ...
    //Flags for SR/CR
    bool doGen(false);
    if(strcmp(argv[7], "doGen")==0) doGen=true;
    bool doSR(false);
    ...
    if(doGen) std::cout << "Gen level studies" << std::endl;
    ...
    */
    ...
    std::vector<JetType>    skimmedJetsDNN;
    ...
    float inv_mass_high_DNN(-1.);
    float inv_mass_tag_DNN(-1.);
    float inv_mass_reco_higgs_0(-1.);
    float inv_mass_reco_higgs_1(-1.);
    int n_j_higgs_0(0);
    int n_j_higgs_1(0);
    float perc_met_held_by_gravitinos(-1.);
    //dR gravitino and higgs
    //dR Higgs
    //dR gravitino and gen met
    //dphi gravitinos and reco met
    //build combination of gravitino --> pt compared to met, delta phi and delta R
    //check on prompt or when both LLPs are in the detector
    //Invariant masses of gen matched jets
    ...
    outputTree->Branch("inv_mass_reco_higgs_0", &inv_mass_reco_higgs_0, "inv_mass_reco_higgs_0/F");
    outputTree->Branch("inv_mass_reco_higgs_1", &inv_mass_reco_higgs_1, "inv_mass_reco_higgs_1/F");
    outputTree->Branch("inv_mass_high_DNN", &inv_mass_high_DNN, "inv_mass_high_DNN/F");
    outputTree->Branch("inv_mass_tag_DNN", &inv_mass_tag_DNN, "inv_mass_tag_DNN/F");
    outputTree->Branch("n_j_higgs_0", &n_j_higgs_0, "n_j_higgs_0/I");
    outputTree->Branch("n_j_higgs_1", &n_j_higgs_1, "n_j_higgs_1/I");
	...
	inv_mass_reco_higgs_0 = -1.;
	inv_mass_reco_higgs_1 = -1.;
	inv_mass_high_DNN = -1.;
	inv_mass_tag_DNN = -1.;
	n_j_higgs_0 = 0;
	n_j_higgs_1 = 0;
	...
	//MET filters always fulfilled
        if(not(doGen) and !Flag2_globalSuperTightHalo2016Filter) continue;
        if(not(doGen) and !Flag2_EcalDeadCellTriggerPrimitiveFilter) continue;
	if(not(doGen) and !Flag2_HBHENoiseFilter) continue;
        if(not(doGen) and !Flag2_HBHEIsoNoiseFilter) continue;
        if(not(doGen) and !Flag2_ecalBadCalibFilter) continue;
        if(not(doGen) and !Flag2_eeBadScFilter) continue;
        if(not(doGen) and !Flag2_BadPFMuonFilter) continue;
	...
	//Gen-level studies
	//std::cout << "~~~~" << std::endl;
	//if(isMC and not(GenLLPs->size()==2 and GenHiggs->size()==2 and GenGravitinos->size()==2))
	//{
	//std::cout << "nGenLLPs " << GenLLPs->size() << std::endl;
	//std::cout << "nGenHiggs " << GenHiggs->size() << std::endl;
	//std::cout << "nGenGravitinoss " << GenGravitinos->size() << std::endl;
	//}
	...
	if(isMC)
	  {
	    ...
	    TLorentzVector RecoHiggs0;
	    TLorentzVector RecoHiggs1;
	    for (unsigned int j=0; j<skimmedJets.size(); j++)
	      {

		skimmedJetsDNN.push_back(skimmedJets.at(j));
		//Higgs 0
		if(GenHiggs->at(0).travelRadiusLLP == skimmedJets.at(j).radiusLLP)
		  {
		    TLorentzVector j_vec;
		    j_vec.SetPtEtaPhiM(skimmedJets.at(j).pt,skimmedJets.at(j).eta,skimmedJets.at(j).phi,skimmedJets.at(j).mass);
		    RecoHiggs0 += j_vec;
		    n_j_higgs_0++;
		  }
		//Higgs 1
		if(GenHiggs->size()==2)
		  {
		    if(GenHiggs->at(1).travelRadiusLLP == skimmedJets.at(j).radiusLLP)
		      {
			TLorentzVector j_vec;
			j_vec.SetPtEtaPhiM(skimmedJets.at(j).pt,skimmedJets.at(j).eta,skimmedJets.at(j).phi,skimmedJets.at(j).mass);
			RecoHiggs1 += j_vec;
	    ...
	    inv_mass_reco_higgs_0 = RecoHiggs0.M();
	    inv_mass_reco_higgs_1 = RecoHiggs1.M();
	  }
	std::sort(skimmedJetsDNN.begin(), skimmedJetsDNN.end(), dnn_sorter);
	if(skimmedJetsDNN.size()>1)
	  {
	    TLorentzVector j_0;
	    j_0.SetPtEtaPhiM(skimmedJetsDNN.at(0).pt,skimmedJetsDNN.at(0).eta,skimmedJetsDNN.at(0).phi,skimmedJets.at(0).mass);
	    TLorentzVector j_1;
	    j_1.SetPtEtaPhiM(skimmedJetsDNN.at(1).pt,skimmedJetsDNN.at(1).eta,skimmedJetsDNN.at(1).phi,skimmedJets.at(1).mass);
	    TLorentzVector jj;
	    jj = j_0+j_1;
	    inv_mass_high_DNN = jj.M();
	    if(skimmedJetsDNN.at(0).sigprob>0.996 and skimmedJetsDNN.at(1).sigprob>0.996) inv_mass_tag_DNN = jj.M();
	  }
