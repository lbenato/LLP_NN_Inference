...
int main(int argc, char **argv) {
    ...
    //New branches
    int   nTagJets_to_LLP0_q0;
    int   nTagJets_to_LLP0_q1;
    int   nTagJets_to_LLP1_q2;
    int   nTagJets_to_LLP1_q3;

    outputTree->Branch("HT_old", &oldHT, "HT_old/F");
    outputTree->Branch("nTagJets_0p996_old", &oldTag, "nTagJets_0p996_old/I");
    outputTree->Branch("nTagJets_to_LLP0_q0", &nTagJets_to_LLP0_q0, "nTagJets_to_LLP0_q0/I");
    outputTree->Branch("nTagJets_to_LLP0_q1", &nTagJets_to_LLP0_q1, "nTagJets_to_LLP0_q1/I");
    outputTree->Branch("nTagJets_to_LLP1_q2", &nTagJets_to_LLP1_q2, "nTagJets_to_LLP1_q2/I");
    outputTree->Branch("nTagJets_to_LLP1_q3", &nTagJets_to_LLP1_q3, "nTagJets_to_LLP1_q3/I");
	...
	nTagJets_0p996 = 0;
	nTagJets_to_LLP0 = 0;
	nTagJets_to_LLP0_q0 = 0;
	nTagJets_to_LLP0_q1 = 0;
	nTagJets_to_LLP1 = 0;
	nTagJets_to_LLP1_q2 = 0;
	nTagJets_to_LLP1_q3 = 0;
	nJets_to_LLP0 = 0;
	nJets_to_LLP1 = 0;
			...
			if (oldJets->at(j).timeRecHitsEB>-1.)
			  {
			    ...
			    if(outputValueAK4>0.996 and oldJets->at(j).muEFrac<0.6 and oldJets->at(j).eleEFrac<0.6 and oldJets->at(j).photonEFrac<0.8 and oldJets->at(j).timeRecHitsEB>-1 and abs(oldJets->at(j).eta)<1.) 
			      {
				nTagJets_0p996++;
				//Try to solve the double-match ambiguity: don't do the calo corr part
				//if(GenLLPs->at(0).travelRadius==oldJets->at(j).radiusLLP or GenLLPs->at(0).travelRadius==oldJets->at(j).radiusLLPCaloCorr) 
				if(GenLLPs->at(0).travelRadius==oldJets->at(j).radiusLLP) 
				  {
				    nTagJets_to_LLP0++;
				    //Assign to single quarks
				    float dR_q0 = fabs(reco::deltaR(oldJets->at(j).eta,oldJets->at(j).phi,GenBquarks->at(0).eta,GenBquarks->at(0).phi));
				    float dR_q1 = fabs(reco::deltaR(oldJets->at(j).eta,oldJets->at(j).phi,GenBquarks->at(1).eta,GenBquarks->at(1).phi));

				    //float dR_corrCalo_q0 = fabs(reco::deltaR(oldJets->at(j).eta,oldJets->at(j).phi,GenBquarks->at(0).corrCaloEta,GenBquarks->at(0).corrCaloPhi));
				    //float dR_corrCalo_q1 = fabs(reco::deltaR(oldJets->at(j).eta,oldJets->at(j).phi,GenBquarks->at(1).corrCaloEta,GenBquarks->at(1).corrCaloPhi));

				    //std::cout << "LLP0 matched: " << std::endl;
				    //std::cout << dR_q0 << std::endl;
				    //std::cout << dR_q1 << std::endl;
				    if(dR_q0<dR_q1) nTagJets_to_LLP0_q0++;
				    if(dR_q1<dR_q0) nTagJets_to_LLP0_q1++;
				  }
				//Try to solve the double-match ambiguity: don't do the calo corr part
				//if(GenLLPs->at(1).travelRadius==oldJets->at(j).radiusLLP or GenLLPs->at(1).travelRadius==oldJets->at(j).radiusLLPCaloCorr) 
				if(GenLLPs->at(1).travelRadius==oldJets->at(j).radiusLLP) 
				  {
				    nTagJets_to_LLP1++;
				    //Assign to single quarks
				    float dR_q2 = fabs(reco::deltaR(oldJets->at(j).eta,oldJets->at(j).phi,GenBquarks->at(2).eta,GenBquarks->at(2).phi));
				    float dR_q3 = fabs(reco::deltaR(oldJets->at(j).eta,oldJets->at(j).phi,GenBquarks->at(3).eta,GenBquarks->at(3).phi));
				    //float dR_corrCalo_q2 = fabs(reco::deltaR(oldJets->at(j).eta,oldJets->at(j).phi,GenBquarks->at(2).corrCaloEta,GenBquarks->at(2).corrCaloPhi));
				    //float dR_corrCalo_q3 = fabs(reco::deltaR(oldJets->at(j).eta,oldJets->at(j).phi,GenBquarks->at(3).corrCaloEta,GenBquarks->at(3).corrCaloPhi));
				    //std::cout << "LLP1 matched: " << std::endl;
				    //std::cout << dR_q2 << std::endl;
				    //std::cout << dR_q3 << std::endl;
				    if(dR_q2<dR_q3) nTagJets_to_LLP1_q2++;
				    if(dR_q3<dR_q2) nTagJets_to_LLP1_q3++;
				  }
