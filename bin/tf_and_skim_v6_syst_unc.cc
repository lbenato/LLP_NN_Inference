	...
	if(isMC)
	  {
	    ...
	    PUReWeightDown = pu_down->GetBinContent(pu_down->GetXaxis()->FindBin(MeanNumInteractions));
            if(doSR) TriggerWeight_JER = tr->GetBinContent(tr->GetXaxis()->FindBin(MEt->pt));
            if(doSR) TriggerWeight_JESUp = tr->GetBinContent(tr->GetXaxis()->FindBin(MEt->ptShiftJetEnUp));
            if(doSR) TriggerWeight_JESDown = tr->GetBinContent(tr->GetXaxis()->FindBin(MEt->ptShiftJetEnDown));
            if(doSR) TriggerWeight_JERUp = tr->GetBinContent(tr->GetXaxis()->FindBin(MEt->ptShiftJetResUp));
            if(doSR) TriggerWeight_JERDown = tr->GetBinContent(tr->GetXaxis()->FindBin(MEt->ptShiftJetResDown));
            if(doSR) TriggerWeight_UnclusteredEnUp = tr->GetBinContent(tr->GetXaxis()->FindBin(MEt->ptShiftUnclusteredEnUp));
            if(doSR) TriggerWeight_UnclusteredEnDown = tr->GetBinContent(tr->GetXaxis()->FindBin(MEt->ptShiftUnclusteredEnDown));
	  }
	...
	for (unsigned int j=0; j<Jets->size(); j++)
	  {
	    ...
	    //3. JER up
	    //New! For jet pT, will be included in the baseline pT already
            if(fabs(Jets->at(j).eta)<2.4 and Jets->at(j).pt>30 and MEt->ptShiftJetResUp>200)
              {
	    ...
	    //4. JER down
	    //New! For jet pT, will be included in the baseline pT already
            if(fabs(Jets->at(j).eta)<2.4 and Jets->at(j).pt>30 and MEt->ptShiftJetResDown>200)
              {
