...
#include "NNInferenceCMSSW/LLP_NN_Inference/plugins/Objects_v7.h"
...
//#include "NNInferenceCMSSW/LLP_NN_Inference/plugins/TF1Custom.cxx"
...
//can get fXmin/max as arguments from constructor?
/*
Bool_t ComputeCdfTable(TF1 * f, Option_t * option) {
...
Double_t GetRandomCustom(TF1 * f, TRandom * rng, Option_t * option)
{
  ...
  if (fIntegral.size() == 0) {
    Bool_t ret = f->ComputeCdfTable(option);
...
int main(int argc, char **argv) {
    ...
    //TF1Custom * check = new TF1Custom("fitFunc","pol1(0)+gaus(2)",0,130);
