...
{    
  ...
  for(unsigned int p = 0; p<points.size(); p++) std::cout << p << " " << points.at(p).x << std::endl;
  // constructor
  DBSCAN ds(MINIMUM_POINTS, EPSILON, points);
  // main loop
  std::cout << "run dbscan: " << std::endl;
  ds.run();
  // result of DBSCAN algorithm
  std::cout << "gonna print results: " << std::endl;
...
double distance2_standalone(double x,double y,double z, const double *p) {
...
struct ToMinimize{
  ...
  std::vector<float> yy;
  std::vector<float> zz;
  ToMinimize(std::vector<float> & x, std::vector<float> & y, std::vector<float> & z) : xx(x), yy(y), zz(z)  {}
  ...
  double operator() (const double *par) {
    ...
    for (int i  = 0; i < npoints; ++i) {
      std::cout << "point["<< i<<"]: " << xx[i] << " , " << yy[i] << " , " << zz[i] << std::endl;
      double d = distance2_standalone(xx[i],yy[i],zz[i],par);
...
struct SumDistance2 {
  ...
  double operator() (const double *par) {
    ...
    double sum = 0;
    for (int i  = 0; i < npoints; ++i) {
      double d = distance2_standalone(x[i],y[i],z[i],par);
      sum += d;
    }
    std::cout << "Total Initial distance square = " << sum << std::endl;
    return sum;
...
int main(int argc, char **argv) {
    ...
    TH3F *h3 = new TH3F("h3","h3", 100, -10, 10, 100, -10, 10, 100, -10, 10);
    TF2 *f2 = new TF2("f2","1 ++ x[0] ++ x[1]");
    TF3 *f3 = new TF3("f3","1 ++ x[0] ++ x[1]");
    TGraph2D *l  = new TGraph2D();
    //TPolyLine3D *l = new TPolyLine3D(10);//does not compile
    ...
    //std::vector<TrackType>       *CosmicMuonsPropagated = 0;
    //std::vector<TrackType>       *CosmicMuonsPropagatedOneLeg = 0;
    ...
    //TBranch        *b_CosmicMuonsPropagatedOneLeg = 0;
    //TBranch        *b_CosmicMuonsPropagated = 0;
    ...
    //inputTree->SetBranchAddress("CosmicMuonsPropagated",  &CosmicMuonsPropagated,   &b_CosmicMuonsPropagated); 
    //inputTree->SetBranchAddress("CosmicMuonsPropagatedOneLeg",  &CosmicMuonsPropagatedOneLeg,   &b_CosmicMuonsPropagatedOneLeg); 
    ...
    //outputTree->Branch("CosmicMuonsPropagated", &CosmicMuonsPropagated);
    //outputTree->Branch("CosmicMuonsOnePropagatedLeg", &CosmicMuonsPropagatedOneLeg);
	...
	for (unsigned int j=0; j<Jets->size(); j++)
	  {
		...
		//
		//std::cout << "================" << std::endl;
		//std::cout << "Jet n. " << j << " score " << Jets->at(j).sigprob << std::endl;
		//std::cout << "vx cosmic muon " << CosmicMuonsOneLeg->at(0).vx << std::endl;
		//std::cout << "vy cosmic muon " << CosmicMuonsOneLeg->at(0).vy << std::endl;
		//std::cout << "vz cosmic muon " << CosmicMuonsOneLeg->at(0).vz << std::endl;
		//std::cout << "dxy cosmic muon " << CosmicMuonsOneLeg->at(0).dxy << std::endl;
		//std::cout << "dz cosmic muon " << CosmicMuonsOneLeg->at(0).dz << std::endl;
		//std::cout << "eta " << CosmicMuonsOneLeg->at(0).eta << std::endl;
		//std::cout << "phi " << CosmicMuonsOneLeg->at(0).phi << std::endl;
		//std::cout << "rapidity " << 2*atan( exp(-CosmicMuonsOneLeg->at(0).eta) ) << std::endl;
		//std::cout << "theta " << CosmicMuonsOneLeg->at(0).theta << std::endl;
		//std::cout << "delta R with cosmic muon " << reco::deltaR(Jets->at(j).eta,Jets->at(j).phi,CosmicMuonsOneLeg->at(0).eta, CosmicMuonsOneLeg->at(0).phi) << std::endl;
		//std::cout << "delta phi with cosmic muon " << reco::deltaPhi(Jets->at(j).phi,CosmicMuonsOneLeg->at(0).phi) << std::endl;
		//TLorentzVector J;
		//TLorentzVector M;
		//J.SetPtEtaPhiM(Jets->at(j).pt,Jets->at(j).eta,Jets->at(j).phi,Jets->at(j).mass);
		//M.SetPtEtaPhiM(CosmicMuonsOneLeg->at(0).pt,CosmicMuonsOneLeg->at(0).eta,CosmicMuonsOneLeg->at(0).phi,0);
		//std::cout << "J mag: " << J.Mag()  << std::endl;
		//std::cout << "M mag: " << M.Mag()  << std::endl;
		//std::cout << "position J x: " << J.X() << "; y:" << J.Y() << "; z: " << J.Z()  <<std::endl;
		//std::cout << "momentum J px: " << J.Px() << "; py:" << J.Py() << "; pz: " << J.Pz()  <<std::endl;
		//std::cout << "position J eta: " << J.Eta() << "; phi:" << J.Phi() <<std::endl;
		//std::cout << "position M x: " << M.X() << "; y:" << M.Y() << "; z: " << M.Z()  <<std::endl;
		//std::cout << "position M eta: " << M.Eta() << "; phi:" << M.Phi() <<std::endl;
		//std::cout << "perp M method: " << M.Perp() <<std::endl;
		//TVector3 J3(J.X(),J.Y(),J.Z());
		//TVector3 M3(M.X(),M.Y(),M.Z());
		//std::cout << "J3 mag: " << J3.Mag()  << std::endl;
		//std::cout << "M3 mag: " << M3.Mag()  << std::endl;
		//std::cout << "perp M3 along J3: " << M3.Perp(J3) <<std::endl;
		//std::cout << "perp J3 along M3: " << J3.Perp(M3) <<std::endl;
		//std::cout << "M3 component along J3: " <<  J3*M3/(J3.Mag())  <<std::endl;
		//std::cout << "retry to combine: " << sqrt( pow(M3.Perp(J3),2) + pow(J3*M3/(J3.Mag()),2) ) << std::endl;
		//std::cout << "3D cosinus: " << J3*M3/(J3.Mag()*M3.Mag())  << std::endl;
		//std::cout << "3D angle: " << acos(J3*M3/(J3.Mag()*M3.Mag()))*180./3.14159265  << std::endl;
		//std::cout << "position J 3d x: " << J3.X() << "; y:" << J3.Y() << "; z: " << J3.Z()  <<std::endl;
		//std::cout << "position M 3d x: " << M3.X() << "; y:" << M3.Y() << "; z: " << M3.Z()  <<std::endl;

		//TVector3 J2D(J.X(),J.Y(),0);
		//TVector3 M2D(M.X(),M.Y(),0);
		//std::cout << "2D cosinus: " << J2D*M2D/(J2D.Mag()*M2D.Mag())  << std::endl;
		//std::cout << "2D angle: " << acos(J2D*M2D/(J2D.Mag()*M2D.Mag()))*180./3.14159265  << std::endl;
		//
	...
	for(unsigned int d=0; d<DTSegments->size(); d++)
	  {
	    Point p;
	    //What if we remove points with invalid time?
	...
	//auto max = *std::max_element(ds.m_points.begin(),
	//			     ds.m_points.end(),
	//			     [](const Point& a,const Point& b) { return a.clusterID < b.clusterID; });
	//std::cout << "max cl id: " << max.clusterID << std::endl;
	...
	int n_clusters_(0);
	int n_noise_(0);
	n_noise_ = std::count (labels.begin(), labels.end(), -1);
	std::cout << "noise: " << n_noise_ << std::endl;
	int max = -1;
	if(labels.size()>0) max = *max_element(labels.begin(), labels.end());
	if(max == -1) n_clusters_ = 0;
	else n_clusters_ = max+1;// - 1*int( bool(n_noise_) );
	std::cout << "clusters: " << n_clusters_ << std::endl;
	...
	std::vector<Point> DT_points_for_fit;
	if(n_clusters_>=2 and nCosmicMuonsOneLeg>0 and nCosmicMuons>1)
	  {
	    //It's not possible to mask arrays or so
	    //can probably create a vector of vectors to keep the size dynamic
	    std::vector<std::vector<float>> vec_xx(n_clusters_,std::vector<float>());
	    std::vector<std::vector<float>> vec_yy(n_clusters_,std::vector<float>());
	    std::vector<std::vector<float>> vec_zz(n_clusters_,std::vector<float>());
	    std::vector<std::vector<int>> vec_label(n_clusters_,std::vector<int>());
	    std::vector<std::vector<float>> vec_tt(n_clusters_,std::vector<float>());
	    std::vector<std::vector<int>> vec_ss(n_clusters_,std::vector<int>());
	    //std::cout<< "size of vec_xx: " << vec_xx.size() << std::endl;
	    for(unsigned int l=0;l<labels.size();l++)
	      {
		//std::cout<<"label: " << labels.at(l) << std::endl;
		if(labels.at(l)>-1)
		{
	    ...
	    //I now have n_clusters_ vectors
	    ...
	    for(int k1 = 0; k1<n_clusters_; k1++)
	      {

		for(int k2 = 1; k2<n_clusters_ && k2>k1; k2++)
		  {
		    ...
		    //std::sort(vec_ss.at(k2).begin(), vec_ss.at(k2).end());
		    //int n_k2_s = 0;//std::unique(vec_ss.at(k2).begin(), vec_ss.at(k2).end() - vec_ss.at(k2).begin());

		    std::cout << "Cluster n. " << k1 << std::endl;
		    std::cout << "x elements: mean " << mean_k1_x << std::endl;
		    std::cout << "y elements: mean " << mean_k1_y << std::endl;
		    std::cout << "t elements: mean " << mean_k1_t << std::endl;
		    std::cout << "t elements: std " << std_k1_t << std::endl;
		    //for(unsigned int el = 0; el<vec_ss.at(k1).size(); el++)
		    //{
		    //std::cout << "station["<< el <<"] " << vec_ss.at(k1).at(el) << std::endl;
		    //}

		    std::cout << "Cluster n. " << k2 << std::endl;
		    std::cout << "x elements: mean "<< mean_k2_x << std::endl;
		    std::cout << "y elements: mean "<< mean_k2_y << std::endl;
		    std::cout << "t elements: mean " << mean_k2_t << std::endl;
		    std::cout << "t elements: std " << std_k2_t << std::endl;
		    //for(unsigned int el = 0; el<vec_ss.at(k2).size(); el++)
		    //{
		    //std::cout << "station["<< el <<"] " << vec_ss.at(k2).at(el) << std::endl;
		    //}
		    //for(unsigned int el = 0; el<vec_xx.at(k2).size(); el++)
		    //{
		    //	std::cout << vec_xx.at(k2).at(el) << std::endl;
		    //}
		    
		    //Opposite emispheres condition plus comsic tracks
		    //can move this earlier
		    if(   (mean_k1_x*mean_k2_x<0 and mean_k1_y*mean_k2_y<0) or (mean_k1_x*mean_k2_x<0 and (abs(mean_k1_y)<0.2 || abs(mean_k2_y)<0.2)) )
		      {
	    ...
	    //if(1>0)
	      {
		std::cout << " ~ ~ ~ ~ ~~ ~ ~ ~~ "  << std::endl;
		std::cout << "Habemus cosmics! Pair " << ch_k1 << " ; " << ch_k2 << std::endl;
		std::cout << "Mean time: " << mean_time_ch_k1 << " ; " << mean_time_ch_k2 << std::endl;
		std::cout << "N. stations with hits: " << n_s_ch_k1 << " ; " << n_s_ch_k2 << std::endl;
		std::cout << "EventNumber " << EventNumber << "\tLumiNumber " << LumiNumber << std::endl;
		//std::cout << RunNumber << " & " << avg(vec_xx.at(ch_k1)) << " & " << avg(vec_xx.at(ch_k2))  << " & " << avg(vec_yy.at(ch_k1)) << " & " << avg(vec_yy.at(ch_k2)) << " & " << mean_time_ch_k1 << " & " <<  mean_time_ch_k2 <<  " & " << std_time_ch_k1 << " & " << std_time_ch_k2 << " & " << n_s_ch_k1 << " & " << n_s_ch_k2 << "\\\ " << std::endl;
		...
		std::cout<<"small1 " << smallness1 <<std::endl;
		std::cout<<"small2 " << smallness2 <<std::endl;
		smallness = (smallness1 || smallness2);
		std::cout<<"smallness " << smallness <<std::endl;
		...
		if(isMC)
		  {
		    ...
		    //printf("%d & %5.2lf & %5.2lf & %5.2lf & %5.2lf & %5.2lf& %5.2lf\n",
		    //   EventNumber,
		    //	   avg(vec_xx.at(ch_k1)),
		    //	   avg(vec_xx.at(ch_k2)),
		    //	   avg(vec_yy.at(ch_k1)),
		    ...
		    std::cout << EventNumber << " & " << avg(vec_xx.at(ch_k1)) << " & " << avg(vec_xx.at(ch_k2)) << " & " << (avg(vec_xx.at(ch_k1))*avg(vec_xx.at(ch_k2))<0)  << " & " << avg(vec_yy.at(ch_k1)) << " & " << avg(vec_yy.at(ch_k2)) << " & " << (avg(vec_yy.at(ch_k1))*avg(vec_yy.at(ch_k2))<0)  << " & " << smallness << "\\\\ " << std::endl;

		    std::cout << EventNumber << " & " << mean_time_ch_k1 << " & " << mean_time_ch_k2 << " & " << (mean_time_ch_k1*mean_time_ch_k2<0) << " & " << std_time_ch_k1 << " & " << std_time_ch_k2 << " & " << n_s_ch_k1 << " & " << n_s_ch_k2 << "\\\\ " << std::endl;
		...
		else
		  {
		    ...
		    std::cout << RunNumber << " & " << avg(vec_xx.at(ch_k1)) << " & " << avg(vec_xx.at(ch_k2)) << " & " << (avg(vec_xx.at(ch_k1))*avg(vec_xx.at(ch_k2))<0)  << " & " << avg(vec_yy.at(ch_k1)) << " & " << avg(vec_yy.at(ch_k2)) << " & " << (avg(vec_yy.at(ch_k1))*avg(vec_yy.at(ch_k2))<0)  << " & " << smallness << "\\\\ " << std::endl;

		    std::cout << RunNumber << " & " << mean_time_ch_k1 << " & " << mean_time_ch_k2 << " & " << (mean_time_ch_k1*mean_time_ch_k2<0) << " & " << std_time_ch_k1 << " & " << std_time_ch_k2 << " & " << n_s_ch_k1 << " & " << n_s_ch_k2 << "\\\\ " << std::endl;
		  }
		printResults(ds.m_points, ds.getTotalPointSize()); 
		//Print cosmic muons
		/*
		for(unsigned int it_c=0; it_c<CosmicMuons->size(); it_c++)
		  {
		    std::cout<< "CosmicMuon["<<it_c<<"]: d0 " << CosmicMuons->at(it_c).d0/100. << " , dxy " << CosmicMuons->at(it_c).dxy/100. << " , dz " << CosmicMuons->at(it_c).dz/100. << " , eta " << CosmicMuons->at(it_c).eta << " , phi: " << CosmicMuons->at(it_c).phi << " , pt " << CosmicMuons->at(it_c).pt << std::endl;
		  }
		for(unsigned int it_c=0; it_c<CosmicMuonsOneLeg->size(); it_c++)
		  {
		    std::cout<< "CosmicMuonOneLeg["<<it_c<<"]: d0 " << CosmicMuonsOneLeg->at(it_c).d0/100. << " , dxy " << CosmicMuonsOneLeg->at(it_c).dxy/100. << " , dz " << CosmicMuonsOneLeg->at(it_c).dz/100. << " , eta " << CosmicMuonsOneLeg->at(it_c).eta << " , phi: " << CosmicMuonsOneLeg->at(it_c).phi << " , pt " << CosmicMuonsOneLeg->at(it_c).pt << std::endl;
		  }
		std::cout << "ECAL tag pos: x " << mean_ECAL_tag_x << ", y  " << mean_ECAL_tag_y << ", z " << mean_ECAL_tag_z << " ; radius " << sqrt(pow(mean_ECAL_tag_x,2)+pow(mean_ECAL_tag_y,2))  << std::endl;
		*/
		if(n_s_ch_k1>=3 and n_s_ch_k2>=3 and (mean_time_ch_k1*mean_time_ch_k2)<0)
		  {
		    isCosmic = true;
		  }

		
		//Here fill DT_points_for_fit...
		double p0[4] = {1,1,1,1};//starting values of the fit
		std::vector<float> DT_fit_xx;
		std::vector<float> DT_fit_yy;
		std::vector<float> DT_fit_zz;
		
		//example: p[0] = 1, p[1] = 2, p[2] = 1, p[3] = 2,
		//Let's first try with one set of points only
		//DT_fit_xx = vec_xx.at(ch_k1);
		//DT_fit_yy = vec_yy.at(ch_k1);
		//DT_fit_zz = vec_zz.at(ch_k1);
		
		//double ptrue[4] = {1,2,1,2};
		//double ptrue[4] = {-545780,518051,-412665,391701};
		//double ptrue[4] = {61.5499,24.0115,41.602,16.2287};
		//double t0 = -5;//0;
		//double x0, y0, z0;
		//line(t0,ptrue,x0,y0,z0);
		
		//double t1 = 5;//1;
		//double x1, y1, z1;
		//line(t1,ptrue,x1,y1,z1);

		//DT_fit_xx.push_back(x0);
		//DT_fit_xx.push_back(x1);
		//DT_fit_yy.push_back(y0);
		//DT_fit_yy.push_back(y1);
		//DT_fit_zz.push_back(z0);
		//DT_fit_zz.push_back(z1);

		//DT_fit_xx = {10.81466002,   2.923139,    -4.96838202, -12.85990304, -20.75142406};
		//DT_fit_yy = {9.87428741,   4.01700108,  -1.84028525,  -7.69757157, -13.5548579};
		//DT_fit_zz = {-5.,  -2.5,  0.,   2.5,  5.};

		//DT_fit_xx.push_back(2.36);
		//DT_fit_xx.push_back(3.08);
		//DT_fit_yy.push_back(3.61);
		//DT_fit_yy.push_back(4.14);
		//DT_fit_zz.push_back(-2.42);
		//DT_fit_zz.push_back(-2.70);
		...
		//std::cout << "something is screwed up, try instead only one vector?" << std::endl;
		//for(unsigned int np=0; np<DT_fit_xx.size(); np++)
		//for(unsigned int np=0; np<2; np++)
		//  {
		//    double xl,yl,zl = 0;
		//    double tl = gRandom->Uniform(-10,10);
		//    line(tl,p0,xl,yl,zl);
		//    std::cout << "What goes in graph?" << std::endl;
		//    //std::cout<< float(DT_fit_xx.at(np)) << " " << float(DT_fit_yy.at(np)) << " " << float(DT_fit_zz.at(np)) << std::endl;
		//    //std::cout<< double(DT_fit_xx.at(np)) << " " << double(DT_fit_yy.at(np)) << " " << double(DT_fit_zz.at(np)) << std::endl;
		//    //gr->SetPoint(np,double(DT_fit_xx.at(np)),double(DT_fit_yy.at(np)),double(DT_fit_zz.at(np)));
		//    // just for testing purposes: throw values around the line
		//    double err = 1;
		//    xl = DT_fit_xx.at(np);
		//    //xl += gRandom->Gaus(0,err);
		//    yl = DT_fit_yy.at(np);
		//    //yl += gRandom->Gaus(0,err);
		//    zl = DT_fit_zz.at(np);
		//    //zl += gRandom->Gaus(0,err);
		//    std::cout << "Goes in line fit: "<< xl << " " << yl << " " << zl << std::endl;
		//    gr->SetPoint(np,xl,yl,zl);
		// }
		...
		MatrixXf A = MatrixXf::Random(8, 2);
		...
		cout << "Axz transposed" << endl;
		cout << Axz.transpose() << endl;


		cout << VZ << endl;

		cout << "Axz transposed * VZ" << endl;
		cout << Axz.transpose() * VZ  << endl;
		...
		ToMinimize sdist(DT_fit_xx,DT_fit_yy,DT_fit_zz);
		ROOT::Math::Functor fcn(sdist,4);
		double pStart[4] = {1,1,1,1};
		fitter.SetFCN(fcn,pStart);
		//for (int i = 0; i < 4; ++i) fitter.Config().ParSettings(i).SetStepSize(0.01);//default?
		for (int i = 0; i < 4; ++i) fitter.Config().ParSettings(i).SetStepSize(0.01);
		//for (int i = 0; i < 4; ++i) fitter.Config().ParSettings(i).SetLimits(0,70);
		...
		//They claim a linear fit can be done in a TGraph2D, let's see
		//TGraph3D *pr = new TGraph3D(DT_fit_xx.size());
		for (unsigned int i = 0; i <DT_fit_xx.size();++i)
		  {
		    gr->SetPoint(i,DT_fit_xx.at(i),DT_fit_yy.at(i),DT_fit_zz.at(i));
		    h3->Fill(DT_fit_xx.at(i),DT_fit_yy.at(i),DT_fit_zz.at(i));
		  }
		...
		f3->SetLineWidth(2);
		//pr->Delete();
		//TCanvas *can = new TCanvas("can_xy","can_xy",900,800);
		//gr->Draw("p0");
	...
	//We should check cosmics also when there are no clusters
	float min_dz_cosmic_ecal = 99999.;
	float min_dxy_cosmic_ecal = 99999.;
	for(unsigned int it_c=0; it_c<CosmicMuonsOneLeg->size(); it_c++)
	  {
	    float tmp_diff_dz = abs(abs(mean_ECAL_tag_z)-abs(float(CosmicMuonsOneLeg->at(it_c).dz))/100.);
	    min_dz_cosmic_ecal = std::min(min_dz_cosmic_ecal,tmp_diff_dz);
	    float tmp_diff_dxy = abs(sqrt( pow(mean_ECAL_tag_x,2)+ pow(mean_ECAL_tag_y,2) ) - abs(float(CosmicMuonsOneLeg->at(it_c).dxy))/100. );
	    min_dxy_cosmic_ecal = std::min(min_dxy_cosmic_ecal,tmp_diff_dxy);
	  }
	if(CosmicMuonsOneLeg->size()>0 && taggedEcalRecHitsAK4.size()>0)
	  {
	    std::cout << " @@@@@@@@@@@@@@@@ " << std::endl;
	    if(isMC) std::cout << EventNumber << " & " << sqrt( pow(mean_ECAL_tag_x,2)+ pow(mean_ECAL_tag_y,2) ) << " & " << min_dxy_cosmic_ecal << " & " << mean_ECAL_tag_z << " & " << min_dz_cosmic_ecal << "\\\\ " << std::endl;
	    else std::cout << RunNumber << " & " << sqrt( pow(mean_ECAL_tag_x,2)+ pow(mean_ECAL_tag_y,2) ) << " & " << min_dxy_cosmic_ecal << " & " << mean_ECAL_tag_z << " & " << min_dz_cosmic_ecal << "\\\\ " << std::endl;
	  }
