...
OUTPUTDIR = ("/nfs/dust/cms/group/cms-llp/v6_calo_AOD/v6_calo_AOD_%s_reinterpretation/")%(RUN_ERA)
...
if run_condor:
    ...
    for s in sample_to_loop:
        ...
        for subd in num_subdirs:
            ...
            cond_name = "skim_"+str(RUN_ERA)+"_v6_reinterpretation"#+"_positive0p7"#"_syst_unc_JERUp"##+"Gen"#+"_nophoton"#+"_BeamHalo"##+"_bin_1_2"#+"_test"#
            ...
            for a in range(0,len(root_files),root_files_per_job):
                ...
                with open('job_skim_'+str(j_num)+'.sh', 'w') as fout:
                    ...
                    for b in np.arange(start,stop+1):
                        ...
                        fout.write('echo ../bin/slc7_amd64_gcc820/tf_and_skim_v6_reinterpretation ' + INPS+root_files[b] + '  ' + OUT+root_files[b] + skip_string  + ' ' + str(isSignal) + ' ' + str(isData)  + ' ' + mc_PU_file+ ' ' + mc_trigger_file + ' ' + mc_trigger_string + ' '  + doRegion + ' ' + smearing_file + ' \n')
                        fout.write('../bin/slc7_amd64_gcc820/tf_and_skim_v6_reinterpretation ' + INPS+root_files[b] + '  ' + OUT+root_files[b] + skip_string  + ' ' + str(isSignal) + ' ' + str(isData)  + ' ' + mc_PU_file+ ' ' + mc_trigger_file + ' ' + mc_trigger_string + ' ' + doRegion + ' ' + smearing_file + ' \n')
